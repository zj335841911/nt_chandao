...
import IbzReportlyAuthService from '@/app-core/auth-service/ibz-reportly/ibz-reportly-auth-service';
 ...
 *
 * @export
 * @class IbzReportlyUIActionBase
 * @extends {UIActionBase}
 */
export default class IbzReportlyUIActionBase extends EntityUIActionBase {
     ...
     */
    public async IbzReportly_MobSubmit(args: any[], contextJO: any = {}, paramJO: any = {}, $event?: any, xData?: any, container?: any, srfParentDeName?: string): Promise<any> {
        ...
        const backend = async () => {
            const curUIService: any = await this.globaluiservice.getAppEntityService('ibzreportly');
            const response: any = await curUIService.Submit(_context, _params);
            if (response && response.status === 200) {
                this.notice.success('提交成功！');
                if (xData && xData.refresh && xData.refresh instanceof Function) {
                    xData.refresh(args);
                    AppCenterService.notifyMessage({name:"IbzReportly",action:'appRefresh',data:args});
                }
            } else {
