...
//getwflink
mock.onGet(new RegExp(/^\/wfcore\/pms-app-mob\/stories\/[a-zA-Z0-9\-\;]+\/usertasks\/[a-zA-Z0-9\-\;]+\/ways$/)).reply((config: any) => {
...
// getwfstep
mock.onGet(new RegExp(/^\/wfcore\/pms-app-mob\/stories\/process-definitions-nodes$/)).reply((config: any) => {
...
// FetchProductPlanStories
mock.onGet(new RegExp(/^\/products\/([a-zA-Z0-9\-\;]{1,35})\/stories\/fetchproductplanstories$/)).reply((config: any) => {
    console.groupCollapsed("实体:story 方法: FetchProductPlanStories");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
    ...
    const paramArray:Array<any> = ['id'];
    let tempValue: any = {};
    const matchArray:any = new RegExp(/^\/products\/([a-zA-Z0-9\-\;]{1,35})\/stories\/fetchproductplanstories$/).exec(config.url);
    if(matchArray && matchArray.length >1 && paramArray && paramArray.length >0){
        paramArray.forEach((item: any, index: number) => {
            Object.defineProperty(tempValue, item, {
                enumerable: true,
...
// FetchProductPlanStories
mock.onGet(new RegExp(/^\/stories\/fetchproductplanstories$/)).reply((config: any) => {
    console.groupCollapsed("实体:story 方法: FetchProductPlanStories");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
...
// FetchProductPlanStories
mock.onGet(new RegExp(/^\/stories\/fetchproductplanstories(\?[\w-./?%&=,]*)*$/)).reply((config: any) => {
    console.groupCollapsed("实体:story 方法: FetchProductPlanStories");
    console.table({url:config.url, method: config.method, data:config.data});
    if(config.url.includes('page')){
        let url = config.url.split('?')[1];
