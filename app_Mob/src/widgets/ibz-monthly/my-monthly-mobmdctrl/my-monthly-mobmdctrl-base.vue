...
export default class MyMonthlyBase extends Vue implements ControlInterface {
     ...
     * 建构部件服务对象
     *
     * @type {MyMonthlyService}
     * @memberof MyMonthly
     */
    protected service: MyMonthlyService = new MyMonthlyService({$store:this.$store});
   ...
   /**
    ...
    public checkboxSelect(item:any){
        ...
        let count = this.selectedArray.findIndex((_item:any) => {
            return _item.ibzmonthlyid == item.ibzmonthlyid;
        });
        if(count == -1){
            this.selectedArray.push(item);
        }else{
...
<style lang='less'>
@import './my-monthly-mobmdctrl.less';
</style>