<template>
    <div  class="app-mob-mdctrl productline-mdctrl ">
            ...
            <ion-list class="items" ref="ionlist">
                <template v-if="(viewType == 'DEMOBMDVIEW9') && controlStyle != 'SWIPERVIEW' ">
                    <ion-item-sliding ref="sliding" v-for="(item,index) in items" @click="item_click(item)" :key="item.srfkey" class="app-mob-mdctrl-item" :disabled="item.sliding_disabled" @ionDrag="ionDrag">
                        <ion-item>
                            <!-- 列表视图样式 -->
                            <app-list-index-text :dataItemNames = "[]" :item="item" :index="index" major="productlinename" v-if="controlStyle.substring(0,8) === 'LISTVIEW'"></app-list-index-text>
                                <!-- 图标视图样式 -->
...
export default class MobBase extends Vue implements ControlInterface {
     ...
     * @memberof MobBase
     */  
    public deUIService:ProductLineUIService = new ProductLineUIService(this.$store);
    ...
    */
    ...
    public async remove(datas: any[]): Promise<any> {
        ...
        datas.forEach((data: any, index: number) => {
            keys.push(data.productlineid);
            if (index < 5) {
                if (!Object.is(infoStr, '')) {
                    infoStr += '、';
                }
                infoStr += data.productlinename;
            }
        });
        if(datas.length <= 0 ){
        ...
        return new Promise((resolve, reject) => {
            const _remove = async () => {
                let _context: any = { productline: keys.join(';') }
...
​           Object.assign(data,this.sort);
    ...
    public checkboxChange(data: any) {
        ...
        this.items.forEach((item: any, index: number) => {
            ...
            if (Object.is(item.productlineid, value)) {
                if (detail.checked) {
   ...
   /**
    ...
    public checkboxSelect(item:any){
        let count = this.selectedArray.findIndex((i) => {
            return i.productlineid == item.productlineid;
        });
        let tempFalg = false;
        if(count == -1){
            tempFalg = true;
        ...
        this.items.forEach((_item:any,index:number)=>{
            if(_item.productlineid == item.productlineid){
