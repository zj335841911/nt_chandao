...
<app-form-group 
    ...
    :caption="$t('testtask.mobactivite_form.details.group1')" 
    :isShowCaption="false" 
    :titleBarCloseMode="0" 
...
<app-form-item 
    ...
    :caption="$t('testtask.mobactivite_form.details.name')"  
    :labelWidth="1"  
    :isShowCaption="true"
...
<app-form-item 
    ...
    :caption="$t('testtask.mobactivite_form.details.comment')"  
    :labelWidth="60"  
    :isShowCaption="true"
    :disabled="detailsModel.comment.disabled"
    :error="detailsModel.comment.error" 
    :isEmptyCaption="false">
        <app-mob-rich-text-editor :formState="formState" :value="data.comment" @change="(val) =>{this.data.comment =val}" :disabled="detailsModel.comment.disabled" :data="JSON.stringify(this.data)"  name="comment" :uploadparams='{objecttype:"testtask",version:"editor"}' :exportparams='{objecttype:"testtask",version:"editor"}'  style=""/>
...
<app-form-group 
    ...
    :caption="$t('testtask.mobactivite_form.details.grouppanel1')" 
    :isShowCaption="true" 
    :titleBarCloseMode="1" 
    :isInfoGroupMode="false" 
    :data="transformData(data)"
...
<app-form-druipart
    ...
    :caption="$t('testtask.mobactivite_form.details.druipart1')"  
    paramItem='testtask' 
    style="" 
    :formState="formState" 
...
import MobActiviteService from '@/app-core/ctrl-service/test-task/mob-activite-form-service';
...
export default class MobActiviteBase extends Vue implements ControlInterface {

    /**
     * 名称
     *
     * @type {string}
     * @memberof MobActivite
     */
     ...
     * 建构部件服务对象
     *
     * @type {MobActiviteService}
     * @memberof MobActivite
     */
    protected service: MobActiviteService = new MobActiviteService({$store:this.$store});
...
, 
        grouppanel1: new FormGroupPanelModel({ caption: '变更历史', detailType: 'GROUPPANEL', name: 'grouppanel1', visible: true, isShowCaption: true, form: this, uiActionGroup: { caption: '', langbase: 'testtask.mobactivite_form', extractMode: 'ITEM', details: [] } })
, 
        group1: new FormGroupPanelModel({ caption: '测试版本基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'testtask.mobactivite_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
    ...
    protected async save(opt: any = {}, showResultInfo?: boolean, isStateNext: boolean = true): Promise<any> {
        ...
        if (response && response.status === 200) {
            ...
            AppCenterService.notifyMessage({name:"TestTask",action:'appRefresh',data:Object.assign(data,{appRefreshAction:action===this.createAction?false:true})});
            this.$store.dispatch('viewaction/datasaved', { viewtag: this.viewtag });
            this.$nextTick(() => {
                this.formState.next({ type: 'save', data: data });
            });
