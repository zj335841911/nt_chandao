<template>
    <div  class="app-mob-mdctrl user-mdctrl ">
                ...
                <template v-if="(viewType == 'DEMOBMDVIEW9') && controlStyle != 'SWIPERVIEW' ">
                    ...
                    <ion-item-sliding ref="sliding" v-for="item in items" @click="item_click(item)" :key="item.srfkey" class="app-mob-mdctrl-item" :disabled="item.sliding_disabled">
                        <ion-item>
                            ...
                            <!-- 列表视图样式 -->
                            <app-list-index-text :dataItemNames = "[]" :item="item" :index="item.srfkey" major="realname" v-if="controlStyle.substring(0,8) === 'LISTVIEW'"></app-list-index-text>
                                <!-- 图标视图样式 -->
                ...
                <template v-else>
                    <ion-list  v-model="selectedArray"   v-if="isMutli" class="pickUpList">
                        <ion-item v-for="(item, index) of items" :key="index" class="app-mob-mdctrl-item" >
                            ...
                            <ion-label>{{item.realname}}</ion-label>
                        </ion-item>
                    </ion-list>
...
import MobService from '@/app-core/ctrl-service/user/mob-mobmdctrl-service';
    ...
    */
    ...
    public async remove(datas: any[]): Promise<any> {
        ...
        return new Promise((resolve, reject) => {
            const _remove = async () => {
                let _context: any = { user: keys.join(';') }
    ...
    */
    ...
    public checkboxChange(data: any) {
        ...
        this.items.forEach((item: any, index: number) => {
            ...
            if (Object.is(item.userid, value)) {
                if (detail.checked) {
