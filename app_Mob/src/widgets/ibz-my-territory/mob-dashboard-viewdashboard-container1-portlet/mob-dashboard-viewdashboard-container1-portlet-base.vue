    ...
    <ion-row>
        <ion-list class='app-mob-portlet ibzmyterritory-dashboard_container1 '>
            <ion-list-header v-if="editTitle"  class='app-mob-portlet__header'>
                ...
                <span v-if="!isEditTitle"><span v-if="customizeTitle">{{customizeTitle}}</span><span v-else>{{$t(`${this.localeDeName}.views.${this.viewName.toLowerCase()}.mobdashboardviewdashboard_container1_portlet`)}}</span></span>
...
export default class MobDashboardViewdashboard_container1Base extends Vue implements ControlInterface {
    ...
    get editTitle(){
        ...
        return (this.$t(`app.views.${this.viewName.toLowerCase()}.mobdashboardviewdashboard_container1_portlet`) as string)
    }
