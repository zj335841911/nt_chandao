<template>
    <div  class="app-mob-mdctrl ibzweekly-mdctrl ">
            ...
            <ion-list class="items" ref="ionlist">
                <template v-if="(viewType == 'DEMOBMDVIEW9') && controlStyle != 'SWIPERVIEW' ">
                    <ion-item-sliding ref="sliding" v-for="(item,index) in items" @click="item_click(item)" :key="item.srfkey" class="app-mob-mdctrl-item" :disabled="item.sliding_disabled" @ionDrag="ionDrag">
                        <ion-item>
                            <!-- 列表视图样式 -->
                            <app-list-index-text :dataItemNames = "[]" :item="item" :index="index" major="ibzweeklyname" v-if="controlStyle.substring(0,8) === 'LISTVIEW'"></app-list-index-text>
                                <!-- 图标视图样式 -->
                ...
                <template v-else>
                    <ion-list  v-model="selectedArray"   v-if="isMutli" class="pickUpList">
                        <ion-item v-for="(item, index) of items" :key="item.srfkey" class="app-mob-mdctrl-item" >
                            ...
                            <ion-label>{{item.ibzweeklyname}}</ion-label>
                        </ion-item>
                    </ion-list>
...
import MobService from '@/app-core/ctrl-service/ibz-weekly/mob-mobmdctrl-service';
    ...
    */
    ...
    public async remove(datas: any[]): Promise<any> {
        ...
        datas.forEach((data: any, index: number) => {
            keys.push(data.ibzweeklyid);
            if (index < 5) {
                if (!Object.is(infoStr, '')) {
                    infoStr += '、';
                }
                infoStr += data.ibzweeklyname;
            }
        });
        if(datas.length <= 0 ){
        ...
        return new Promise((resolve, reject) => {
            const _remove = async () => {
                let _context: any = { ibzweekly: keys.join(';') }
