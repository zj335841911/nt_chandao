<template>
    <div  class="app-mob-mdctrl ibzreportly-mdctrl ">
            ...
            <ion-list class="items" ref="ionlist">
                <template v-if="(viewType == 'DEMOBMDVIEW9') && controlStyle != 'SWIPERVIEW' ">
                    <ion-item-sliding ref="sliding" v-for="(item,index) in items" @click="item_click(item)" :key="item.srfkey" class="app-mob-mdctrl-item" :disabled="item.sliding_disabled" @ionDrag="ionDrag">
                        <ion-item>
                            <!-- 列表视图样式 -->
                            <app-list-index-text :dataItemNames = "[]" :item="item" :index="index" major="ibzreportlyname" v-if="controlStyle.substring(0,8) === 'LISTVIEW'"></app-list-index-text>
                                <!-- 图标视图样式 -->
                ...
                <template v-else>
                    <ion-list  v-model="selectedArray"   v-if="isMutli" class="pickUpList">
                        <ion-item v-for="(item, index) of items" :key="item.srfkey" class="app-mob-mdctrl-item" >
                            ...
                            <ion-label>{{item.ibzreportlyname}}</ion-label>
                        </ion-item>
                    </ion-list>
...
import MyReportlyService from '@/app-core/ctrl-service/ibz-reportly/my-reportly-mobmdctrl-service';
...
export default class MyReportlyBase extends Vue implements ControlInterface {
     ...
     * 建构部件服务对象
     *
     * @type {MyReportlyService}
     * @memberof MyReportly
     */
    protected service: MyReportlyService = new MyReportlyService({$store:this.$store});
    ...
    */
    ...
    public async remove(datas: any[]): Promise<any> {
        ...
        datas.forEach((data: any, index: number) => {
            keys.push(data.ibzreportlyid);
            if (index < 5) {
                if (!Object.is(infoStr, '')) {
                    infoStr += '、';
                }
                infoStr += data.ibzreportlyname;
            }
        });
        if(datas.length <= 0 ){
        ...
        return new Promise((resolve, reject) => {
            const _remove = async () => {
                let _context: any = { ibzreportly: keys.join(';') }
...
​           Object.assign(data,this.sort);
    ...
    public checkboxChange(data: any) {
        ...
        this.items.forEach((item: any, index: number) => {
            ...
            if (Object.is(item.ibzreportlyid, value)) {
                if (detail.checked) {
   ...
   /**
    ...
    public checkboxSelect(item:any){
        let count = this.selectedArray.findIndex((i) => {
            return i.ibzreportlyid == item.ibzreportlyid;
        });
        let tempFalg = false;
        if(count == -1){
            tempFalg = true;
        ...
        this.items.forEach((_item:any,index:number)=>{
            if(_item.ibzreportlyid == item.ibzreportlyid){
...
<style lang='less'>
@import './my-reportly-mobmdctrl.less';
</style>