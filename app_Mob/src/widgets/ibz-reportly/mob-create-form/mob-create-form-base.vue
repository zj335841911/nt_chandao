<template>
    <div ref='form' class="app-form ibzreportly-form ">
...
<app-form-group 
    ...
    :caption="$t('ibzreportly.mobcreate_form.details.group1')" 
    :isShowCaption="false" 
    :titleBarCloseMode="0" 
...
<app-form-item 
    ...
    uiStyle="DEFAULT"  
    labelPos="LEFT" 
    ref="ibz_reportlyname_item"  
    :itemValue="this.data.ibz_reportlyname" 
    v-show="detailsModel.ibz_reportlyname.visible" 
    :itemRules="this.rules.ibz_reportlyname" 
    :caption="$t('ibzreportly.mobcreate_form.details.ibz_reportlyname')"  
    :labelWidth="100"  
    :isShowCaption="true"
    :disabled="detailsModel.ibz_reportlyname.disabled"
    :error="detailsModel.ibz_reportlyname.error" 
    :isEmptyCaption="false">
        <app-mob-input 
    class="app-form-item-input"  
        type="text"  
    :value="data.ibz_reportlyname"
    unit=""
    :disabled="detailsModel.ibz_reportlyname.disabled" 
    @change="($event)=>this.data.ibz_reportlyname = $event" />
</app-form-item>
...
<app-form-item 
    ...
    :caption="$t('ibzreportly.mobcreate_form.details.content')"  
    :labelWidth="100"  
    :isShowCaption="true"
    :disabled="detailsModel.content.disabled"
    :error="detailsModel.content.error" 
    :isEmptyCaption="false">
        <app-mob-rich-text-editor-pms :formState="formState"  :value="data.content" @change="(val) =>{this.data.content =val}" :disabled="detailsModel.content.disabled" :data="JSON.stringify(this.data)"  name="content" :uploadparams='{objecttype:"reportly",version:"editor"}' :exportparams='{objecttype:"reportly",version:"editor"}'  style=""  @noticeusers_change="(val)=>{this.data.noticeusers =val}"/>
...
<app-form-item 
    ...
    :caption="$t('ibzreportly.mobcreate_form.details.files')"  
    :labelWidth="100"  
    :isShowCaption="true"
    ...
    :uploadParam='{objecttype:"reportly",objectid:"0",version:"1"}' 
    :exportParam='{objecttype:"reportly",objectid:"0",version:"1"}' 
...
<app-form-item 
    ...
    :caption="$t('ibzreportly.mobcreate_form.details.reporttopk')"  
    :labelWidth="100"  
    :isShowCaption="true"
...
<app-form-item 
    ...
    :caption="$t('ibzreportly.mobcreate_form.details.mailto')"  
    :labelWidth="100"  
    :isShowCaption="true"
...
import MobCreateService from '@/app-core/ctrl-service/ibz-reportly/mob-create-form-service';
...
export default class MobCreateBase extends Vue implements ControlInterface {
     ...
     * 建构部件服务对象
     *
     * @type {MobCreateService}
     * @memberof MobCreate
     */
    protected service: MobCreateService = new MobCreateService({$store:this.$store});
     ...
     * @memberof MobCreateBase
     */  
    public deUIService:IbzReportlyUIService = new IbzReportlyUIService(this.$store);
    ...
    protected detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: '汇报基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'ibzreportly.mobcreate_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
        srfkey: new FormItemModel({ caption: '汇报标识', detailType: 'FORMITEM', name: 'srfkey', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        srfmajortext: new FormItemModel({ caption: '汇报名称', detailType: 'FORMITEM', name: 'srfmajortext', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        ibz_reportlyname: new FormItemModel({ caption: '汇报名称', detailType: 'FORMITEM', name: 'ibz_reportlyname', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    /**
     * 监控表单属性 ibz_reportlyname 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof MobCreate
     */
    @Watch('data.ibz_reportlyname')
    onIbz_reportlynameChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'ibz_reportlyname', newVal: newVal, oldVal: oldVal });
    }
     ...
     */
    @Watch('data.content')
    onContentChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'content', newVal: newVal, oldVal: oldVal });
    }
    ...
    public validItem(property:string, data:any):Promise<any>{
        return new Promise((resolve, reject) => {
            ...
            Util.validateItem(property,data,this.rules[property]).then(()=>{
                ...
                let _message :any = (this.$t(`ibzreportly.mobcreate_form.details.${field}`) as string) +' '+ this.$t(`app.form.rules.${message}`);
    ...
    protected afterCreated(){
        if(this.isautoload){
            this.autoLoad({srfkey:this.context.ibzreportly});
        }
        if (this.viewState) {
    ...
    protected async loadDraft(opt: any = {}): Promise<any> {
        ...
        if (response && response.status === 200) {
            const data = response.data;
            if(data.ibzreportly){
                Object.assign(this.context, { ibzreportly: data.ibzreportly });
            }
    ...
    protected async save(opt: any = {}, showResultInfo?: boolean, isStateNext: boolean = true): Promise<any> {
        ...
        Object.assign(this.viewparams,{ ibzreportlyname: arg.ibzreportlyname});
        Object.assign(arg, this.viewparams);
        ...
        if (response && response.status === 200) {
            ...
            AppCenterService.notifyMessage({name:"IbzReportly",action:'appRefresh',data:Object.assign(data,{appRefreshAction:action===this.createAction?false:true})});
            this.$store.dispatch('viewaction/datasaved', { viewtag: this.viewtag });
            this.$nextTick(() => {
                this.formState.next({ type: 'save', data: data });
            });
    ...
    private async remove(opt: Array<any> = [], showResultInfo?: boolean): Promise<any> {
        ...
        if (response && response.status === 200) {
            ...
            AppCenterService.notifyMessage({name:"IbzReportly",action:'appRefresh',data:data});
