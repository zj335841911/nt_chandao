<template>
    <div ref='form' class="app-form ibztaskteam-form ">
...
<app-form-group 
    ...
    :caption="$t('ibztaskteam.mobmain_form.details.group1')" 
    :isShowCaption="false" 
    :titleBarCloseMode="0" 
...
<app-form-item 
    ...
    :caption="$t('ibztaskteam.mobmain_form.details.account')"  
    :labelWidth="130"  
    :isShowCaption="true"
...
<app-form-item 
    ...
    :caption="$t('ibztaskteam.mobmain_form.details.estimate')"  
    :labelWidth="130"  
    :isShowCaption="true"
...
export default class MobMainBase extends Vue implements ControlInterface {
     ...
     * @memberof MobMainBase
     */  
    public deUIService:IbztaskteamUIService = new IbztaskteamUIService(this.$store);
    ...
    protected detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: '任务团队基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'ibztaskteam.mobmain_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
        root: new FormItemModel({ caption: '编号', detailType: 'FORMITEM', name: 'root', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        type: new FormItemModel({ caption: '团队类型', detailType: 'FORMITEM', name: 'type', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    public validItem(property:string, data:any):Promise<any>{
        return new Promise((resolve, reject) => {
            ...
            Util.validateItem(property,data,this.rules[property]).then(()=>{
                ...
                let _message :any = (this.$t(`ibztaskteam.mobmain_form.details.${field}`) as string) +' '+ this.$t(`app.form.rules.${message}`);
    ...
    protected afterCreated(){
        if(this.isautoload){
            this.autoLoad({srfkey:this.context.ibztaskteam});
        }
        if (this.viewState) {
        ...
        if(AppCenterService && AppCenterService.getMessageCenter()){
            this.appStateEvent = AppCenterService.getMessageCenter().subscribe(({ name, action, data }) =>{
                ...
                if(Object.is(action,'appRefresh') && data.appRefreshAction && this.context.ibztaskteam){
                    this.refresh([data]);
                }
            })
        }
    ...
    protected async loadDraft(opt: any = {}): Promise<any> {
        ...
        if (response && response.status === 200) {
            const data = response.data;
            if(data.ibztaskteam){
                Object.assign(this.context, { ibztaskteam: data.ibztaskteam });
            }
    ...
    protected async save(opt: any = {}, showResultInfo?: boolean, isStateNext: boolean = true): Promise<any> {
        ...
        if (response && response.status === 200) {
            ...
            AppCenterService.notifyMessage({name:"ibztaskteam",action:'appRefresh',data:Object.assign(data,{appRefreshAction:action===this.createAction?false:true})});
            this.$store.dispatch('viewaction/datasaved', { viewtag: this.viewtag });
            this.$nextTick(() => {
                this.formState.next({ type: 'save', data: data });
            });
