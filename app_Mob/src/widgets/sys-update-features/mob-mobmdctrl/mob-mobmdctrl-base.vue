            ...
            <ion-list class="items" ref="ionlist">
                <template v-if="(viewType == 'DEMOBMDVIEW9') && controlStyle != 'SWIPERVIEW' ">
                    <ion-item-sliding ref="sliding" v-for="item in items" @click="item_click(item)" :key="item.srfkey" class="app-mob-mdctrl-item" :disabled="item.sliding_disabled" @ionDrag="ionDrag">
                        <ion-item>
                            <!-- 列表视图样式 -->
                            <app-list-index-text :dataItemNames = "[]" :item="item" :index="item.srfkey" major="sysupdatefeaturesname" v-if="controlStyle.substring(0,8) === 'LISTVIEW'"></app-list-index-text>
                                <!-- 图标视图样式 -->
                ...
                <template v-else>
                    <ion-list  v-model="selectedArray"   v-if="isMutli" class="pickUpList">
                        <ion-item v-for="(item, index) of items" :key="index" class="app-mob-mdctrl-item" >
                            ...
                            <ion-label>{{item.sysupdatefeaturesname}}</ion-label>
                        </ion-item>
                    </ion-list>
...
import MobService from '@/app-core/ctrl-service/sys-update-features/mob-mobmdctrl-service';
