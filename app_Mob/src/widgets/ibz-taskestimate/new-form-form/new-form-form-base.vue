...
<app-form-item 
    ...
    :caption="$t('ibztaskestimate.newform_form.details.dates')"  
    :labelWidth="130"  
    :isShowCaption="true"
...
, 
    ...
    public validItem(property:string, data:any):Promise<any>{
        return new Promise((resolve, reject) => {
            ...
            Util.validateItem(property,data,this.rules[property]).then(()=>{
                ...
                let _message :any = (this.$t(`ibztaskestimate.newform_form.details.${field}`) as string) +' '+ this.$t(`app.form.rules.${message}`);
    ...
    protected afterCreated(){
        if(this.isautoload){
            this.autoLoad({srfkey:this.context.ibztaskestimate});
        }
        if (this.viewState) {
    ...
    protected async save(opt: any = {}, showResultInfo?: boolean, isStateNext: boolean = true): Promise<any> {
        ...
        if (response && response.status === 200) {
            ...
            AppCenterService.notifyMessage({name:"IbzTaskestimate",action:'appRefresh',data:Object.assign(data,{appRefreshAction:action===this.createAction?false:true})});
            this.$store.dispatch('viewaction/datasaved', { viewtag: this.viewtag });
            this.$nextTick(() => {
                this.formState.next({ type: 'save', data: data });
            });
    ...
    private async remove(opt: Array<any> = [], showResultInfo?: boolean): Promise<any> {
        ...
        if (response && response.status === 200) {
            ...
            AppCenterService.notifyMessage({name:"IbzTaskestimate",action:'appRefresh',data:data});
