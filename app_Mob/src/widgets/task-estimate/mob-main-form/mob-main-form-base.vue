<template>
    <div ref='form' class="app-form taskestimate-form ">
...
<app-form-item 
    ...
    :caption="$t('taskestimate.mobmain_form.details.consumed')"  
    :labelWidth="130"  
    :isShowCaption="true"
...
<app-form-item 
    ...
    :caption="$t('taskestimate.mobmain_form.details.left')"  
    :labelWidth="130"  
    :isShowCaption="true"
...
<app-form-item 
    ...
    :caption="$t('taskestimate.mobmain_form.details.work')"  
    :labelWidth="130"  
    :isShowCaption="true"
...
import MobMainService from '@/app-core/ctrl-service/task-estimate/mob-main-form-service';
...
, 
    ...
    public validItem(property:string, data:any):Promise<any>{
        return new Promise((resolve, reject) => {
            ...
            Util.validateItem(property,data,this.rules[property]).then(()=>{
                ...
                let _message :any = (this.$t(`taskestimate.mobmain_form.details.${field}`) as string) +' '+ this.$t(`app.form.rules.${message}`);
    ...
    protected afterCreated(){
        if(this.isautoload){
            this.autoLoad({srfkey:this.context.taskestimate});
        }
        if (this.viewState) {
        ...
        if(AppCenterService && AppCenterService.getMessageCenter()){
            this.appStateEvent = AppCenterService.getMessageCenter().subscribe(({ name, action, data }) =>{
                if(!Object.is(name,"TaskEstimate")){
                    return;
                }
                if(Object.is(action,'appRefresh') && data.appRefreshAction && this.context.taskestimate){
                    this.refresh([data]);
                }
            })
        }
    ...
    protected async loadDraft(opt: any = {}): Promise<any> {
        ...
        if (response && response.status === 200) {
            const data = response.data;
            if(data.taskestimate){
                Object.assign(this.context, { taskestimate: data.taskestimate });
            }
    ...
    protected async save(opt: any = {}, showResultInfo?: boolean, isStateNext: boolean = true): Promise<any> {
        ...
        Object.assign(this.viewparams,{ id: arg.id});
        Object.assign(arg, this.viewparams);
        ...
        if (response && response.status === 200) {
            ...
            AppCenterService.notifyMessage({name:"TaskEstimate",action:'appRefresh',data:Object.assign(data,{appRefreshAction:action===this.createAction?false:true})});
            this.$store.dispatch('viewaction/datasaved', { viewtag: this.viewtag });
            this.$nextTick(() => {
                this.formState.next({ type: 'save', data: data });
            });
    ...
    private async remove(opt: Array<any> = [], showResultInfo?: boolean): Promise<any> {
        ...
        if (response && response.status === 200) {
            ...
            AppCenterService.notifyMessage({name:"TaskEstimate",action:'appRefresh',data:data});
