    ...
    <ion-row>
        <ion-list class='app-mob-portlet task-dashboard_sysportlet2 '>
            <ion-list-header v-if="editTitle"  class='app-mob-portlet__header'>
                ...
                <span v-if="!isEditTitle"><span v-if="customizeTitle">{{customizeTitle}}</span><span v-else>{{$t(`${this.localeDeName}.views.${this.viewName.toLowerCase()}.mytaskmob_portlet`)}}</span></span>
            ...
            <task-ass-mob-mdview9 :_context="JSON.stringify(context)" :isChildView="true" :isPortalView="true" :_viewparams="JSON.stringify(viewparams)" viewDefaultUsage="includedView" ></task-ass-mob-mdview9>
        </ion-list>
...
export default class MyTaskMobBase extends Vue implements ControlInterface {
     ...
     */
    protected async dashboard_sysportlet2_u2be114f_click(params: any = {}, tag?: any, $event?: any): Promise<any> {
    ...
    protected actionBarModelData: any[] = [
        {
            viewlogicname: "dashboard_sysportlet2_u2be114f_click",
            name: "更多",
        }
    ];
    ...
    protected handleItemClick($event: any) {
        if (Object.is($event, 'dashboard_sysportlet2_u2be114f_click')) {
            this.dashboard_sysportlet2_u2be114f_click(null);
        }
    }
    ...
    get editTitle(){
        ...
        return (this.$t(`app.views.${this.viewName.toLowerCase()}.mytaskmob_portlet`) as string)
    }
