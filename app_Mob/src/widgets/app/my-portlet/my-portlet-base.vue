    ...
    <ion-row>
        <ion-list class='app-mob-portlet  '>
            <ion-list-header v-if="editTitle"  class='app-mob-portlet__header'>
                ...
                <span v-if="!isEditTitle"><span v-if="customizeTitle">{{customizeTitle}}</span><span v-else>{{$t(`app.views.${this.viewName.toLowerCase()}.my_portlet`)}}</span></span>
...
import MyService from '@/app-core/ctrl-service/app/my-portlet-service';
...
export default class MyBase extends Vue implements ControlInterface {
    ...
    get editTitle(){
        ...
        return (this.$t(`app.views.${this.viewName.toLowerCase()}.my_portlet`) as string)
    }
