<template>
    <div ref='searchform' class="app-form ibzreport-searchform ">
...
<app-form-item 
    ...
    :caption="$t('ibzreport.mobdef_searchform.details.n_type_eq')"  
    :labelWidth="100"  
    :isShowCaption="true"
...
, 
        n_type_eq: new FormItemModel({ caption: '模板', detailType: 'FORMITEM', name: 'n_type_eq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };
    ...
    protected afterCreated(){
        if(this.isautoload){
            this.autoLoad({srfkey:this.context.ibzreport});
        }
        if (this.viewState) {
        ...
        if(AppCenterService && AppCenterService.getMessageCenter()){
            this.appStateEvent = AppCenterService.getMessageCenter().subscribe(({ name, action, data }) =>{
                ...
                if(Object.is(action,'appRefresh') && data.appRefreshAction && this.context.ibzreport){
                    this.refresh([data]);
                }
            })
        }
    ...
    protected async loadDraft(opt: any = {}): Promise<any> {
        ...
        if (response && response.status === 200) {
            const data = response.data;
            if(data.ibzreport){
                Object.assign(this.context, { ibzreport: data.ibzreport });
            }
