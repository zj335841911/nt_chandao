...
, 
    ...
    protected afterCreated(){
        ...
        if(AppCenterService && AppCenterService.getMessageCenter()){
            this.appStateEvent = AppCenterService.getMessageCenter().subscribe(({ name, action, data }) =>{
                if(!Object.is(name,"Bug") && data.appRefreshAction){
                    return;
                }
    ...
    protected async save(opt: any = {}, showResultInfo?: boolean, isStateNext: boolean = true): Promise<any> {
        ...
        if (response && response.status === 200) {
            ...
            AppCenterService.notifyMessage({name:"Bug",action:'appRefresh',data:data:Object.assign(data,{appRefreshAction:action===this.updateAction?false:true})}});
            this.$store.dispatch('viewaction/datasaved', { viewtag: this.viewtag });
            this.$nextTick(() => {
                this.formState.next({ type: 'save', data: data });
            });
