            ...
            <ion-list class="items" ref="ionlist">
                <template v-if="(viewType == 'DEMOBMDVIEW9') && controlStyle != 'SWIPERVIEW' ">
                    <ion-item-sliding ref="sliding" v-for="(item,index) in items" @click="item_click(item)" :key="item.srfkey" class="app-mob-mdctrl-item" :disabled="item.sliding_disabled" @ionDrag="ionDrag">
                        <ion-item>
                            <!-- 列表视图样式 -->
                            <app-list-index-text :dataItemNames = "['pri','resolution','pri1','severity',]" :item="item" :index="index" major="title" v-if="controlStyle.substring(0,8) === 'LISTVIEW'"></app-list-index-text>
                                <!-- 图标视图样式 -->
...
export default class MOB_ProductPlanLink_BugBase extends Vue implements ControlInterface {
     ...
     *
     * @param {any} args
     * @memberof  MOB_ProductPlanLink_BugBase
     */
     ...
     * 建构部件服务对象
     *
     * @type {MOB_ProductPlanLink_BugService}
     * @memberof MOB_ProductPlanLink_Bug
     */
    protected service: MOB_ProductPlanLink_BugService = new MOB_ProductPlanLink_BugService({$store:this.$store});
...
<style lang='less'>
@import './mob-product-plan-link-bug-mobmdctrl.less';
</style>