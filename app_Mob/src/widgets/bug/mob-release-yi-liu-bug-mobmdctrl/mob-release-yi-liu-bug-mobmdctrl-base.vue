                ...
                <ion-list class="items" ref="ionlist"  >
                  <ion-item-sliding  :ref="item.srfkey" v-for="(item,index) in items" @click="item_click(item)" :key="item.srfkey" class="app-mob-mdctrl-item" :disabled="item.sliding_disabled" @ionDrag="ionDrag">
                        <ion-item-options v-if="controlStyle != 'LISTVIEW3'" side="end">
                            <ion-item-option v-show="item.releaseUnlinkBugByLeftBug.visabled" :disabled="item.releaseUnlinkBugByLeftBug.disabled" color="primary" @click="mdctrl_click($event, 'uf22fc2b', item)"><app-mob-icon v-if="item.releaseUnlinkBugByLeftBug.icon && item.releaseUnlinkBugByLeftBug.isShowIcon" :name="item.releaseUnlinkBugByLeftBug.icon"></app-mob-icon><ion-label v-if="item.releaseUnlinkBugByLeftBug.isShowCaption">移除bug</ion-label></ion-item-option>
                        </ion-item-options>
                    <ion-item>
...
import MOB_Release_YiLiuBugService from '@/app-core/ctrl-service/bug/mob-release-yi-liu-bug-mobmdctrl-service';
...
export default class MOB_Release_YiLiuBugBase extends Vue implements ControlInterface {
     ...
     *
     * @param {any} args
     * @memberof  MOB_Release_YiLiuBugBase
     */
     ...
     * 建构部件服务对象
     *
     * @type {MOB_Release_YiLiuBugService}
     * @memberof MOB_Release_YiLiuBug
     */
    protected service: MOB_Release_YiLiuBugService = new MOB_Release_YiLiuBugService({$store:this.$store});
     ...
     * @memberof MdctrlBase
     */
    protected async mdctrl_uf22fc2b_click(params: any = {}, tag?: any, $event?: any): Promise<any> {
        ...
        if (curUIService) {
            curUIService.Bug_releaseUnlinkBugByLeftBug(datas, contextJO, paramJO, $event, xData, this);
        }
    }
...
​           Object.assign(data,this.sort);
    ...
    */
    ...
    public mdctrl_click($event: any, tag: any, item: any): void {
        ...
        this.selectedArray = [];
        this.selectedArray.push(item);
        if (Object.is(tag, 'uf22fc2b')) {
            this.mdctrl_uf22fc2b_click();
        }
   ...
   /**
    ...
    public ActionModel:any ={
        releaseUnlinkBugByLeftBug: { name: 'releaseUnlinkBugByLeftBug',disabled: false, visabled: true,noprivdisplaymode:1,dataaccaction: 'SRFUR__BUG_UNLINK_BUT', target: 'SINGLEKEY',icon:'unlink',isShowCaption:false,isShowIcon:true}
    };
...
<style lang='less'>
@import './mob-release-yi-liu-bug-mobmdctrl.less';
</style>