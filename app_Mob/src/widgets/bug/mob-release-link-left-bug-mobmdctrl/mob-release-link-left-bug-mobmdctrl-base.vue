            ...
            <ion-list class="items" ref="ionlist">
                <template v-if="(viewType == 'DEMOBMDVIEW9') && controlStyle != 'SWIPERVIEW' ">
                    <ion-item-sliding ref="sliding" v-for="(item,index) in items" @click="item_click(item)" :key="item.srfkey" class="app-mob-mdctrl-item" :disabled="item.sliding_disabled" @ionDrag="ionDrag">
                        <ion-item>
                            <!-- 列表视图样式 -->
                            <app-list-index-text :dataItemNames = "['resolution','pri1','pri','severity',]" :item="item" :index="index" major="title" v-if="controlStyle.substring(0,8) === 'LISTVIEW'"></app-list-index-text>
                                <!-- 图标视图样式 -->
...
import MOB_ReleaseLink_LeftBugService from '@/app-core/ctrl-service/bug/mob-release-link-left-bug-mobmdctrl-service';
...
export default class MOB_ReleaseLink_LeftBugBase extends Vue implements ControlInterface {
     ...
     *
     * @param {any} args
     * @memberof  MOB_ReleaseLink_LeftBugBase
     */
     ...
     * 建构部件服务对象
     *
     * @type {MOB_ReleaseLink_LeftBugService}
     * @memberof MOB_ReleaseLink_LeftBug
     */
    protected service: MOB_ReleaseLink_LeftBugService = new MOB_ReleaseLink_LeftBugService({$store:this.$store});
...
<style lang='less'>
@import './mob-release-link-left-bug-mobmdctrl.less';
</style>