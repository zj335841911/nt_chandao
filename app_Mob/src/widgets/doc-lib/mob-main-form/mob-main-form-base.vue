<template>
    <div ref='form' class="app-form doclib-form ">
...
<app-form-group 
    ...
    :caption="$t('doclib.mobmain_form.details.group1')" 
    :isShowCaption="true" 
    :titleBarCloseMode="0" 
...
<app-form-item 
    ...
    :caption="$t('doclib.mobmain_form.details.name')"  
    :labelWidth="130"  
    :isShowCaption="true"
...
<app-form-group 
    ...
    :caption="$t('doclib.mobmain_form.details.group2')" 
    :isShowCaption="true" 
    :titleBarCloseMode="0" 
    :isInfoGroupMode="false" 
    :data="transformData(data)"
...
export default class MobMainBase extends Vue implements ControlInterface {
    ...
    protected rules: any = {
        name: [
            { required: true, type: 'string', message: '文档库名称 值不能为空', trigger: 'change' },
            { required: true, type: 'string', message: '文档库名称 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    protected detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: '文档库基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: true, form: this, uiActionGroup: { caption: '', langbase: 'doclib.mobmain_form', extractMode: 'ITEM', details: [] } })
, 
        group2: new FormGroupPanelModel({ caption: '操作信息', detailType: 'GROUPPANEL', name: 'group2', visible: true, isShowCaption: true, form: this, uiActionGroup: { caption: '', langbase: 'doclib.mobmain_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
        srfmajortext: new FormItemModel({ caption: '文档库名称', detailType: 'FORMITEM', name: 'srfmajortext', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        name: new FormItemModel({ caption: '文档库名称', detailType: 'FORMITEM', name: 'name', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    protected afterCreated(){
        if(this.isautoload){
            this.autoLoad({srfkey:this.context.doclib});
        }
        if (this.viewState) {
    ...
    protected async loadDraft(opt: any = {}): Promise<any> {
        ...
        if (response && response.status === 200) {
            const data = response.data;
            if(data.doclib){
                Object.assign(this.context, { doclib: data.doclib });
            }
    ...
    protected async save(opt: any = {}, showResultInfo?: boolean, isStateNext: boolean = true): Promise<any> {
        ...
        if (response && response.status === 200) {
            ...
            AppCenterService.notifyMessage({name:"DocLib",action:'appRefresh',data:Object.assign(data,{appRefreshAction:action===this.createAction?false:true})});
            this.$store.dispatch('viewaction/datasaved', { viewtag: this.viewtag });
            this.$nextTick(() => {
                this.formState.next({ type: 'save', data: data });
            });
    ...
    private async remove(opt: Array<any> = [], showResultInfo?: boolean): Promise<any> {
        ...
        if (response && response.status === 200) {
            ...
            AppCenterService.notifyMessage({name:"DocLib",action:'appRefresh',data:data});
