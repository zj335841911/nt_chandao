<template>
    <div  class="app-mob-mdctrl sysupdatelog-mdctrl ">
...
import MobService from '@/app-core/ctrl-service/sys-update-log/mob-mobmdctrl-service';
    ...
    */
    ...
    public async remove(datas: any[]): Promise<any> {
        ...
        datas.forEach((data: any, index: number) => {
            keys.push(data.sysupdatelogid);
            if (index < 5) {
                if (!Object.is(infoStr, '')) {
                    infoStr += '、';
                }
                infoStr += data.sysupdatelogname;
            }
        });
        if(datas.length <= 0 ){
        ...
        return new Promise((resolve, reject) => {
            const _remove = async () => {
                let _context: any = { sysupdatelog: keys.join(';') }
...
​           Object.assign(data,this.sort);
    ...
    public checkboxChange(data: any) {
        ...
        this.items.forEach((item: any, index: number) => {
            ...
            if (Object.is(item.sysupdatelogid, value)) {
                if (detail.checked) {
   ...
   /**
    ...
    public checkboxSelect(item:any){
        let count = this.selectedArray.findIndex((i) => {
            return i.sysupdatelogid == item.sysupdatelogid;
        });
        let tempFalg = false;
        if(count == -1){
            tempFalg = true;
        ...
        this.items.forEach((_item:any,index:number)=>{
            if(_item.sysupdatelogid == item.sysupdatelogid){
