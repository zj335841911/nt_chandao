    ...
    <div class="app-picker">
      ...
      >
        ...
        <template v-slot:suffix>
          ...
          ></i>
          <i
            v-if="!Object.is(editortype, 'ac') && showButton"
...
export default class AppPicker extends Vue {
  ...
  get refvalue() {
    if (this.valueitem && this.data) {
      if (
        this.valFormat &&
        this.valFormat.hasOwnProperty(this.data[this.valueitem])
      ) {
  ...
  public created() {
    if (
      !Object.is(this.editortype, "pickup-no-ac") &&
      !Object.is(this.editortype, "dropdown")
    ) {
   ...
   */
  public mounted() {
    const dropdownDom: any = this.$el.getElementsByClassName(
      "app-picker-dropdown"
    )[0];
  ...
  public onACSelect(item: any): void {
    ...
    if (this.valueitem) {
      this.$emit("formitemvaluechange", {
        name: this.valueitem,
        value: item[this.deKeyField]
      });
    }
    if (this.name) {
      this.$emit("formitemvaluechange", {
        name: this.name,
        value: item[this.deMajorField]
      });
    }
  }
  ...
  public onSelect(val: string) {
    let index = this.items.findIndex(item =>
      Object.is(item[this.deKeyField], val)
    );
    if (index >= 0) {
  ...
  public openView($event: any): void {
    ...
    let _param = data.param;
    _param.selectedData = [
      { srfkey: this.data[this.valueitem], srfmajortext: this.curvalue }
    ];
    // 判断打开方式
  ...
  private openRedirectView($event: any, view: any, data: any): void {
    this.$http
      ...
      .then((response: any) => {
        ...
        if (
          Object.is(result.openmode, "POPUPAPP") &&
          result.url &&
          !Object.is(result.url, "")
        ) {
          this.openPopupApp(result.url);
        } else if (
          Object.is(result.openmode, "INDEXVIEWTAB") ||
          Object.is(result.openmode, "")
        ) {
          // 所有数据保持在同一级
  ...
  public openLinkView($event: any): void {
    ...
    if (view.isRedirectView) {
      ...
      Object.is(view.placement, "INDEXVIEWTAB") ||
      Object.is(view.placement, "")
    ) {
