  ...
  <div class="update-password">
    ...
    <div class="password-item">
      <label for>
        ...
        <Input
          type="password"
          v-model="confirmPwd"
          :disabled="!this.newPwd"
          @on-blur="confirmVaild"
        />
      </label>
    </div>
    <div class="password-item password-btn">
      <Button
        type="primary"
        long
        :disabled="!oldPwd || !newPwd || !confirmPwd || disUpdate"
        @click="updatePwd"
        >{{ $t("components.appUpdatePassword.sure") }}</Button
      >
    </div>
  </div>
</template>
...
export default class AppUpdatePassword extends Vue {
   ...
   */
  public updatePwd() {
    const post: Promise<any> = this.entityService.changPassword(null, {
      oldPwd: this.oldPwd,
      newPwd: this.newPwd
    });
    post
