    ...
    >
      <Input
        ...
        :placeholder="
          placeholder
            ? placeholder
            : $t('components.appPickerSelectView.placeholder')
        "
        :disabled="disabled"
...
export default class AppPickerSelectView extends Vue {
  ...
  public onValueChange(newVal: any, oldVal: any) {
    if (this.isSingleSelect) {
      ...
      if (!this.data || !this.valueitem || !this.data[this.valueitem]) {
        ...
        _viewparam.selectedData = [
          { srfkey: this.data[this.valueitem], srfmajortext: this.value }
        ];
  ...
  public onViewdatasChange($event: any) {
    ...
    if (this.isSingleSelect) {
      this.visible = false;
      if (this.valueitem) {
        let tempvalue = $event[0][this.deKeyField]
          ? $event[0][this.deKeyField]
          : $event[0].srfkey;
      ...
      if (this.name) {
        let tempvalue = $event[0][this.deMajorField]
          ? $event[0][this.deMajorField]
