<template>
  <div class="app-file-upload">
    <el-row>
      <el-col
        v-if="rowPreview && files.length > 0"
        :span="12"
        class="upload-col"
      >
        <el-button
          ...
          "
          >{{ $t("components.appFileUpload.preview")
          }}<Badge :count="files.length" type="info"></Badge
        ></el-button>
      </el-col>
      <el-col
        :span="rowPreview && files.length > 0 ? 12 : 24"
        class="upload-col"
      >
        <el-upload
          :disabled="disabled"
        ...
        >
          <el-button
            ...
            >{{ this.$t("app.fileUpload.caption") }}</el-button
          >
          <i v-if="isdrag" class="el-icon-upload"></i>
          <div
            v-if="isdrag"
            class="el-upload__text"
            v-html="$t('components.appFileUpload.uploadText')"
          ></div>
        </el-upload>
      </el-col>
    </el-row>
    <modal
      ...
      footer-hide
      class-name="upload-preview-modal"
    >
      <ul class="">
        <li
          v-for="(file, index) in files"
          :key="index"
          class="preview-file-list-item"
        >
...
export default class AppFileUpload extends Vue {
  ...
  public getParams() {
    ...
    if (this.uploadparams && !Object.is(this.uploadparams, "")) {
      upload_params = this.$util.computedNavData(
        _data,
        param,
        context,
        uploadparams
    ...
    if (this.exportparams && !Object.is(this.exportparams, "")) {
      export_params = this.$util.computedNavData(
        _data,
        param,
        context,
        exportparams
  ...
  public beforeUpload(file: any) {
    if (this.imageOnly) {
      ...
      const isImage = imageTypes.some((type: any) =>
        Object.is(type, file.type)
      );
      if (!isImage) {
        this.$Notice.error({
  ...
  public onError(error: any, file: any, fileList: any) {
    this.$Notice.error({
      title: this.$t("components.appFileUpload.uploadError") as any
    });
  }
