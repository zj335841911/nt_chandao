<template>
  <div class="app-data-upload-view">
    <el-row :gutter="20">
      <el-col :span="4">
        <i-button type="primary" @click="handleUpLoad">{{
          $t("components.appDataUploadView.selectfile")
        }}</i-button>
        <input
          ref="inputUpLoad"
          type="file"
      ...
      <el-col :span="4">
        <i-button type="primary" @click="uploadServer">{{
      ...
      <el-col :span="16">
        <div class="import-temp">
          <span style="cursor: pointer;" @click="downloadTemp">{{
            $t("components.appDataUploadView.datatemplate")
          }}</span>
        </div>
      </el-col>
    </el-row>
    ...
    <el-row style="height:480px;padding: 0px 12px;">
      <div class="data-info-content">
        <template v-if="importDataArray.length > 0 && isUploading === false">
          <ul>
            <li
              v-for="(item, index) in importDataArray"
              :key="index"
              class="font-class"
            >
              {{
                item[importUniqueItem]
                  ? $t("components.appDataUploadView.dataid") +
                    item[importUniqueItem] +
                    $t("components.appDataUploadView.read") +
                    "......"
                  : $t("components.appDataUploadView.read") + "......"
              }}
            </li>
        ...
        <template v-if="hasImported === true && importDataArray.length === 0">
          <span class="font-class">{{
            isUploading === true
              ? $t("components.appDataUploadView.importing") + "......"
              : promptInfo
          }}</span>
        </template>
      </div>
    ...
    <el-row>
      ...
      <el-col :span="2" :offset="22">
        <i-button type="primary" @click="handleOK">{{
...
export default class AppDataUploadView extends Vue {
  ...
  public downloadTemp() {
    this.importExcel(
      this.viewparams.appDeLogicName +
        this.$t("components.appDataUploadView.datatemp"),
      []
    );
  }
  ...
  public uploadServer() {
    ...
    try {
      this.entityService
        .getService(this.viewparams.serviceName)
        .then((service: any) => {
          service
            .ImportData(this.viewdata, {
              name: this.importId,
              importData: tempDataArray
            })
            .then((res: any) => {
              const result: any = res.data;
              if (result && result.rst !== 0) {
                this.promptInfo = this.$t(
  ...
  public importFile($event: any) {
    ...
    reader.onload = (e: any) => {
      ...
      let xlsxData = XLSX.utils.sheet_to_json(
  ...
  public async getChartAllCodeList() {
    ...
    if (Object.values(this.importDataModel).length > 0) {
      await Object.values(this.importDataModel).forEach(
        async (singleDataModel: any) => {
  ...
  public getCodeList(codeListObject: any): Promise<any> {
    return new Promise((resolve: any, reject: any) => {
      if (codeListObject.tag && Object.is(codeListObject.type, "STATIC")) {
        ...
        codeListObject.tag &&
        Object.is(codeListObject.type, "DYNAMIC")
      ) {
    ...
    data.forEach((item: any) => {
      ...
      Object.keys(item).forEach((ele: any) => {
        if (this.allFieldMap.get(ele).codelist) {
          ...
          let codelistIsNumber: boolean = this.allFieldMap.get(ele).codelist
            .isnumber;
          let curCodeList: any = this.transCodeList(
            codelistTag,
            codelistIsNumber,
            true
          );
   ...
   * @memberof AppDataUploadView
   */
  public transCodeList(
    codeListTag: string,
    codelistIsNumber: boolean,
    isTransform: boolean
  ) {
