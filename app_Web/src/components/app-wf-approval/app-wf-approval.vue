<template>
  <div class="app-wf-approval">
    <div class="app-wf-approval-header">
      ...
      <span
        >{{ data.startUserName
        }}{{ $t("components.appWFApproval.commit") }}</span
      >
    </div>
    <div
      class="app-wf-approval-content"
      v-if="data.usertasks && data.usertasks.length > 0"
    >
      <div
        class="approval-content-item"
      ...
      >
        ...
        <div class="approval-content-item-right">
          <div
            class="approval-content-item-wait"
            v-if="usertask.identitylinks.length > 0"
          >
            {{ $t("components.appWFApproval.wait")
            }}<span
              v-for="(identitylink, inx) in usertask.identitylinks"
              :key="inx"
              >{{ identitylink.displayname
              }}<span v-if="inx > 0">、</span></span
            >{{ $t("components.appWFApproval.handle") }}
          </div>
          <div
            class="approval-content-item-info"
            v-if="usertask.comments.length > 0"
          >
            <div
              v-for="(comment, commentInx) in usertask.comments"
              :key="commentInx"
            >
              <div
              ...
              <div
                class="approval-content-item-info-item approval-content-item-info-bottom"
              >
          ...
          >
            <el-input
              ...
              :rows="2"
              @blur="handleBlur"
              :placeholder="$t('components.appWFApproval.placeholder')"
            ></el-input>
          </div>
        </div>
      </div>
    ...
    <div class="app-wf-approval-bottom">
      <span v-if="data.endTime"
        >{{ data.endTime }}{{ $t("components.appWFApproval.end") }}</span
      >
    </div>
  </div>
</template>
