...
export default class AppOrgSector extends Vue {
  ...
  public getData() {
    if (this.$store.getters.getAppData()) {
      ...
      let _srforgsectorname = this.$store.getters.getAppData().context
        .srforgsectorname;
        ...
        if (response.status == 200) {
          ...
          this.$Notice.error({
            ...
            desc: response.data
              ...
              : (this.$t("components.appOrgSector.errorSwitch") as string)
          });
        }
      })
      .catch((error: any) => {
        this.$Notice.error({
          title: this.$t("app.commonWords.wrong") as string,
          desc: this.$t("components.appOrgSector.errorSwitch") as string
        });
      });
  }
