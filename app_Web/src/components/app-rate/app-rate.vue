...
export default class AppRate extends Vue {
    ...
    */
    public handleModel() {
        if(this.dataModel && this.dataModel.getDataItems && (this.dataModel.getDataItems instanceof Function) && this.dataModel.getDataItems().length > 0) {
    ...
    public handleCodelist(tag: any, codelistType: any) {
      if (Object.is(codelistType,"STATIC")) {
          ...
          if (codelist) {
              ...
              //  获取最大值
              this.maxItem = Math.max.apply(Math, items.map((item: any) => {return item.value}));
          } else {
          ...
          this.codeListService.getItems(tag,_context,_param).then((res:any) => {
              const items = res;
              //  获取最大值
              this.maxItem = Math.max.apply(Math, items.map((item: any) => {return item.value}));
          }).catch((error:any) => {
