<template>
  <div class="ibiz-group-picker">
    ...
    <div class="ibiz-group-footer">
      <el-button size="small" type="primary" @click="onOK">{{
        $t("app.commonWords.ok")
      }}</el-button>
      <el-button size="small" @click="onCancel">{{
        $t("app.commonWords.cancel")
      }}</el-button>
    </div>
  </div>
</template>
...
export default class AppGroupPicker extends Vue {
  ...
  public groupSelect(event: any) {
    ...
    if (event.rselect) {
      let index: number = this.selects.findIndex((item: any) =>
        Object.is(event.rselect, item.id)
      );
      if (index >= 0) {
      ...
      event.select.forEach((key: string) => {
        let index: number = this.selects.findIndex((item: any) =>
          Object.is(key, item.id)
        );
        if (index >= 0) {
          return;
        }
        let item: any = this.cardItems.find((item: any) =>
          Object.is(key, item.id)
        );
        if (item) {
