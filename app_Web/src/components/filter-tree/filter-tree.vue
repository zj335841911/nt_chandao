<template>
  ...
  >
    <template slot-scope="{ node, data }">
      <template
        v-if="Object.is(data.label, '$and') || Object.is(data.label, '$or')"
      >
        <div class="filter-tree-item">
          ...
          <div class="filter-tree-action">
            <i-button
              :title="$t('components.filterTree.title1')"
              @click="onAddItem(data)"
              ><i class="fa fa-plus" aria-hidden="true"></i>
              {{ $t("components.filterTree.title1") }}</i-button
            >
            <i-button
              :title="$t('components.filterTree.title2')"
              @click="onAddGroup(data)"
              ><i class="fa fa-plus" aria-hidden="true"></i>
              {{ $t("components.filterTree.title2") }}</i-button
            >
            <icon
              v-if="!data.isroot"
              type="md-close"
              @click="onRemoveItem(node, data)"
            />
          </div>
        </div>
      </template>
...
export default class FilterTree extends Vue {
  ...
  public created() {
    ...
    this.fields.forEach((field: any) => {
      let index: number = this.fieldItems.findIndex((item: any) =>
        Object.is(item.value, field.prop)
      );
      if (index < 0) {
  ...
  public getModes(field: string) {
    if (this.fieldItems.length > 0) {
      let item: any = this.fieldItems.find((item: any) =>
        Object.is(item.value, field)
      );
      if (item) {
        return item.modes;
      }
