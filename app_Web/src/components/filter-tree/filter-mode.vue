...
export default class FilterMode extends Vue {
  ...
  get fieldFilterMode() {
    if (this.modes && this.modes.length > 0) {
      let index: number = this.modes.findIndex((mode: any) =>
        Object.is(mode.mode, "all")
      );
      if (index < 0) {
        let items: any[] = [];
        this.modes.forEach((mode: any) => {
          let item: any = this.filterMode.find((filter: any) =>
            Object.is(filter["en-US"], mode.mode)
          );
          items.push(item);
        });
        return items;
  ...
  public onChange() {
    this.$nextTick(() => {
      let item: any = this.filterMode.find((filter: any) =>
        Object.is(filter.value, this.curVal)
      );
      if (this.modes && this.modes.length > 0) {
        let mode: any = this.modes.find((mode: any) =>
          Object.is(mode.mode, item["en-US"])
        );
        if (!mode) {
