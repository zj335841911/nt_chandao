...
export default class AppFormGroup extends Vue {
  ...
  get classes(): string[] {
    return [
      "app-form-group",
      this.isShowCaption && this.collapseContant
  ...
  protected getCaption(detail: any): any {
    if (detail.isShowCaption) {
      if (
        isExistAndNotEmpty(detail.uiactiontag) &&
        isExistAndNotEmpty(this.uiActionGroup.langbase)
      ) {
        return this.$t(
          `${this.uiActionGroup.langbase}.uiactions.${detail.uiactiontag}`
        );
      }
  ...
  protected renderActionGroup(): any {
    if (this.uiActionGroup) {
      if (
        this.uiActionGroup.extractMode &&
        Object.is(this.uiActionGroup.extractMode, "ITEMS")
      ) {
        return (
              ...
              {this.uiActionGroup.details.map((detail: any, i: number) => {
                return (
                  <dropdown-item key={i} name={detail.name}>
                    <span
                      class="item"
                      on-click={(e: any) => this.doUIAction(e, detail)}
                    >
                      {this.getIcon(detail)}
      ...
      <card bordered={false} dis-hover={true} class={this.classes}>
        ...
        <div class="show-more" v-show={this.model.showMoreMode === 2}>
          <i-button on-click={() => this.model.changeShowMore()} size="small">
            {this.model?.showMore
              ? this.$t("form.group.hidden_more")
              : this.$t("form.group.show_more")}
          </i-button>
        </div>
      </card>
    );
