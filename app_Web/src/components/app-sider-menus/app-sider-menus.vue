<template>
  <div class="app-sider-menus">
    <template v-for="(item, index) in menus">
      <template
        v-if="item.items && Array.isArray(item.items) && item.items.length > 0"
      >
        <el-submenu :index="item.id" :disabled="item.disabled">
          <template slot="title">
            <img
              v-if="item.icon && !Object.is(item.icon, '')"
              :src="item.icon"
              class="app-menu-icon"
            />
            <i
              v-else-if="item.iconcls && !Object.is(item.iconcls, '')"
              :class="item.iconcls + ' app-menu-icon'"
            ></i>
