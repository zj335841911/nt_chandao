<template>
  <div>
    <div style="position: relative;width: 100%;">
      ...
      <span
        style="position: absolute;right: 5px;color: #c0c4cc;top: 0;font-size: 13px;"
      >
...
export default class AppMpicker extends Vue {
      ...
      try {
        ...
        this.selectItems.forEach((item: any) => {
          ...
          let index = this.items.findIndex(i =>
            Object.is(i[this.deKeyField], item[this.deKeyField])
          );
          if (index < 0) {
            this.items.push({
          ...
          if (
            ...
            srfkeys.forEach((id: any, index: number) => {
              ...
              this.selectItems.push({
                [this.deKeyField]: id,
                [this.deMajorField]: srfmajortexts[index]
              });
              let _index = this.items.findIndex(i =>
                Object.is(i[this.deKeyField], id)
              );
              if (_index < 0) {
                this.items.push({
                  [this.deKeyField]: id,
  ...
  public onSearch(query: any) {
    ...
    let requestException: string = this.$t(
    ...
    if (!this.service) {
      ...
      this.service
        .getItems(
          this.acParams.serviceName,
          this.acParams.interfaceName,
          _context,
          _param
        )
        .then((response: any) => {
    ...
    if (selects.length > 0) {
      selects.forEach((select: any) => {
        let index = this.items.findIndex(item =>
          Object.is(item[this.deKeyField], select)
        );
        if (index >= 0) {
          ...
          index = this.selectItems.findIndex((item: any) =>
            Object.is(item[this.deKeyField], select)
          );
          if (index >= 0) {
  ...
  public onRemove(tag: any) {
    let index = this.selectItems.findIndex((item: any) =>
      Object.is(item[this.deKeyField], tag)
    );
    if (index >= 0) {
      this.selectItems.splice(index, 1);
      let value =
        this.selectItems.length > 0
          ? JSON.stringify(this.formatValue(this.selectItems))
          : "";
      this.$emit("formitemvaluechange", { name: this.name, value: value });
    }
  }
  ...
  public openView() {
    ...
    if (this.pickupView && Object.keys(this.pickupView).length > 0) {
      ...
      _context = Object.assign(_context, {
        srfparentdata: { srfparentkey: this.activeData[this.deKeyField] }
      });
      _viewparams = Object.assign(_viewparams, {
        selectedData: [..._selectItems]
      });
      let formdata = this.activeData;
      const modal: Subject<any> = this.$appmodal.openModal(
        view,
        _context,
        _viewparams
      );
      modal.subscribe((result: any) => {
        ...
        if (this.name && this.activeData) {
          let value =
            selects.length > 0 ? JSON.stringify(this.formatValue(selects)) : "";
          this.$emit("formitemvaluechange", { name: this.name, value: value });
        }
      });
    }
  ...
  public parseValue(value: any[]) {
    ...
    if (this.deKeyField !== "srfkey" || this.deMajorField !== "srfmajortext") {
      value.forEach((item: any) => {
        const params = {
          [this.deMajorField]: item.srfmajortext,
          [this.deKeyField]: item.srfkey
        };
        this.fillExtraValue(params, item);
        result.push(params);
      });
  ...
  public formatValue(value: any[]) {
    ...
    if (this.deKeyField !== "srfkey" || this.deMajorField !== "srfmajortext") {
      value.forEach((item: any) => {
        const params = {
          srfmajortext: item[this.deMajorField],
          srfkey: item[this.deKeyField]
        };
        this.fillExtraValue(params, item);
        result.push(params);
      });
