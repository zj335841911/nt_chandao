    ...
    <div class="design-tree">
      ...
      <div class="design-tree-content">
        <el-menu v-show="!filterVal" :unique-opened="true">
          <template v-for="(item, index) of list">
            <el-submenu :key="item.type + index" :index="item.type + index">
              <div slot="title">
                {{
                  Object.is(item.type, "app")
                    ? $t("components.appDashboardDesign.global")
                    : item.name
                }}
              </div>
        ...
        <el-menu
          v-show="filterVal"
          :unique-opened="true"
          :default-openeds="[filterVal]"
        >
          <template v-for="item of groups">
            <el-submenu
              v-show="filterVal == item.value"
              :key="item.value"
              :index="item.value"
            >
      ...
      <div ref="dragDiv" v-if="dragItem" class="drag-tree-item">
        {{ dragItem.caption }}
      </div>
    </div>
    <div class="design-panel" ref="gridLayoutPanel">
      ...
      >
        <div
          ...
          :style="{
            backgroundSize: `calc((100% - 10px) / ${layoutColNum}) ${layoutRowH +
              10}px`
          }"
        ></div>
        <grid-item
...
export default class AppDashboardDesign extends Vue {
   ...
   */
  public loadList() {
    let post = this.designService.loadPortletList(
      this.context,
      this.viewparams
    );
    post
   ...
   */
  public load() {
    let post = this.designService.loadModel(
  ...
  public save() {
    ...
    let post = this.designService.saveModel(
      this.utilServiceName,
      this.context,
      param
    );
  ...
  public removeItem(id: string) {
    ...
    let item = this.portlets.find((item: any) =>
      Object.is(item.portletCodeName, id)
    );
    if (item) {
      item.moved = false;
    }
  ...
  public handleDrag(event: any) {
    ...
    let item = this.portlets.find((item: any) =>
      Object.is(item.portletCodeName, tag)
    );
    if (item.moved) {
      return;
    }
        ...
        if (
          ...
          if (index === -1) {
            let addItem: any = {
              ...
              w: 4,
              h: 3,
              i: this.dragItem.portletCodeName
            };
