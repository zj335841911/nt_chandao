<template>
  <div class="app-header-user">
    <dropdown
      transfer-class-name="user-dropdownMenu"
      @on-click="userSelect"
      :transfer="true"
    >
      <div class="user">
      ...
      >
        <dropdown-item name="updatepwd" style="font-size: 15px !important;">
          <span
            ><Icon type="ios-create-outline" style="margin-right: 8px;"
          /></span>
        ...
        <dropdown-item name="logout" style="font-size: 15px !important;">
          <span
            ><i
              aria-hidden="true"
              class="ivu-icon ivu-icon-md-power"
  ...
  public mounted() {
    ...
    if (this.$store.getters.getAppData()) {
      if (
        ...
        this.$store.getters.getAppData().context.srfusername
      ) {
        _user.name = this.$store.getters.getAppData().context.srfusername;
      }
      if (
        this.$store.getters.getAppData().context &&
        this.$store.getters.getAppData().context.srfusericonpath
      ) {
