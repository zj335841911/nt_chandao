...
export const createdView = (
  state: any,
  { viewtag, secondtag }: { viewtag: string; secondtag: string }
) => {
  // 原始数据中是否存在
  const appview = state.appviews.find((appview: any) =>
...
export const removeView = (state: any, viewtag: string) => {
  const index = state.createdviews.findIndex((view: any) =>
    Object.is(view.secondtag, viewtag)
  );
  if (index === -1) {
    return;
  }
...
export const setViewDataChange = (
  ...
  const createdview = state.createdviews.find((appview: any) =>
    Object.is(appview.secondtag, viewtag)
  );
  if (!createdview) {
 ...
 * @param param1
 */
export const refreshViewData = (
  state: any,
  { viewtag }: { viewtag: string }
) => {
  const createdview = state.createdviews.find((appview: any) =>
    Object.is(appview.secondtag, viewtag)
  );
