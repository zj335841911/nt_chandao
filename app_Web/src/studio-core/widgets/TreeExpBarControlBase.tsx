...
export class TreeExpBarControlBase extends ExpControlBase {
  ...
  public onSearch(): void {
    ...
    this.viewState.next({
      tag: "treeexpbar_tree",
      action: "filter",
      data: { srfnodefilter: this.srfnodefilter }
    });
  }
   ...
   * @memberof TreeExpBarControlBase
   */
  public treeexpbar_selectionchange(
    args: any[],
    tag?: string,
    $event2?: any
  ): void {
    ...
    if (
      arg &&
      arg.navigateContext &&
      Object.keys(arg.navigateContext).length > 0
    ) {
      let tempData: any = arg.curData
        ? JSON.parse(JSON.stringify(arg.curData))
        : {};
    ...
    if (
      arg &&
      arg.navigateParams &&
      Object.keys(arg.navigateParams).length > 0
    ) {
      let tempData: any = arg.curData
        ? JSON.parse(JSON.stringify(arg.curData))
        : {};
      Object.assign(tempData, arg);
      let _params = this.$util.computedNavData(
        tempData,
        tempContext,
        tempViewparam,
        arg.navigateParams
