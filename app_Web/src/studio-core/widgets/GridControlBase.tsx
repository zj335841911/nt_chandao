...
export class GridControlBase extends MDControlBase {
  ...
  public validate(property: string, data: any, rowIndex: number): Promise<any> {
    return new Promise(resolve => {
      this.$util
        ...
        .catch((res: any) => {
          this.gridItemsModel[rowIndex][property].setError(
        ...
        for (let property of Object.keys(this.rules() as any)) {
          if (!(await this.validate(property, item, index))) {
            ...
            tempMessage =
              tempMessage +
              "<p>" +
              this.gridItemsModel[index][property].error +
              "<p>";
          }
        }
      }
  ...
  get adaptiveState(): boolean {
    return !this.allColumns.find(
      (column: any) => column.show && Object.is(column.unit, "STAR")
    );
  }
  ...
  public async save(args: any[], params?: any, $event?: any, xData?: any) {
          ...
          if (!this.updateAction) {
            ...
            if (item[this.appDeName]) {
              Object.assign(this.context, {
                [this.appDeName]: item[this.appDeName]
              });
            }
    ...
    if (errorItems.length === 0) {
      this.$Notice.success({
        title: "",
        desc: this.$t("app.commonWords.saveSuccess") as string
      });
    } else {
      errorItems.forEach((item: any, index: number) => {
        if (errorMessage[index] && errorMessage[index].data) {
          if (Object.is(errorMessage[index].data.errorKey, "DupCheck")) {
            let errorProp: string = errorMessage[index].data.message.match(
              /\[[a-zA-Z]*\]/
            )[0];
            let name: string = errorProp
              ? this.service.getNameByProp(
                  errorProp.substr(1, errorProp.length - 2)
                )
              : "";
            if (name) {
            ...
            Object.is(
              errorMessage[index].data.errorKey,
  ...
  public load(opt: any = {}, pageReset: boolean = false): void {
    ...
    post
      .then((response: any) => {
        ...
        // 向上下文中填充当前数据
        this.$appService.contextStore.setContextData(
          this.context,
          this.appDeName,
          { items: this.items }
        );
        ...
        if (
          _this.isEnableGroup &&
          _this.group &&
          _this.group instanceof Function
        ) {
          _this.group();
        }
      })
          ...
          .then((response: any) => {
            if (!response || response.status !== 200) {
              this.$Notice.error({
                title: "",
                desc:
                  (this.$t("app.gridpage.delDataFail") as string) +
                  "," +
                  response.info
              });
              return;
              ...
              this.$Notice.success({
                title: "",
                desc: this.$t("app.gridpage.delSuccess") as string
              });
            }
            //删除items中已删除的项
            arr.forEach((data: any) => {
  ...
  public importExcel(data: any = {}, importMode: any = {}): void {
    ...
    const container = this.$appmodal.openModal(
      view,
      JSON.parse(JSON.stringify(this.context)),
      importMode
    );
  ...
  public exportExcel(data: any = {}): void {
    ...
    const doExport = async (_data: any) => {
      ...
      (this.isDeExport ? this.allExportColumns : this.allColumns).forEach(
        (item: any) => {
      ...
      this.$export.exportExcel().then((excel: any) => {
        excel.export_json_to_excel({
          ...
          data, //具体数据 必填
          filename:
            this.appDeLogicName + (this.$t("app.gridpage.grid") as string), //非必填
          autoWidth: true, //非必填
          bookType: "xlsx" //非必填
        });
      });
    ...
    post
      .then((response: any) => {
        if (!response || response.status !== 200) {
          this.$Notice.error({
            title: "",
            desc:
              (this.$t("app.gridpage.exportFail") as string) +
              "," +
              response.info
          });
          return;
      ...
      .catch((response: any) => {
        ...
        this.$Notice.error({
          title: "",
          desc: this.$t("app.gridpage.exportFail") as string
        });
      });
  }
    ...
    for (const codelist of codelistColumns) {
      ...
      if (Object.is(codelist.codelistType, "DYNAMIC")) {
        ...
        jsonData.forEach((row: any) => {
          row[codelist.name] = this.getCodelistValue(
            items,
            row[codelist.name],
            codelist
          );
  ...
  public getCodelistValue(items: any[], value: any, codelist: any) {
    ...
    if (items) {
      ...
      if (Object.is(codelist.renderMode, "number")) {
        ...
        const arrayValue: Array<any> = (value as any).split(
  ...
  public rowDBLClick($event: any): void {
    ...
    if (
      !$event ||
      this.actualIsOpenEdit ||
      Object.is(this.gridRowActiveMode, 0)
    ) {
      return;
    }
    this.selections = [];
    ...
    if (_this.groupAppField) {
      ...
      if (isContain) {
        ...
        if (row.children && row.children.length > 0) {
          ...
          _this.selections = _this.computeCheckedData(
            this.selections,
            row.children
          );
        } else {
   ...
   * @memberof MainBase
   */
  public computeCheckedData(
    selectionArray: any[],
    cancelData: Array<any> | any
  ) {
  ...
  public onSortChange({
    ...
    const dir = Object.is(order, "ascending")
      ? "asc"
      : Object.is(order, "descending")
      ? "desc"
      : "";
    if (
      Object.is(dir, this.minorSortDir) &&
      Object.is(this.minorSortPSDEF, prop)
    ) {
      return;
    }
  ...
  public onRowClassName({
    ...
    const index = this.selections.findIndex((select: any) =>
      Object.is(select.srfkey, row.srfkey)
    );
  ...
  public setColState(): void {
    ...
    if (_data) {
      ...
      columns.forEach((col: any) => {
        let column = this.allColumns.find(item =>
          Object.is(col.name, item.name)
        );
        if (column) {
  ...
  public onGridItemValueChange(
    ...
    if (
      !$event.name ||
      Object.is($event.name, "") ||
      !row.hasOwnProperty($event.name)
    ) {
      return;
    }
  ...
  public getColumnDisabled(data: any, name: string) {
    if (this.allColumns || Array.isArray(this.allColumns)) {
      ...
      if (curColumn.hasOwnProperty("enableCond")) {
        return data.srfuf == 1
          ? (curColumn.enableCond & 2) !== 2
          : (curColumn.enableCond & 1) !== 1;
      } else {
        return false;
      }
    }
  ...
  public verifyDeRules(
    ...
    for (let i = 0; i < rule[name].length; i++) {
      ...
      if (item.type == "REGEX") {
        startOp(
          !this.$verify.checkFieldRegExRule(
            value,
            item.regExCode,
            item.ruleInfo,
            item.isKeyCond
      ...
      // 分组
      if (item.type == "GROUP") {
        falg = this.verifyDeRules(
          "group",
          item,
          item.condOP ? item.condOP : "AND",
          value
        );
        if (item.isNotMode) {
