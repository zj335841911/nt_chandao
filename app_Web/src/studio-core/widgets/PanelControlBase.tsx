...
export class PanelControlBase extends ControlBase {
  ...
  public async computePanelData() {
    ...
    if (
      ...
      if (codelistArray.length > 0) {
        ...
        this.dataModel.getDataItems().forEach((item: any) => {
          if (item.codelist) {
            panelData[item.prop] = res
              .get(item.codelist.tag)
              .get(this.data[item.name]);
          } else {
   ...
   * @memberof PanelControlBase
   */
  public getAllCodeList(
    codelistArray: Array<any>,
  ...
  public getCodeList(codeListObject: any): Promise<any> {
    return new Promise((resolve: any, reject: any) => {
      if (
        codeListObject.tag &&
        Object.is(codeListObject.codelistType, "STATIC")
      ) {
        ...
        codeListObject.tag &&
        Object.is(codeListObject.codelistType, "DYNAMIC")
      ) {
  ...
  public opendata(
    ...
    if (
      this.parentRef.opendata &&
      this.parentRef.opendata instanceof Function
    ) {
  ...
  public onPanelItemValueChange(
    ...
    if (
      ...
      !data.hasOwnProperty($event.name)
    ) {
      return;
    }
      ...
      if (this.detailsModel && Object.keys(this.detailsModel).length > 0) {
        Object.keys(this.detailsModel).forEach((name: any) => {
          ...
          if (
            ...
            model.isPower = true;
            const tempUIAction: any = JSON.parse(
              JSON.stringify(model.uiaction)
            );
