...
export class TabExpPanelControlBase extends ExpControlBase {
      ...
      this.viewStateEvent = this.viewState.subscribe(
        ({ tag, action, data }) => {
          ...
          if (Object.is(action, "loadmodel")) {
            ...
            this.action = action;
            this.viewState.next({
              tag: this.activatedTabViewPanel,
              action: action,
              data: data
            });
            this.$forceUpdate();
  ...
  public computedAuthPanel(data: any) {
    ...
    if (
      ...
      Object.keys(this.authResourceObject).length > 0
    ) {
      Object.keys(this.authResourceObject).forEach((key: string) => {
        if (
          this.authResourceObject[key] &&
          this.authResourceObject[key]["dataaccaction"]
        ) {
          let tempUIAction: any = this.$util.deepCopy(
            this.authResourceObject[key]
          );
          let result: any[] = this.$viewTool.calcActionItemAuthState(
            data,
            [tempUIAction],
            this.appUIService
          );
          this.authResourceObject[key].visible = this.computedPanelWithResource(
            key,
            tempUIAction.visible
          );
          this.authResourceObject[
            key
          ].disabled = this.computedPanelWithResource(
            key,
            tempUIAction.disabled
          );
        }
      });
