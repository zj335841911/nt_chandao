...
export class EditFormControlBase extends FormControlBase {
      ...
      this.$acc.commandLocal(
        (data: any) => {
          if (
            ...
            (!data.___localUpdateDate ||
              this.data.___localUpdateDate !== data.___localUpdateDate)
          ) {
            const appview = this.$store.getters["viewaction/getAppView"](
              this.viewtag
            );
  ...
  public onFormLoad(data: any = {}, action: string): void {
    ...
    // 更新上下文，当前数据视图数据
    this.$appService.contextStore.setContextData(this.context, this.appDeName, {
      data
    });
  ...
  public autoSave(opt: any = {}): void {
    ...
    const action: any = Object.is(data.srfuf, "1")
      ? this.updateAction
      : this.createAction;
    if (!action) {
      let actionName: any = Object.is(data.srfuf, "1")
        ? "updateAction"
        : "createAction";
      this.$Notice.error({
        ...
        if (response && response.status && response.data) {
          if (response.data.errorKey) {
            if (Object.is(response.data.errorKey, "versionCheck")) {
              ...
              let errorProp: string = response.data.message.match(
                /\[[a-zA-Z]*\]/
              )[0];
              let name: string = this.service.getNameByProp(
              ...
              Object.is(response.data.errorKey, "DuplicateKeyException")
            ) {
              this.$Notice.error({
                title: this.$t("app.commonWords.createFailed") as string,
                desc:
  ...
  public formItemValidate(prop: string, status: boolean, error: string) {
    ...
    if (this.errorMessages && this.errorMessages.length > 0) {
      const index = this.errorMessages.findIndex((errorMessage: any) =>
        Object.is(errorMessage.prop, prop)
      );
      if (index != -1) {
    ...
    return new Promise((resolve: any, reject: any) => {
      ...
      post
        .then((response: any) => {
          ...
          this.$emit("save", data);
          this.$store.dispatch("viewaction/datasaved", {
            viewtag: this.viewtag
          });
          this.sendAccMessage(Object.is(data.srfuf, "1") ? "update" : "create");
          this.$nextTick(() => {
  ...
  public remove(opt: Array<any> = [], showResultInfo?: boolean): Promise<any> {
    return new Promise((resolve: any, reject: any) => {
      if (!this.removeAction) {
        this.$Notice.error({
          title: "错误",
          desc: `${this.name}表单removeAction参数未配置`
        });
        return;
      }
      const arg: any = opt[0];
        ...
        .then((response: any) => {
          ...
          result
            .then((response: any) => {
              ...
              this.$Notice.info({
                title: "",
                desc: this.$t("app.formpage.workflow.startsuccess") as string
              });
              resolve(response);
            })
            .catch((response: any) => {
