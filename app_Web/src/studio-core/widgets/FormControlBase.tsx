...
export class FormControlBase extends MainControlBase {
  ...
  public onFormItemValueChange($event: { name: string; value: any }): void {
    ...
    if (
      ...
      !this.data.hasOwnProperty($event.name)
    ) {
      return;
    }
  ...
  public loadDraft(opt: any = {}): void {
    if (!this.loaddraftAction) {
      this.$Notice.error({
        title: "错误",
        desc: `${this.name}表单loaddraftAction参数未配置`
      });
      return;
    }
    const arg: any = { ...opt };
  ...
  public verifyDeRules(
    ...
    for (let i = 0; i < rule[name].length; i++) {
      ...
      let dataValue = item.deName
        ? this.data[this.service.getItemNameByDeName(item.deName)]
        : "";
      // 提供表单值规则默认提示信息(未配置值规则提示信息时)
      item.ruleInfo = item.ruleInfo
        ? item.ruleInfo
        : this.$t("app.formpage.valuecheckex");
      // 为空值时，属性值规则不做校验
      if (
    ...
    if (this.detailsModel && Object.keys(this.detailsModel).length > 0) {
      Object.keys(this.detailsModel).forEach((name: any) => {
        if (
          ...
          let tempUIAction: any = JSON.parse(
            JSON.stringify(this.detailsModel[name].uiaction)
          );
          let result: any[] = ViewTool.calcActionItemAuthState(
            targetData,
            [tempUIAction],
            this.appUIService
          );
