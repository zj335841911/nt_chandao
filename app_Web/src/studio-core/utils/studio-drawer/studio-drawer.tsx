...
export class StudioDrawer extends Vue {
  ...
  protected created(): void {
    on(document, "keydown", ($event: KeyboardEvent) => {
      if ($event && $event.keyCode === 27 && this.viewList.length > 0) {
        ...
        this.refCloseView(
          this.viewList[this.viewList.length - 1],
  ...
  public closeView(item: any): void {
    if (
      this.closeModalData &&
      this.closeModalData.length > 0 &&
      this.closeModalData[0] !== undefined
    ) {
    ...
    return (
      <div class="studio-drawer-header">
        <div class="studio-drawer-breadcrumb">
          <div
            class="studio-drawer-back"
            on-click={() => this.closeByIndex(this.viewList.length - 1)}
          >
          ...
          {this.showViewList.map((item, i) => {
            ...
            return (
              <span key={i}>
                {i !== 0 ? (
                  <span class="studio-drawer-breadcrumb-item-separator">
                    &gt;
                  </span>
                ) : null}
                <span
                  class={{
                    text: true,
                    active: i === this.showViewList.length - 1
                  }}
                  on-click={() => {
...
export class StudioDrawerController {
  ...
  public openDrawer(
    ...
    this.getVueInstance()
      .$refs.StudioDrawer.openModal({
        ...view,
        viewdata: Util.deepCopy(context),
        viewparams: data
      })
      .then((data: any) => {
        subject.next(data);
