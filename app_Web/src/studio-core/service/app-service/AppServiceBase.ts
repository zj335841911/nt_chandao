  ...
  async doLogin(
    ...
    if (
      ...
      if (Environment.LoginMode.toUpperCase() === "UAA") {
        location.href = `${Environment.LoginUrl}?redirect=${encodeURIComponent(
          redirect
        )}`;
        ...
        if (isLogin) {
          location.href = `${
            ...
            `${location.origin}/cas-login.html?RU=${encodeURIComponent(
              location.href
            )}`
          )}`;
        } else {
          location.href = `${
            Environment.CasUrl
          }/logout?service=${encodeURIComponent(
