...
export class AppNavHistoryBase {
   ...
   * @memberof AppNavHistoryBase
   */
  public readonly navIgnoreParameters: RegExp = new RegExp(
      ...
      addEventListener("hashchange", ({ oldURL, newURL }) => {
        if (this.historyList.length > 0) {
          ...
          if (this.isRouteSame(param, lastHistory.to)) {
            ...
            this.isRouteSame(param2, lastHistory.to) &&
  ...
  public findHistoryByTag(tag: string): any {
    return this.historyList.find(
      item => isExistAndNotEmpty(item.tag) && item.tag === tag
    );
  }
  ...
  public add(to: any): void {
    if (this.findHistoryIndex(to) === -1) {
      if (
        this.uiStateService.layoutState.styleMode === "DEFAULT" &&
        to?.matched?.length === 1
      ) {
        return;
      }
