...
export class AppCommunicationsCenter {
  ...
  constructor(
    params?: {
      psdsconsoleurl: string;
      psdcconsoleid?: string;
      psdevslnsysid?: string;
      psdsconsoleid?: string;
    },
    opt: any = {}
  ) {
    let me: any = this;
  ...
  public sendMessage(message: AppMessage, deName?: string) {
    ...
    if (deName) {
      ...
      (!data.srfdename || Object.is(data.srfdename, "")) &&
      data.srfdeid
    ) {
  ...
  public consoleLocal(observer: (content: any) => void): string {
    const key: string = this.createUUID();
    this.unsubscribeCache.set(key, [
      this.current.console.subscribeLocal(observer)
    ]);
    return key;
  }
