  ...
  public calcSplit(): void {
    ...
    if (this.ctrlWidth > 0 && this.mode === "horizontal") {
      this.defaultSplit =
        1 -
        parseFloat(
          ((el.offsetWidth - this.ctrlWidth) / el.offsetWidth).toFixed(2)
        );
    } else if (this.ctrlHeight > 0 && this.mode === "vertical") {
      this.defaultSplit =
        1 -
        parseFloat(
          ((el.offsetHeight - this.ctrlHeight) / el.offsetHeight).toFixed(2)
        );
    }
  }
  ...
  protected renderSplit(): any {
    ...
    return (
      <split
        ...
        mode={this.mode}
        min={
          this.ctrlHeight > 0 || this.ctrlWidth > 0 ? this.defaultSplit : null
        }
      >
          ...
          <div class="exp-control-layout-exp">
            {this.$slots.toolbar ? (
              <div key="exp-top" class="exp-top">
                {this.$slots.toolbar ? (
                  <div class="exp-toolbar">{this.$slots.toolbar}</div>
                ) : null}
            ...
            {showHeader ? (
              <div key="exp-header" class="exp-header">
                {this.$slots.title ? (
                  <div class="exp-header-title">{this.$slots.title}</div>
                ) : null}
                {this.$slots.quickGroupSearch ? (
                  <div
                    key="exp-quick-group-search"
                    class="exp-quick-group-search"
                  >
                    {this.$slots.quickGroupSearch}
                  </div>
                ) : null}
