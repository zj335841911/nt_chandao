...
export class AppContentLeftNavMenu extends Vue {
  ...
  public created(): void {
    ...
    this.footerRenderItem = this.$footerRenderService.registerLeftItem(
      (h: any) => {
        return (
          <icon
            title={
              this.uiState.layoutState.leftNavMenuCollapse
                ? "展开菜单"
                : "收起菜单"
            }
            type="md-menu"
  ...
  protected open(name: string): void {
    const i: number = this.uiState.layoutState.leftNavOpenedMenus.findIndex(
      (str: any) => Object.is(str, name)
    );
    if (i === -1) {
