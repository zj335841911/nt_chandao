...
export class AppBreadcrumb extends Vue {
  ...
  protected getItems(context: any, tag?: string): any[] | null {
    if (tag) {
      ...
      if (view) {
        const data = this.appService.contextStore.getContextData(
          view.context,
          context.srfappdename
        );
        if (data && data.items) {
          return data.items;
        }
  ...
  public render(): any {
    ...
    arr.forEach((item, i) => {
      ...
      items.push(
        <span
          key={item.tag}
          class={{ "app-breadcrumb-item": true, last: i === arr.length - 1 }}
        >
          {!indexMeta && i === 0 ? null : (
            <span class="separator">{this.NavBarDelimiter}</span>
          )}
