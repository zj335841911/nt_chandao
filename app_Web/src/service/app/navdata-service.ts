...
export default class NavDataService {
  ...
  public addNavData(curNavData: NavDataElement) {
    if (this.navDataStack.length > 0) {
      let tempIndex: number = this.navDataStack.findIndex(
        (element: NavDataElement) => {
      ...
      if (tempIndex === -1) {
        ...
        this.sessionStore.setItem(
          "srfnavdata",
          JSON.stringify(this.navDataStack)
        );
        return curNavData;
      } else {
    ...
    if (this.navDataStack && this.navDataStack.length > 0) {
      this.navDataStack.forEach((item: any, index) => {
        if (item && item.data && this.navDataStack[index + 1]) {
          this.navDataStack[index + 1] = Object.assign(
            this.navDataStack[index + 1],
            { isselected: true }
          );
        }
      });
