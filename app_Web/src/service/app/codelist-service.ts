  ...
  public getPredefinedItems(
    ...
    return new Promise((resolve: any, reject: any) => {
        ...
        promise
          .then((res: any) => {
            ...
            if (result.items && result.items.length > 0) {
              CodeListService.codelistCached.set(`${tag}`, {
                items: result.items
              });
      ...
      if (CodeListService.codelistCache.get(`${tag}`)) {
        ...
        let result: Promise<any> = this.entityService.getPredefinedCodelist(
          tag
        );
          ...
          if (isEnableCache) {
            const callback: Function = (
              ...
              promise
                .then((result: any) => {
                  if (result.length > 0) {
                    CodeListService.codelistCached.set(callbackKey, {
                      items: result
                    });
            ...
            if (codelist) {
              ...
              if (CodeListService.codelistCache.get(key)) {
                callback(
                  ...
                  data,
                  tag,
                  CodeListService.codelistCache.get(key)
                );
              } else {
                let result: Promise<any> = codelist.getItems(
                  context,
                  data,
                  isloading
                );
