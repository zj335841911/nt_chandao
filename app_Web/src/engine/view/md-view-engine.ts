...
export default class MDViewEngine extends ViewEngine {
  ...
  public load(opts: any = {}, isnotify: boolean = false): void {
    ...
    if (this.getSearchForm() && (this.isLoadDefault || isnotify)) {
      ...
      this.setViewState2({
        tag: tag,
        action: "load",
        viewdata: Object.assign(this.view.viewparams, opts)
      });
    } else {
  ...
  public onEditData(arg: any): void {
    ...
    if (
      this.keyPSDEField &&
      data[this.keyPSDEField] &&
      !Object.is(data[this.keyPSDEField], "")
    ) {
      Object.assign(loadParam, {
        [this.keyPSDEField]: data[this.keyPSDEField]
      });
    }
  ...
  public selectionChange(args: any[]): void {
    ...
    const state =
      args.length > 0 && !Object.is(args[0].srfkey, "") ? false : true;
  ...
  public MDCtrlBeforeLoad(arg: any = {}): void {
    ...
    if (
      this.view &&
      this.view.$refs.searchbar &&
      this.view.isExpandSearchForm
    ) {
  ...
  public transformData(arg: any) {
    if (
      !this.getMDCtrl() ||
      !(this.getMDCtrl().transformData instanceof Function)
    ) {
      return null;
    }
