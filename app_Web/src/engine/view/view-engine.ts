...
export default class ViewEngine {
  ...
  public calcToolbarItemState(state: boolean, dataNum?: number) {
    ...
    if (
      !_this.view.toolBarModels ||
      Object.keys(_this.view.toolBarModels).length === 0
    ) {
      return;
    }
    ...
    for (const key in _this.view.toolBarModels) {
      ...
      if (
        _item.uiaction &&
        (Object.is(_item.uiaction.target, "SINGLEKEY") ||
          Object.is(_item.uiaction.target, "MULTIKEY"))
      ) {
        if (
          dataNum &&
          dataNum > 1 &&
  ...
  public calcToolbarItemAuthState(data: any, dataNum?: number) {
    ...
    for (const key in _this.view.toolBarModels) {
      ...
      if (_item && _item["dataaccaction"] && _this.view.appUIService) {
        ...
        if (
          _item.uiaction &&
          (Object.is(_item.uiaction.target, "NONE") ||
            Object.is(_item.uiaction.target, ""))
        ) {
          dataActionResult = _this.view.appUIService.getResourceOPPrivs(
            _item["dataaccaction"]
          );
        } else {
          if (
            ...
            dataActionResult = _this.view.appUIService.getAllOPPrivs(data)[
              _item["dataaccaction"]
            ];
          }
        }
