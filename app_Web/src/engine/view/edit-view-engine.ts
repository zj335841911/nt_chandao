...
export default class EditViewEngine extends ViewEngine {
  ...
  public onFormSave(arg: any): void {
    ...
    this.view.$emit(
      "viewdataschange",
      JSON.stringify({ action: "save", status: "success", data: arg })
    );
  }
  ...
  public onFormRemove(arg: any): void {
    ...
    this.view.$emit(
      "viewdataschange",
      JSON.stringify({ action: "remove", status: "success", data: arg })
    );
  }
  ...
  public setTabCaption(info: string, isNew: boolean): void {
    ...
    if (viewdata && info && !Object.is(info, "")) {
      ...
      if (this.view.viewUsage === 1 || this.view.viewUsage === 7) {
        this.view.$appService.navHistory.setCaption({
          tag: this.view.viewtag,
          info: viewdata.dataInfo
        });
      }
    }
  }
  ...
  public transformData(arg: any) {
    if (
      !this.getForm() ||
      !(this.getForm().transformData instanceof Function)
    ) {
      return null;
    }
