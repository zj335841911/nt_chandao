...
export default class ClosePauseCancelFormBase extends Vue implements ControlInterface {
     ...
     * @memberof ClosePauseCancelForm
     */
    protected service: ClosePauseCancelFormService = new ClosePauseCancelFormService({ $store: this.$store });
...
, 
    ...
    protected print(){
        let _this:any = this;
        _this.$print({id:'form',popTitle:'关闭、暂停、取消表单'});
    }
    ...
    */
    ...
    protected async wfsubmit(data: any): Promise<any> {
        ...
        if(!this.WFSubmitAction){
            this.$Notice.error({ title: '错误', desc: 'TaskPauseTaskView视图表单WFSubmitAction参数未配置' });
            return;
        }
        const _this: any = this;
        const arg: any = data[0];
