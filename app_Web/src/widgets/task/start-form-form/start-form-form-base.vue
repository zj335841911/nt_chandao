...
<i-col v-show="detailsModel.comment.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='comment' :itemRules="this.rules.comment" class='' :caption="$t('entities.task.startform_form.details.comment')" uiStyle="DEFAULT" :labelWidth="100" :isShowCaption="true" :error="detailsModel.comment.error" :isEmptyCaption="false" labelPos="LEFT">
...
export default class StartFormBase extends Vue implements ControlInterface {
     ...
     * @memberof StartForm
     */
    protected service: StartFormService = new StartFormService({ $store: this.$store });
...
, 
    ...
    protected print(){
        let _this:any = this;
        _this.$print({id:'form',popTitle:'开始表单'});
    }
    ...
    */
    ...
    protected async wfsubmit(data: any): Promise<any> {
        ...
        if(!this.WFSubmitAction){
            this.$Notice.error({ title: '错误', desc: 'TaskOpenTaskView视图表单WFSubmitAction参数未配置' });
            return;
        }
        const _this: any = this;
        const arg: any = data[0];
