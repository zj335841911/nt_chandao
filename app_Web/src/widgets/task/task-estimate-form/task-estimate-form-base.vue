...
<i-col v-show="detailsModel.id.visible" :style="{'height': '32px !important','width':'40px !important'}"  :lg="{ span: 24, offset: 0 }" :xl="{ span: 1, offset: 0 }">
    <app-form-item name='id' :itemRules="this.rules.id" class='panelitem-borad' :caption="$t('entities.task.taskestimate_form.details.id')" uiStyle="DEFAULT" :labelWidth="1" :isShowCaption="true" :error="detailsModel.id.error" :isEmptyCaption="true" labelPos="LEFT">
    <app-span   name='id'
...
<i-col v-show="detailsModel.name.visible" :style="{}"  :lg="{ span: 24, offset: 0 }" :xl="{ span: 21, offset: 0 }">
    <app-form-item name='name' :itemRules="this.rules.name" class='MainInfo' :caption="$t('entities.task.taskestimate_form.details.name')" uiStyle="DEFAULT" :labelWidth="2" :isShowCaption="true" :error="detailsModel.name.error" :isEmptyCaption="true" labelPos="LEFT">
    <app-span   name='name'
...
<i-col v-show="detailsModel.druipart2.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    ...
    viewname='task-estimate-edit-grid-view9' 
...
<i-col v-show="detailsModel.druipart1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    ...
    refreshitems='' 
    :ignorefieldvaluechange="ignorefieldvaluechange"
    viewname='task-estimate-grid-view9' 
    :data="JSON.stringify(this.data)" 
    @drdatasaved="drdatasaved($event)"
    style=";overflow: auto;">
</app-form-druipart>
...
export default class TaskEstimateBase extends Vue implements ControlInterface {
     ...
     * @memberof TaskEstimate
     */
    protected service: TaskEstimateService = new TaskEstimateService({ $store: this.$store });
...
, 
     ...
     *
     * @param {*} [arg={}]
     * @memberof @memberof TaskEstimate
     */
    ...
    protected print(){
        let _this:any = this;
        _this.$print({id:'form',popTitle:'工时'});
    }
    ...
    private load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'TaskWorkHoursView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    protected loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TaskWorkHoursView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    protected autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'TaskWorkHoursView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    private remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'TaskWorkHoursView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    protected async wfsubmit(data: any): Promise<any> {
        ...
        if(!this.WFSubmitAction){
            this.$Notice.error({ title: '错误', desc: 'TaskWorkHoursView视图表单WFSubmitAction参数未配置' });
            return;
        }
        const _this: any = this;
        const arg: any = data[0];
