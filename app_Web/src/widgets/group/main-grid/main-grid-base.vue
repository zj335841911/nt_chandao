    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('id')">
                <el-table-column show-overflow-tooltip :prop="'id'" :label="$t('entities.group.main_grid.columns.id')" :width="70"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.group.main_grid.columns.id')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('name')">
                <el-table-column show-overflow-tooltip :prop="'name'" :label="$t('entities.group.main_grid.columns.name')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.group.main_grid.columns.name')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.name}}</span>
            ...
            <template v-if="getColumnState('desc')">
                <el-table-column show-overflow-tooltip :prop="'desc'" :label="$t('entities.group.main_grid.columns.desc')" :min-width="1"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.group.main_grid.columns.desc')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.desc}}</span>
            ...
            <template v-if="getColumnState('uagridcolumn1')">
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.group.main_grid.columns.uagridcolumn1')" :width="300"  :align="'center'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.group.main_grid.columns.uagridcolumn1')}}
                      </span>
                    </template>
                    <template slot-scope="scope">
                        <span>
                            ...
                            <a @click="uiAction(scope.row, 'Edit', $event)">
                              ...
                              {{$t('entities.group.main_grid.uiactions.edit')}}
                            </a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'Remove', $event)">
                              <i class='fa fa-remove'></i>
                              {{$t('entities.group.main_grid.uiactions.remove')}}
                            </a>
                        </span>
                    </template>
                </el-table-column>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof Main
     */
    public appEntityService: GroupService = new GroupService({ $store: this.$store });
     ...
     * @memberof 
     */
    public grid_uagridcolumn1_u6da9e14_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
        ...
        // 界面行为
        this.Edit(datas, contextJO,paramJO,  $event, xData,this,"Group");
    }
     ...
     * @memberof 
     */
    public grid_uagridcolumn1_u1107955_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
        ...
        // 界面行为
        this.Remove(datas, contextJO,paramJO,  $event, xData,this,"Group");
    }
    ...
    public Edit(args: any[],contextJO?:any, params?: any, $event?: any, xData?: any,actionContext?:any,srfParentDeName?:string) {
        ...
        if (_this.opendata && _this.opendata instanceof Function) {
            const data: any = { };
            if (args.length > 0) {
                Object.assign(data, { group: args[0].group })
            }
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'id',
            label: 'ID',
            langtag: 'entities.group.main_grid.columns.id',
            show: true,
            util: 'PX'
        },
        {
            name: 'name',
            label: '分组名称',
            langtag: 'entities.group.main_grid.columns.name',
            show: true,
            util: 'PX'
        },
        {
            name: 'desc',
            label: '分组描述',
            langtag: 'entities.group.main_grid.columns.desc',
            show: true,
            util: 'STAR'
        },
        {
            name: 'uagridcolumn1',
            label: '操作',
            langtag: 'entities.group.main_grid.columns.uagridcolumn1',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'GroupMainGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'GroupMainGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ group: _keys }),Object.assign({ group: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.zt_group,$event.zt_group);
        });
	...
	public uiAction(row: any, tag: any, $event: any) {
        ...
        if(Object.is('Edit', tag)) {
            this.grid_uagridcolumn1_u6da9e14_click(row, tag, $event);
        }
        if(Object.is('Remove', tag)) {
            this.grid_uagridcolumn1_u1107955_click(row, tag, $event);
        }
    }
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('zt_group_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('zt_group_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'GroupMainGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'GroupMainGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.group){
                            Object.assign(this.context,{group:item.group});
                        }
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.group,args.row.group);
        });
