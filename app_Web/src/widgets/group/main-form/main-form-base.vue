...
export default class MainBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        srfmajortext: [
            { type: 'string', message: '分组名称 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '分组名称 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '分组名称 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '分组名称 值不能为空', trigger: 'blur' },
        ],
        srftempmode: [
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'blur' },
        ...
        name: [
            { type: 'string', message: '分组名称 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '分组名称 值必须为字符串类型', trigger: 'blur' },
            { required: true, type: 'string', message: '分组名称 值不能为空', trigger: 'change' },
            { required: true, type: 'string', message: '分组名称 值不能为空', trigger: 'blur' },
        ],
        desc: [
            { type: 'string', message: '分组描述 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '分组描述 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '分组描述 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '分组描述 值不能为空', trigger: 'blur' },
        ],
        id: [
            { type: 'number', message: 'ID 值必须为数值类型', trigger: 'change' },
            { type: 'number', message: 'ID 值必须为数值类型', trigger: 'blur' },
    ...
    public detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: '基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'entities.group.main_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
        srfmajortext: new FormItemModel({ caption: '分组名称', detailType: 'FORMITEM', name: 'srfmajortext', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        name: new FormItemModel({ caption: '分组名称', detailType: 'FORMITEM', name: 'name', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        desc: new FormItemModel({ caption: '分组描述', detailType: 'FORMITEM', name: 'desc', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    public onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.group){
            Object.assign(this.context,{group:data.group})
        }
    ...
    public copy(srfkey: string): void {
        ...
        copyData.group = copyData.srfkey;
        copyData.id = copyData.srfkey;
        Object.assign(this.context,{group:copyData.group})
        this.data = copyData;
        this.$nextTick(() => {
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'group_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'GroupEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'GroupEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'GroupEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'GroupEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.group || Object.is(arg.group, '')) {
            return;
        }
