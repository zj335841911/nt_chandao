<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='storyspec_main' style="">
    <input style="display:none;" />
    <row >
            
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' :uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.storyspec.main_form.details.group1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false" >    
    <row>
        <i-col v-show="detailsModel.verify.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='verify' :itemRules="this.rules.verify" class='' :caption="$t('entities.storyspec.main_form.details.verify')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.verify.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.rawitem1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <div class="" style="">
    <div style="border-bottom: 1p solid var(--view-dividing-line-color-bright)"></div>
</div>

</i-col>
<i-col v-show="detailsModel.spec.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='spec' :itemRules="this.rules.spec" class='' :caption="$t('entities.storyspec.main_form.details.spec')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.spec.error" :isEmptyCaption="false" labelPos="LEFT">
    <app-span   name='spec'
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof Main
     */
    public appEntityService: StorySpecService = new StorySpecService({ $store: this.$store });
    ...
    public rules: any = {
        ...
        srfkey: [
            { type: 'string', message: '虚拟主键 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '虚拟主键 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '虚拟主键 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '虚拟主键 值不能为空', trigger: 'blur' },
        ],
        srfmajortext: [
...
, 
        group1: new FormGroupPanelModel({ caption: 'storyspec基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'entities.storyspec.main_form', extractMode: 'ITEM', details: [] } })
, 
        formpage1: new FormPageModel({ caption: '基本信息', detailType: 'FORMPAGE', name: 'formpage1', visible: true, isShowCaption: true, form: this })
, 
        srforikey: new FormItemModel({ caption: '', detailType: 'FORMITEM', name: 'srforikey', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        srfkey: new FormItemModel({ caption: '虚拟主键', detailType: 'FORMITEM', name: 'srfkey', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        id: new FormItemModel({ caption: '虚拟主键', detailType: 'FORMITEM', name: 'id', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };
    ...
    public onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.storyspec){
            Object.assign(this.context,{storyspec:data.storyspec})
        }
    ...
    public copy(srfkey: string): void {
        ...
        copyData.storyspec = copyData.srfkey;
        copyData.id = copyData.srfkey;
        Object.assign(this.context,{storyspec:copyData.storyspec})
        this.data = copyData;
        this.$nextTick(() => {
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'storyspec_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'StorySpecMainView9视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'StorySpecMainView9视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'StorySpecMainView9视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'StorySpecMainView9视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.storyspec || Object.is(arg.storyspec, '')) {
            return;
        }
