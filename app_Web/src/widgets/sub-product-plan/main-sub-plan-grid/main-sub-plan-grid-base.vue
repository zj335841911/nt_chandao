...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('id')">
                <el-table-column show-overflow-tooltip :prop="'id'" :label="$t('entities.subproductplan.mainsubplan_grid.columns.id')" :width="80" :align="'left'" :sortable="'custom'">
            ...
            <template v-if="getColumnState('title')">
                <el-table-column show-overflow-tooltip :prop="'title'" :label="$t('entities.subproductplan.mainsubplan_grid.columns.title')" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <app-column-link deKeyField='productplan' :context="JSON.parse(JSON.stringify(context))" :viewparams="JSON.parse(JSON.stringify(viewparams))" :data="row" :linkview="{viewname: 'product-plan-main-tab-exp', height: 0,width: 0,title: $t('entities.productplan.views.maintabexp.title'),placement: 'DRAWER_TOP', isRedirectView: false,deResParameters: [
            ...
            <template v-if="getColumnState('begin')">
                <el-table-column show-overflow-tooltip :prop="'begin'" :label="$t('entities.subproductplan.mainsubplan_grid.columns.begin')" :width="120" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('end')">
                <el-table-column show-overflow-tooltip :prop="'end'" :label="$t('entities.subproductplan.mainsubplan_grid.columns.end')" :width="120" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
...
export default class MainSubPlanBase extends Vue implements ControlInterface {
     ...
     * @memberof MainSubPlan
     */
    protected service: MainSubPlanService = new MainSubPlanService({ $store: this.$store });
     ...
     * @memberof MainSubPlan
     */
    protected appEntityService: SubProductPlanService = new SubProductPlanService({ $store: this.$store });
    ...
    */
    ...
    protected async remove(datas: any[]): Promise<any> {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(this.removeAction,Object.assign(context,{ subproductplan: keys.join(';') }),Object.assign({ id: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    protected save(args: any[], params?: any, $event?: any, xData?: any): void {
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                Object.assign(item,{viewparams:this.viewparams});
                if(item.subproductplan){
                    Object.assign(this.context,{subproductplan:item.subproductplan})
                }
    ...
    protected getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.subproductplan,args.row.subproductplan);
        });
