...
export default class TestExpBase extends Vue implements ControlInterface {
     ...
     * @memberof TestExp
     */
    protected service: TestExpService = new TestExpService({ $store: this.$store });
     ...
     * @memberof TestExp
     */
    protected listexpbar_list_quicktoolbar_click($event: any, $event2?: any) {
        if (Object.is($event.tag, 'deuiaction1')) {
            this.listexpbar_list_quicktoolbar_deuiaction1_click($event, 'listexpbar_list_quicktoolbar', $event2);
        }
    }
     ...
     * @memberof 
     */
    protected listexpbar_list_quicktoolbar_deuiaction1_click(params: any = {}, tag?: any, $event?: any) {
        // 参数
        // 取数
        let datas: any[] = [];
        let xData: any = null;
    ...
    */
    ...
    private load(opt: any = {}): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ProductTestListExpView视图列表fetchAction参数未配置' });
            return;
        }        
        const arg: any = {...opt};
        const page: any = {};
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ProductTestListExpView视图表格removeAction参数未配置' });
            return;
        }
        if (datas.length === 0) {
            return;
