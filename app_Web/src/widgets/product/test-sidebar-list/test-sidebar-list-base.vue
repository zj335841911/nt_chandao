<template>  
    <div :class="['app-list', this.items.length > 0 ? '' : 'app-list-empty' ]">
            <div v-if="items.length > 0">
                                <test-list :items="items" @item-click="handleDblClick.apply(_self, arguments)"/>
            </div>
            <div v-else>
                暂无数据
            </div>
...
export default class TestSidebarBase extends Vue implements ControlInterface {
     ...
     * @memberof TestSidebar
     */
    protected service: TestSidebarService = new TestSidebarService({ $store: this.$store });
