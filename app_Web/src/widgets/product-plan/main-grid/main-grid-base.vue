...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('id')">
                <el-table-column show-overflow-tooltip :prop="'id'" :label="$t('entities.productplan.main_grid.columns.id')" :width="80" :align="'left'" :sortable="'custom'">
            ...
            <template v-if="getColumnState('title')">
                <el-table-column show-overflow-tooltip :prop="'title'" :label="$t('entities.productplan.main_grid.columns.title')" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('begin')">
                <el-table-column show-overflow-tooltip :prop="'begin'" :label="$t('entities.productplan.main_grid.columns.begin')" :width="120" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('end')">
                <el-table-column show-overflow-tooltip :prop="'end'" :label="$t('entities.productplan.main_grid.columns.end')" :width="120" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                ...
                <el-table-column :column-key="'actions'" :label="$t('entities.productplan.main_grid.columns.actions')" :width="160" :align="'center'">
                    <template slot-scope="scope">
                        <span>
                            ...
                            <a @click="uiAction(scope.row, 'RelationStory', $event)">{{$t('entities.productplan.main_grid.uiactions.relationstory')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'RelationBug', $event)">{{$t('entities.productplan.main_grid.uiactions.relationbug')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'MainEdit', $event)">{{$t('entities.productplan.main_grid.uiactions.mainedit')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'NewSubPlan', $event)">{{$t('entities.productplan.main_grid.uiactions.newsubplan')}}</a>
                        </span>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof 
     */
    protected grid_actions_uaa449c7_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_actions_uae31417_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_actions_u7a43501_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_actions_u663d352_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
    ...
    */
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ProductPlanGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ProductPlanGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(this.removeAction,Object.assign(context,{ productplan: keys.join(';') }),Object.assign({ id: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'ProductPlanGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'ProductPlanGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.productplan){
