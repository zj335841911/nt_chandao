...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('title')">
                <el-table-column show-overflow-tooltip :prop="'title'" :label="$t('entities.productplan.mainproject_grid.columns.title')" :align="'left'" :sortable="'custom'">
...
export default class MainProjectBase extends Vue implements ControlInterface {

    /**
     * 名称
     *
     * @type {string}
     * @memberof MainProject
     */
     ...
     * @memberof MainProject
     */
    protected service: MainProjectService = new MainProjectService({ $store: this.$store });
    ...
    */
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ProductPlanProjectGridView9视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ProductPlanProjectGridView9视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
    ...
    protected setColState() {
		const _data: any = localStorage.getItem('zt_productplan_mainproject_grid');
		if (_data) {
    ...
    protected onColChange() {
        localStorage.setItem('zt_productplan_mainproject_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'ProductPlanProjectGridView9视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'ProductPlanProjectGridView9视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.productplan){
