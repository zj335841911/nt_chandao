...
export default class SubTaskNewService extends ControlService {
     ...
     */
    public appEntityService: SubTaskService = new SubTaskService({ $store: this.getStore() });
        ...
        return new Promise((resolve: any, reject: any) => {
            ...
            result.then((response) => {
                ...
                if(response.data){
                    Object.assign(response.data,{srfuf:'0'});
                    //仿真主键数据
                    response.data.subtask = Util.createUUID();
                }
    ...
    public mergeDefaults(response:any = {}){ 
        if(response.data){                    
            Object.assign(response.data,{'isleaf':'1'});
