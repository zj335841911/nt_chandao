...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('name')">
                <el-table-column show-overflow-tooltip :prop="'name'" :label="$t('entities.testtask.main_grid.columns.name')" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <app-column-link deKeyField='testtask' :context="JSON.parse(JSON.stringify(context))" :viewparams="JSON.parse(JSON.stringify(viewparams))" :data="row" :linkview="{viewname: 'test-task-main-tab-exp-view', height: 0,width: 0,title: $t('entities.testtask.views.maintabexpview.title'),placement: 'DRAWER_TOP', isRedirectView: false,deResParameters: [
            ...
            <template v-if="getColumnState('build')">
                <el-table-column show-overflow-tooltip :prop="'build'" :label="$t('entities.testtask.main_grid.columns.build')" :width="150" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <span>{{row.build}}</span>
                    </template>
                </el-table-column>
            ...
            <template v-if="getColumnState('owner')">
                <el-table-column show-overflow-tooltip :prop="'owner'" :label="$t('entities.testtask.main_grid.columns.owner')" :width="100" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            <codelist :value="row.owner" tag='UserRealName' codelistType='DYNAMIC' renderMode="STR" valueSeparator="," textSeparator="," ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            ...
            <template v-if="getColumnState('uagridcolumn1')">
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.testtask.main_grid.columns.uagridcolumn1')" :width="150" :align="'center'">
                    <template slot-scope="scope">
                        <span>
                            
                            <a @click="uiAction(scope.row, 'LinkCase', $event)">{{$t('entities.testtask.main_grid.uiactions.linkcase')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'OpenInfoView', $event)">{{$t('entities.testtask.main_grid.uiactions.openinfoview')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'EditEdit', $event)">{{$t('entities.testtask.main_grid.uiactions.editedit')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'Remove', $event)">{{$t('entities.testtask.main_grid.uiactions.remove')}}</a>
                        </span>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_u6b02f33_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_u9008f43_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_editedit_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_u4d10156_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
    ...
    */
    ...
    protected async remove(datas: any[]): Promise<any> {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(this.removeAction,Object.assign(context,{ testtask: keys.join(';') }),Object.assign({ id: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
