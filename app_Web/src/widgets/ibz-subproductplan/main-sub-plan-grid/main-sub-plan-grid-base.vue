    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('id')">
                <el-table-column show-overflow-tooltip :prop="'id'" :label="$t('entities.ibz_subproductplan.mainsubplan_grid.columns.id')" :width="80"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ibz_subproductplan.mainsubplan_grid.columns.id')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('title')">
                <el-table-column show-overflow-tooltip :prop="'title'" :label="$t('entities.ibz_subproductplan.mainsubplan_grid.columns.title')" :min-width="1"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ibz_subproductplan.mainsubplan_grid.columns.title')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.title}}</span>
            ...
            <template v-if="getColumnState('begin')">
                <el-table-column show-overflow-tooltip :prop="'begin'" :label="$t('entities.ibz_subproductplan.mainsubplan_grid.columns.begin')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ibz_subproductplan.mainsubplan_grid.columns.begin')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('end')">
                <el-table-column show-overflow-tooltip :prop="'end'" :label="$t('entities.ibz_subproductplan.mainsubplan_grid.columns.end')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ibz_subproductplan.mainsubplan_grid.columns.end')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
export default class MainSubPlanBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'id',
            label: '编号',
            langtag: 'entities.ibz_subproductplan.mainsubplan_grid.columns.id',
            show: true,
            util: 'PX'
        },
        {
            name: 'title',
            label: '名称',
            langtag: 'entities.ibz_subproductplan.mainsubplan_grid.columns.title',
            show: true,
            util: 'STAR'
        },
        {
            name: 'begin',
            label: '开始日期',
            langtag: 'entities.ibz_subproductplan.mainsubplan_grid.columns.begin',
            show: true,
            util: 'PX'
        },
        {
            name: 'end',
            label: '结束日期',
            langtag: 'entities.ibz_subproductplan.mainsubplan_grid.columns.end',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ProductPlanSubPlanGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ProductPlanSubPlanGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ ibz_subproductplan: _keys }),Object.assign({ ibz_subproductplan: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'ProductPlanSubPlanGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'ProductPlanSubPlanGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.ibz_subproductplan){
                            Object.assign(this.context,{ibz_subproductplan:item.ibz_subproductplan});
                        }
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.ibz_subproductplan,args.row.ibz_subproductplan);
        });
...
<style lang='less'>
@import './main-sub-plan-grid.less';
</style>