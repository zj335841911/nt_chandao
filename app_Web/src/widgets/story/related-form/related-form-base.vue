        ...
        @on-click="detailsModel.form.clickPage($event)">
            <tab-pane v-show="detailsModel.formpage1.visible" name='formpage1' :index="0" tab='form' class=''  
                :label="(h) =>{
                    ...
                    $t('entities.story.related_form.details.formpage1')
                    ])
                }">
                    
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.story.related_form.details.group1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="true">
    ...
    :formState="formState"
    paramItem='story' 
    :parentdata='{"srfparentdefname":"STORY","srfparentdename":"ZT_STORY","SRFPARENTTYPE":"DER1N","srfparentmode":"DER1N_ZT_TASK_ZT_STORY_STORY","SRFDER1NID":"DER1N_ZT_TASK_ZT_STORY_STORY"}'
    :parameters="[
    ...
    viewname='task-grid-view9-story-related' 
    ...
    style=";overflow: auto;">
            ...
            <tab-pane v-show="detailsModel.formpage2.visible" name='formpage2' :index="1" tab='form' class=''  
                :label="(h) =>{
                    ...
                    $t('entities.story.related_form.details.formpage2')
                    ])
                }">
...
<i-col v-show="detailsModel.druipart4.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    ...
    refreshitems='' 
    :ignorefieldvaluechange="ignorefieldvaluechange"
    viewname='story-grid-view9-related' 
    :data="JSON.stringify(this.data)" 
    @drdatasaved="drdatasaved($event)"
    style=";overflow: auto;">
</app-form-druipart>
...
<i-col v-show="detailsModel.druipart5.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    ...
    refreshitems='' 
    :ignorefieldvaluechange="ignorefieldvaluechange"
    viewname='story-grid-view9-child' 
    :data="JSON.stringify(this.data)" 
    @drdatasaved="drdatasaved($event)"
    style=";overflow: auto;">
</app-form-druipart>
...
<i-col v-show="detailsModel.grouppanel1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' uiActionGroup="detailsModel.grouppanel1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.story.related_form.details.grouppanel1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="true">
    ...
    refreshitems='' 
    :ignorefieldvaluechange="ignorefieldvaluechange"
    viewname='bug-grid-view9-story-related' 
    :data="JSON.stringify(this.data)" 
    @drdatasaved="drdatasaved($event)"
    style=";overflow: auto;">
</app-form-druipart>
...
<i-col v-show="detailsModel.grouppanel2.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' uiActionGroup="detailsModel.grouppanel2.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.story.related_form.details.grouppanel2')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="true">
...
export default class RelatedBase extends Vue implements ControlInterface {
     ...
     * @memberof Related
     */
    protected service: RelatedService = new RelatedService({ $store: this.$store });
    ...
    protected rules: any = {
        ...
        childstories: [
            { type: 'string', message: '细分需求 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '细分需求 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '细分需求 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '细分需求 值不能为空', trigger: 'blur' },
        ],
        id: [
            { type: 'number', message: '编号 值必须为数值类型', trigger: 'change' },
            { type: 'number', message: '编号 值必须为数值类型', trigger: 'blur' },
...
, 
        group1: new FormGroupPanelModel({ caption: '项目任务', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'entities.story.related_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
        grouppanel2: new FormGroupPanelModel({ caption: '相关用例', detailType: 'GROUPPANEL', name: 'grouppanel2', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'entities.story.related_form', extractMode: 'ITEM', details: [] } })
, 
        formpage2: new FormPageModel({ caption: '相关信息', detailType: 'FORMPAGE', name: 'formpage2', visible: true, isShowCaption: true, form: this })
, 
...
, 
        childstories: new FormItemModel({ caption: '细分需求', detailType: 'FORMITEM', name: 'childstories', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    /**
     * 监控表单属性 childstories 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof Related
     */
    @Watch('data.childstories')
    onChildstoriesChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'childstories', newVal: newVal, oldVal: oldVal });
    }
    ...
    protected print(){
        let _this:any = this;
        _this.$print({id:'form',popTitle:'关联信息表单'});
    }
    ...
    private load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'StoryMainView9_Related视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    protected loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'StoryMainView9_Related视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    protected autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'StoryMainView9_Related视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    private remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'StoryMainView9_Related视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    protected async wfsubmit(data: any): Promise<any> {
        ...
        if(!this.WFSubmitAction){
            this.$Notice.error({ title: '错误', desc: 'StoryMainView9_Related视图表单WFSubmitAction参数未配置' });
            return;
        }
        const _this: any = this;
        const arg: any = data[0];
