...
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.story.projectplan_form.details.group1')" :isShowCaption="true" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false">
    
    <row>
        <i-col v-show="detailsModel.plan.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='plan' :itemRules="this.rules.plan" class='' :caption="$t('entities.story.projectplan_form.details.plan')" uiStyle="DEFAULT" :labelWidth="1" :isShowCaption="true" :error="detailsModel.plan.error" :isEmptyCaption="true" labelPos="LEFT">
     <dropdown-list v-model="data.plan" :data="data" :itemParam="{}" :disabled="detailsModel.plan.disabled"  tag='ProjectProductPlan' codelistType='DYNAMIC' placeholder='请选择...' style=""></dropdown-list>
</app-form-item>
...
export default class ProjectPlanBase extends Vue implements ControlInterface {
     ...
     * @memberof ProjectPlan
     */
    protected service: ProjectPlanService = new ProjectPlanService({ $store: this.$store });
    ...
    protected detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: '(此操作会将所选计划下面的需求全部关联到此项目中)', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: true, form: this, uiActionGroup: { caption: '', langbase: 'entities.story.projectplan_form', extractMode: 'ITEM', details: [] } })
, 
        formpage1: new FormPageModel({ caption: '按照计划关联', detailType: 'FORMPAGE', name: 'formpage1', visible: true, isShowCaption: true, form: this })
, 
...
, 
    ...
    protected print(){
        let _this:any = this;
        _this.$print({id:'form',popTitle:'项目计划关联'});
    }
    ...
    private load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'StoryProjectPlanView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    protected loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'StoryProjectPlanView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    protected autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'StoryProjectPlanView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    private remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'StoryProjectPlanView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    protected async wfsubmit(data: any): Promise<any> {
        ...
        if(!this.WFSubmitAction){
            this.$Notice.error({ title: '错误', desc: 'StoryProjectPlanView视图表单WFSubmitAction参数未配置' });
            return;
        }
        const _this: any = this;
        const arg: any = data[0];
