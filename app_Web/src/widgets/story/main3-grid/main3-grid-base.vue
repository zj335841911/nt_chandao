...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('pri')">
                <el-table-column show-overflow-tooltip :prop="'pri'" :label="$t('entities.story.main3_grid.columns.pri')" :width="70" :align="'left'">
                    <template v-slot="{row,column}">
                        <template >
            ...
            <template v-if="getColumnState('title')">
                <el-table-column show-overflow-tooltip :prop="'title'" :label="$t('entities.story.main3_grid.columns.title')" :align="'left'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('uagridcolumn1')">
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.story.main3_grid.columns.uagridcolumn1')" :width="300" :align="'center'">
                    <template slot-scope="scope">
                        <span>
                            
                            <a @click="uiAction(scope.row, 'ChangeStoryDetail', $event)">{{$t('entities.story.main3_grid.uiactions.changestorydetail')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'ReviewStory', $event)">{{$t('entities.story.main3_grid.uiactions.reviewstory')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'CloseStory', $event)">{{$t('entities.story.main3_grid.uiactions.closestory')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'OpenBaseInfoEditView', $event)">{{$t('entities.story.main3_grid.uiactions.openbaseinfoeditview')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'OpenCaseCreateView', $event)">{{$t('entities.story.main3_grid.uiactions.opencasecreateview')}}</a>
                        </span>
                    </template>
                </el-table-column>
            </template>
...
export default class Main3Base extends Vue implements ControlInterface {
     ...
     * @memberof Main3
     */
    protected service: Main3Service = new Main3Service({ $store: this.$store });
     ...
     * @memberof Main3
     */
    protected isNoSort: boolean = true;
    ...
    */
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'StoryGridView9_SubStory视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'StoryGridView9_SubStory视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
    ...
    protected setColState() {
		const _data: any = localStorage.getItem('zt_story_main3_grid');
		if (_data) {
    ...
    protected onColChange() {
        localStorage.setItem('zt_story_main3_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'StoryGridView9_SubStory视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'StoryGridView9_SubStory视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.story){
