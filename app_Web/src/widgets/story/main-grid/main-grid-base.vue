        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
                ...
                <el-table-column show-overflow-tooltip :prop="'id'" :label="$t('entities.story.main_grid.columns.id')" :width="70"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.story.main_grid.columns.id')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('pri')">
                <el-table-column show-overflow-tooltip :prop="'pri'" :label="$t('entities.story.main_grid.columns.pri')" :width="70"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.story.main_grid.columns.pri')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('title')">
                <el-table-column show-overflow-tooltip :prop="'title'" :label="$t('entities.story.main_grid.columns.title')" :min-width="1"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.story.main_grid.columns.title')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.title}}</span>
            ...
            <template v-if="getColumnState('modulename')">
                <el-table-column show-overflow-tooltip :prop="'modulename'" :label="$t('entities.story.main_grid.columns.modulename')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.story.main_grid.columns.modulename')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('uagridcolumn1')">
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.story.main_grid.columns.uagridcolumn1')" :width="100"  :align="'center'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.story.main_grid.columns.uagridcolumn1')}}
                      </span>
                    </template>
                    <template slot-scope="scope">
                        <span>
                            ...
                            <a @click="uiAction(scope.row, 'ChangeStoryDetail', $event)">
                              ...
                              {{$t('entities.story.main_grid.uiactions.changestorydetail')}}
                            </a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'CloseStory', $event)">
                              <i class='fa fa-power-off'></i>
                              {{$t('entities.story.main_grid.uiactions.closestory')}}
                            </a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'OpenBaseInfoEditView', $event)">
                              <i class=''></i>
                              {{$t('entities.story.main_grid.uiactions.openbaseinfoeditview')}}
                            </a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'OpenCaseCreateView', $event)">
                              <i class='fa fa-lightbulb-o'></i>
                              {{$t('entities.story.main_grid.uiactions.opencasecreateview')}}
                            </a>
                        </span>
                    </template>
                </el-table-column>
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'StoryMainGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'StoryMainGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'StoryMainGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'StoryMainGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.story){
