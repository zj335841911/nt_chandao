...
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.case.maintesttask_form.details.group1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="true">
    
    <row>
        <i-col v-show="detailsModel.precondition.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='precondition' :itemRules="this.rules.precondition" class='' :caption="$t('entities.case.maintesttask_form.details.precondition')" uiStyle="DEFAULT" :labelWidth="100" :isShowCaption="true" :error="detailsModel.precondition.error" :isEmptyCaption="false" labelPos="LEFT">
    <app-span   name='precondition'
:value="data.precondition" style=""></app-span>
</app-form-item>
...
<i-col v-show="detailsModel.druipart1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    ...
    viewname='case-step-carry-out-edit-mode' 
...
<i-col v-show="detailsModel.resultcnt.visible" :style="{}"  :lg="{ span: 24, offset: 0 }" :xl="{ span: 6, offset: 0 }">
    <app-form-item name='resultcnt' :itemRules="this.rules.resultcnt" class='' :caption="$t('entities.case.maintesttask_form.details.resultcnt')" uiStyle="DEFAULT" :labelWidth="100" :isShowCaption="true" :error="detailsModel.resultcnt.error" :isEmptyCaption="false" labelPos="LEFT">
    <app-span   name='resultcnt'
:value="data.resultcnt" style=""></app-span>
</app-form-item>

</i-col>
<i-col v-show="detailsModel.resultfalicnt.visible" :style="{}"  :lg="{ span: 24, offset: 0 }" :xl="{ span: 8, offset: 0 }">
    <app-form-item name='resultfalicnt' :itemRules="this.rules.resultfalicnt" class='' :caption="$t('entities.case.maintesttask_form.details.resultfalicnt')" uiStyle="DEFAULT" :labelWidth="100" :isShowCaption="true" :error="detailsModel.resultfalicnt.error" :isEmptyCaption="false" labelPos="LEFT">
    <app-span   name='resultfalicnt'
:value="data.resultfalicnt" style=""></app-span>
</app-form-item>
...
export default class MainTestTaskBase extends Vue implements ControlInterface {

    /**
     * 名称
     *
     * @type {string}
     * @memberof MainTestTask
     */
     ...
     * @memberof MainTestTask
     */
    protected service: MainTestTaskService = new MainTestTaskService({ $store: this.$store });
    ...
    protected rules: any = {
        ...
        task: [
            { type: 'string', message: '属性 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '属性 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '属性 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '属性 值不能为空', trigger: 'blur' },
        ],
        precondition: [
        ...
        resultcnt: [
            { type: 'number', message: '测试结果数 值必须为数值类型', trigger: 'change' },
            { type: 'number', message: '测试结果数 值必须为数值类型', trigger: 'blur' },
            { required: false, type: 'number', message: '测试结果数 值不能为空', trigger: 'change' },
            { required: false, type: 'number', message: '测试结果数 值不能为空', trigger: 'blur' },
        ],
        resultfalicnt: [
            { type: 'number', message: '测试失败数 值必须为数值类型', trigger: 'change' },
            { type: 'number', message: '测试失败数 值必须为数值类型', trigger: 'blur' },
            { required: false, type: 'number', message: '测试失败数 值不能为空', trigger: 'change' },
            { required: false, type: 'number', message: '测试失败数 值不能为空', trigger: 'blur' },
        ],
        id: [
            { type: 'number', message: '用例编号 值必须为数值类型', trigger: 'change' },
            { type: 'number', message: '用例编号 值必须为数值类型', trigger: 'blur' },
...
, 
        group1: new FormGroupPanelModel({ caption: '测试用例基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'entities.case.maintesttask_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
        task: new FormItemModel({ caption: '属性', detailType: 'FORMITEM', name: 'task', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        precondition: new FormItemModel({ caption: '前置条件', detailType: 'FORMITEM', name: 'precondition', visible: false, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        resultcnt: new FormItemModel({ caption: '测试结果数', detailType: 'FORMITEM', name: 'resultcnt', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        resultfalicnt: new FormItemModel({ caption: '测试失败数', detailType: 'FORMITEM', name: 'resultfalicnt', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
     ...
     * @memberof MainTestTask
     */
    @Watch('data.resultcnt')
    onResultcntChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'resultcnt', newVal: newVal, oldVal: oldVal });
    }

    /**
     * 监控表单属性 resultfalicnt 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof MainTestTask
     */
    @Watch('data.resultfalicnt')
    onResultfalicntChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'resultfalicnt', newVal: newVal, oldVal: oldVal });
    }
     ...
     *
     * @param {*} [arg={}]
     * @memberof @memberof MainTestTask
     */
    ...
    protected print(){
        let _this:any = this;
        _this.$print({id:'form',popTitle:'表单（测试单）'});
    }
    ...
    private load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'CaseTestOptionView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    protected loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'CaseTestOptionView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    protected autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'CaseTestOptionView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    private remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'CaseTestOptionView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    protected async wfsubmit(data: any): Promise<any> {
        ...
        if(!this.WFSubmitAction){
            this.$Notice.error({ title: '错误', desc: 'CaseTestOptionView视图表单WFSubmitAction参数未配置' });
            return;
        }
        const _this: any = this;
        const arg: any = data[0];
