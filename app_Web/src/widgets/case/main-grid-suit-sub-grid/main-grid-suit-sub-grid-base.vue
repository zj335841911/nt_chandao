    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('title')">
                <el-table-column show-overflow-tooltip :prop="'title'" :label="$t('entities.case.maingrid_suitsub_grid.columns.title')" :min-width="1"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
            ...
            <template v-if="getColumnState('status')">
                <el-table-column show-overflow-tooltip :prop="'status'" :label="$t('entities.case.maingrid_suitsub_grid.columns.status')" :width="80"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.case.maingrid_suitsub_grid.columns.status')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('uagridcolumn1')">
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.case.maingrid_suitsub_grid.columns.uagridcolumn1')" :width="160"  :align="'center'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                    ...
                    <template slot-scope="scope">
                        <span>
                            ...
                            <a title="" @click="uiAction(scope.row, 'OpenTestRunResultView', $event)">
                              <i class='fa fa-stack-overflow'></i>
                              
                            </a>
                            <divider type='vertical'></divider>
                            <a title="" @click="uiAction(scope.row, 'Execute', $event)">
...
export default class MainGrid_SuitSubBase extends CtrlBase {
    ...
    */
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'CaseCurSuitGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'CaseCurSuitGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'CaseCurSuitGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'CaseCurSuitGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.case){
...
<style lang='less'>
@import './main-grid-suit-sub-grid.less';
</style>