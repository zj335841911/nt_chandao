...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('title')">
                <el-table-column show-overflow-tooltip :prop="'title'" :label="$t('entities.case.testmaingrid_grid.columns.title')" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('lastrunner')">
                <el-table-column show-overflow-tooltip :prop="'lastrunner'" :label="$t('entities.case.testmaingrid_grid.columns.lastrunner')" :width="100" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            <codelist :value="row.lastrunner" tag='UserRealName' codelistType='DYNAMIC' renderMode="STR" valueSeparator="," textSeparator="," ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            ...
            <template v-if="getColumnState('uagridcolumn1')">
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.case.testmaingrid_grid.columns.uagridcolumn1')" :width="180" :align="'center'">
                    <template slot-scope="scope">
                        <span>
                            
                            <a @click="uiAction(scope.row, 'confirmChange', $event)">{{$t('entities.case.testmaingrid_grid.uiactions.confirmchange')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'NewBugByCaseResult', $event)">{{$t('entities.case.testmaingrid_grid.uiactions.newbugbycaseresult')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'CASEOpenTestRunResultView', $event)">{{$t('entities.case.testmaingrid_grid.uiactions.caseopentestrunresultview')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'CaseExecute', $event)">{{$t('entities.case.testmaingrid_grid.uiactions.caseexecute')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'unlinkCase', $event)">{{$t('entities.case.testmaingrid_grid.uiactions.unlinkcase')}}</a>
                        </span>
                    </template>
                </el-table-column>
            </template>
...
export default class TestMainGridBase extends Vue implements ControlInterface {

    /**
     * 名称
     *
     * @type {string}
     * @memberof TestMainGrid
     */
     ...
     * @memberof TestMainGrid
     */
    protected service: TestMainGridService = new TestMainGridService({ $store: this.$store });
     ...
     * @memberof 
     */
    protected grid_title_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_u5414985_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_u06228b0_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_u38f3657_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_u292efe3_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_u84b598d_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
    ...
    */
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'CaseCurTestTaskGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'CaseCurTestTaskGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(this.removeAction,Object.assign(context,{ case: keys.join(';') }),Object.assign({ id: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    protected rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.zt_case,$event.zt_case);
        });
    ...
    protected setColState() {
		const _data: any = localStorage.getItem('zt_case_testmaingrid_grid');
		if (_data) {
    ...
    protected onColChange() {
        localStorage.setItem('zt_case_testmaingrid_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'CaseCurTestTaskGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'CaseCurTestTaskGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.case){
                    Object.assign(this.context,{case:item.case})
                }
...
<style lang='less'>
@import './test-main-grid-grid.less';
</style>