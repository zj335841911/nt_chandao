    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('title')">
                <el-table-column show-overflow-tooltip :prop="'title'" :label="$t('entities.case.maingrid_grid.columns.title')" :min-width="1"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
            ...
            <template v-if="getColumnState('status')">
                <el-table-column show-overflow-tooltip :prop="'status'" :label="$t('entities.case.maingrid_grid.columns.status')" :width="80"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.case.maingrid_grid.columns.status')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
                ...
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.case.maingrid_grid.columns.uagridcolumn1')" :width="160"  :align="'center'">
                    ...
                    <template slot-scope="scope">
                        <span>
                            ...
                            <divider type='vertical'></divider>
                            <a title="" @click="uiAction(scope.row, 'NewBugByCaseResult', $event)">
                              <i class='fa fa-bug'></i>
...
export default class MainGridBase extends CtrlBase {
    ...
    */
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'CaseMainGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'CaseMainGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'CaseMainGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'CaseMainGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.case){
