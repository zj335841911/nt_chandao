...
<i-col v-show="detailsModel.grouppanel2.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class=' form-padding' uiActionGroup="detailsModel.grouppanel2.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.action.main_form.details.grouppanel2')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false">
    
    <row>
        <i-col v-show="detailsModel.lastcomment.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='lastcomment' :itemRules="this.rules.lastcomment" class='' :caption="$t('entities.action.main_form.details.lastcomment')" uiStyle="DEFAULT" :labelWidth="1" :isShowCaption="true" :error="detailsModel.lastcomment.error" :isEmptyCaption="true" labelPos="LEFT">
    <app-rich-text-editor :formState="formState" :value="data.lastcomment" @change="(val) =>{this.data.lastcomment =val}" :disabled="detailsModel.lastcomment.disabled"  name="lastcomment" style=""></app-rich-text-editor>
</app-form-item>

</i-col>
<i-col v-show="detailsModel.grouppanel1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' uiActionGroup="detailsModel.grouppanel1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.action.main_form.details.grouppanel1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false">
    
    <div style="height: 100%;display: flex;justify-content: flex-end;">
        <div  v-show="detailsModel.button1.visible" :style="{'width': '64px',}">
    <i-button type="primary" @click="button1_click($event)"
         style="">
        <i class="fa fa-sign-out" style="margin-right: 2px;"></i>   
        <span >{{$t('entities.action.main_form.details.button1')}}</span>
    </i-button>

</div>
<div  v-show="detailsModel.button2.visible" :style="{'width': '58px',}">
    <i-button type="primary" @click="button2_click($event)"
         style="">
        <i class="fa fa-save" style="margin-right: 2px;"></i>   
        <span >{{$t('entities.action.main_form.details.button2')}}</span>
    </i-button>
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    protected detailsModel: any = {
        button1: new FormButtonModel({ caption: '关闭', detailType: 'BUTTON', name: 'button1', visible: true, isShowCaption: true, form: this, uiaction: { type: 'DEUIACTION', 
 tag: 'Exit' } })
, 
        button2: new FormButtonModel({ caption: '保存', detailType: 'BUTTON', name: 'button2', visible: true, isShowCaption: true, form: this, uiaction: { type: 'DEUIACTION', 
 tag: 'SaveAndExit' } })
, 
        grouppanel1: new FormGroupPanelModel({ caption: '分组面板', detailType: 'GROUPPANEL', name: 'grouppanel1', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'entities.action.main_form', extractMode: 'ITEM', details: [] } })
, 
        grouppanel2: new FormGroupPanelModel({ caption: '分组面板', detailType: 'GROUPPANEL', name: 'grouppanel2', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'entities.action.main_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
        srfmajortext: new FormItemModel({ caption: '备注', detailType: 'FORMITEM', name: 'srfmajortext', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        lastcomment: new FormItemModel({ caption: '', detailType: 'FORMITEM', name: 'lastcomment', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    /**
     * 监控表单属性 lastcomment 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof Main
     */
    @Watch('data.lastcomment')
    onLastcommentChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'lastcomment', newVal: newVal, oldVal: oldVal });
    }
    ...
    private onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.action){
            Object.assign(this.context,{action:data.action})
        }
    ...
    private load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'ActionEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    protected loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ActionEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    protected autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'ActionEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    private remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'ActionEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    protected async wfsubmit(data: any): Promise<any> {
        ...
        if(!this.WFSubmitAction){
            this.$Notice.error({ title: '错误', desc: 'ActionEditView视图表单WFSubmitAction参数未配置' });
            return;
        }
        const _this: any = this;
        const arg: any = data[0];
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.action || Object.is(arg.action, '')) {
            return;
        }
