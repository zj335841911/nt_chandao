<template>  
    <div :class="['app-list', this.items.length > 0 ? '' : 'app-list-empty' ]">
            <div v-if="items.length > 0">
                                <action-timeline  :items="items"></action-timeline>
            </div>
            <div v-else>
                暂无数据
            </div>
...
export default class GroupTrendsBase extends Vue implements ControlInterface {
     ...
     * @memberof GroupTrends
     */
    protected service: GroupTrendsService = new GroupTrendsService({ $store: this.$store });
    ...
    */
    ...
    private load(opt: any = {}): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ActionProductTrendsListView视图列表fetchAction参数未配置' });
            return;
        }        
        const arg: any = {...opt};
        const page: any = {};
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ActionProductTrendsListView视图表格removeAction参数未配置' });
            return;
        }
        if (datas.length === 0) {
            return;
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(this.removeAction,Object.assign(context,{ action: keys.join(';') }),Object.assign({ id: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
