<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='user_main' style="">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        srfmajortext: [
            { type: 'string', message: '真实姓名 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '真实姓名 值必须为字符串类型', trigger: 'blur' },
        ...
        join: [
            { type: 'string', message: '入职日期 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '入职日期 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '入职日期 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '入职日期 值不能为空', trigger: 'blur' },
        ],
        dept: [
        ...
        account: [
            ...
            { required: false, type: 'string', message: '账户 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '账户 值不能为空', trigger: 'blur' },
        ],
        email: [
        ...
        password: [
            { type: 'string', message: '密码 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '密码 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '密码 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '密码 值不能为空', trigger: 'blur' },
        ],
        mobile: [
            { type: 'string', message: '手机 值必须为字符串类型', trigger: 'change' },
        ...
        dingding: [
            ...
            { type: 'string', message: '钉钉 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '钉钉 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '钉钉 值不能为空', trigger: 'blur' },
        ],
        weixin: [
...
, 
        srfmajortext: new FormItemModel({ caption: '真实姓名', detailType: 'FORMITEM', name: 'srfmajortext', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        join: new FormItemModel({ caption: '入职日期', detailType: 'FORMITEM', name: 'join', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        password: new FormItemModel({ caption: '密码', detailType: 'FORMITEM', name: 'password', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
     ...
     */
    @Watch('data.realname')
    onRealnameChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'realname', newVal: newVal, oldVal: oldVal });
    }
     ...
     */
    @Watch('data.dingding')
    onDingdingChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'dingding', newVal: newVal, oldVal: oldVal });
    }
    ...
    public onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.user){
            Object.assign(this.context,{user:data.user})
        }
    ...
    public copy(srfkey: string): void {
        ...
        copyData.user = copyData.srfkey;
        copyData.id = copyData.srfkey;
        Object.assign(this.context,{user:copyData.user})
        this.data = copyData;
        this.$nextTick(() => {
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'user_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'UserEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'UserEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'UserEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'UserEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.user || Object.is(arg.user, '')) {
            return;
        }
