    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('visits')">
                <el-table-column show-overflow-tooltip :prop="'visits'" :label="$t('entities.user.main_grid.columns.visits')" :width="100"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
            ...
            <template v-if="getColumnState('uagridcolumn1')">
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.user.main_grid.columns.uagridcolumn1')" :width="200"  :align="'center'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                    ...
                    <template slot-scope="scope">
                        <span>
                            ...
                            <a @click="uiAction(scope.row, 'Edit', $event)">
                              ...
                              {{$t('entities.user.main_grid.uiactions.edit')}}
                            </a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'Remove', $event)">
                              <i class='fa fa-remove'></i>
                              {{$t('entities.user.main_grid.uiactions.remove')}}
                            </a>
                        </span>
                    </template>
                </el-table-column>
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'UserMainGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'UserMainGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.realname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ user: _keys }),Object.assign({ user: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.zt_user,$event.zt_user);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('zt_user_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('zt_user_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'UserMainGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'UserMainGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.user){
                            Object.assign(this.context,{user:item.user});
                        }
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.user,args.row.user);
        });
