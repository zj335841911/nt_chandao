...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('id')">
                <el-table-column show-overflow-tooltip :prop="'id'" :label="$t('entities.project.mainmy_grid.columns.id')" :width="70" :align="'left'" :sortable="'custom'">
            ...
            <template v-if="getColumnState('code')">
                <el-table-column show-overflow-tooltip :prop="'code'" :label="$t('entities.project.mainmy_grid.columns.code')" :width="150" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <span>{{row.code}}</span>
                    </template>
                </el-table-column>
            ...
            <template v-if="getColumnState('name')">
                <el-table-column show-overflow-tooltip :prop="'name'" :label="$t('entities.project.mainmy_grid.columns.name')" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('begin')">
                <el-table-column show-overflow-tooltip :prop="'begin'" :label="$t('entities.project.mainmy_grid.columns.begin')" :width="100" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('end')">
                <el-table-column show-overflow-tooltip :prop="'end'" :label="$t('entities.project.mainmy_grid.columns.end')" :width="120" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('status')">
                <el-table-column show-overflow-tooltip :prop="'status'" :label="$t('entities.project.mainmy_grid.columns.status')" :width="100" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            ...
            <template v-if="getColumnState('role')">
                <el-table-column show-overflow-tooltip :prop="'role'" :label="$t('entities.project.mainmy_grid.columns.role')" :width="100" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <span>{{row.role}}</span>
                    </template>
                </el-table-column>
            ...
            <template v-if="getColumnState('join')">
                <el-table-column show-overflow-tooltip :prop="'join'" :label="$t('entities.project.mainmy_grid.columns.join')" :width="100" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('hours')">
                <el-table-column show-overflow-tooltip :prop="'hours'" :label="$t('entities.project.mainmy_grid.columns.hours')" :width="100" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                            <app-format-data dataType="DECIMAL" precision="0" :data="row.hours"></app-format-data>
                    </template>
                </el-table-column>
            </template>
...
export default class MainMyBase extends Vue implements ControlInterface {
     ...
     * @memberof MainMy
     */
    protected service: MainMyService = new MainMyService({ $store: this.$store });
    ...
    */
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ProjectMainMyGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ProjectMainMyGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
    ...
    protected setColState() {
		const _data: any = localStorage.getItem('zt_project_mainmy_grid');
		if (_data) {
    ...
    protected onColChange() {
        localStorage.setItem('zt_project_mainmy_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'ProjectMainMyGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'ProjectMainMyGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.project){
