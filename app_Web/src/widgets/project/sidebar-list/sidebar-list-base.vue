<template>  
    <div :class="['app-list', this.items.length > 0 ? '' : 'app-list-empty' ]">
            <div v-if="items.length > 0" style="height:100%;">
                                <project-list :items="items" @item-click="handleDblClick"/>
            </div>
            <div v-else>
                暂无数据
            </div>
    ...
    */
    ...
    public async remove(datas: any[]): Promise<any> {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ project: keys.join(';') }),Object.assign({ project: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
