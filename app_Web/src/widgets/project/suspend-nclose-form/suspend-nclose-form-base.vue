...
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 23, offset: 0 }" :xl="{ span: 23, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.project.suspendnclose_form.details.group1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false">
    
    <row>
        <i-col v-show="detailsModel.comment.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='comment' :itemRules="this.rules.comment" class='' :caption="$t('entities.project.suspendnclose_form.details.comment')" uiStyle="DEFAULT" :labelWidth="100" :isShowCaption="true" :error="detailsModel.comment.error" :isEmptyCaption="false" labelPos="LEFT">
...
export default class SuspendNCloseBase extends Vue implements ControlInterface {

    /**
     * 名称
     *
     * @type {string}
     * @memberof SuspendNClose
     */
     ...
     * @memberof SuspendNClose
     */
    protected service: SuspendNCloseService = new SuspendNCloseService({ $store: this.$store });
...
, 
     ...
     *
     * @param {*} [arg={}]
     * @memberof @memberof SuspendNClose
     */
    ...
    protected print(){
        let _this:any = this;
        _this.$print({id:'form',popTitle:'挂起、关闭操作通用表单'});
    }
    ...
    private load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'ProjectEditView_Suspend视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    protected loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ProjectEditView_Suspend视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    protected autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'ProjectEditView_Suspend视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    private remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'ProjectEditView_Suspend视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    protected async wfsubmit(data: any): Promise<any> {
        ...
        if(!this.WFSubmitAction){
            this.$Notice.error({ title: '错误', desc: 'ProjectEditView_Suspend视图表单WFSubmitAction参数未配置' });
            return;
        }
        const _this: any = this;
        const arg: any = data[0];
...
<style lang='less'>
@import './suspend-nclose-form.less';
</style>