...
export default class ControlService {
    ...
    dataItems.forEach((item: any) => {
      if (item && item.dataType && Object.is(item.dataType, "FONTKEY")) {
        ...
        if (
          ...
          item.prop &&
          item.name &&
          (data[item.name] || Object.is(data[item.name], 0))
        ) {
    ...
    const handleResult: any = (
      ...
      if (response.data instanceof Array) {
        ...
        response.data.forEach((item: any) => {
          result.push(
            this.handleResponseData(action, item, isCreate, codelistArray)
          );
        });
      } else {
        result = this.handleResponseData(
          action,
          response.data,
          isCreate,
          codelistArray
      ...
      if (response.headers) {
        ...
        if (response.headers["x-per-page"]) {
          Object.assign(response, {
            size: Number(response.headers["x-per-page"])
          });
        }
  ...
  public handleResponseData(
    ...
    dataItems.forEach(dataitem => {
      ...
      if (
        (isCreate === undefined || isCreate === null) &&
        Object.is(dataitem.dataType, "GUID") &&
        Object.is(dataitem.name, "srfkey") &&
        val && !Object.is(val, "")
      ) {
        isCreate = true;
      }
      ...
      if (codelistArray && dataitem.codelist) {
        if (
          codelistArray.get(dataitem.codelist.tag) &&
          codelistArray.get(dataitem.codelist.tag).get(val)
        ) {
          item[dataitem.name] = codelistArray
            .get(dataitem.codelist.tag)
            .get(val);
        }
      }
    });
  ...
  public getCodeList(codeListObject: any): Promise<any> {
    return new Promise((resolve: any, reject: any) => {
      if (
        ...
        const codelist = (this.getStore() as Store<any>).getters.getCodeList(
          codeListObject.tag
        );
        if (codelist) {
