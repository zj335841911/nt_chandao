...
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }" :xl="{ span: 8, offset: 0 }">
    ...
    <app-range-editor v-model="data.formitemex1" :activeData="data" :disabled="detailsModel.formitemex1.disabled" name="formitemex1" editorType="USERCONTROL" format="" :refFormItem="['custom1','custom2']" @formitemvaluechange="onFormItemValueChange" style="height:250px;"></app-range-editor>
</app-form-item>
...
:value="data.assigntomebugcnt" style=""></app-span>
</app-form-item>
...
<app-form-group layoutType="FLEX" titleStyle="" class='' uiActionGroup="detailsModel.grouppanel3.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.productstats.testmain2_form.details.grouppanel3')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false">
    
    <row>
        <i-col v-show="detailsModel.rawitem1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <div class="" style="">
    <div style="padding-left: 69px;"><Icon type="md-checkmark-circle" color="#49db44"/>&nbsp;昨天解决Bug数&nbsp;<b>{{data.yesterdayresolvedbugcnt ? data.yesterdayresolvedbugcnt : 0}}</b></div>
</div>
...
<i-col v-show="detailsModel.rawitem2.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <div class="" style="">
    <div style="padding-left: 69px;"><Icon type="md-checkmark-circle" color="#49db44"/>&nbsp;昨天确认Bug数&nbsp;<b>{{data.yesterdayconfirmbugcnt ? data.yesterdayconfirmbugcnt : 0}}</b></div>
</div>
...
:value="data.unconfirmbugcnt" style=""></app-span>
</app-form-item>

</i-col>
<i-col v-show="detailsModel.rawitem3.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <div class="" style="">
    <div style="padding-left: 69px;"><Icon type="md-checkmark-circle" color="#49db44"/>&nbsp;昨天关闭Bug数&nbsp;<b>{{data.yesterdayClosedBugCnt ? data.yesterdayClosedBugCnt: 0}}</b></div>
</div>
...
<i-col v-show="detailsModel.formitemex4.visible" :style="{}"  :lg="{ span: 24, offset: 0 }" :xl="{ span: 20, offset: 0 }">
    ...
    <app-range-editor v-model="data.formitemex4" :activeData="data" :disabled="detailsModel.formitemex4.disabled" name="formitemex4" editorType="USERCONTROL" format="" :refFormItem="['custom7','custom8']" @formitemvaluechange="onFormItemValueChange" style=""></app-range-editor>
</app-form-item>
...
:value="data.notclosedbugcnt" style=""></app-span>
</app-form-item>
...
export default class TestMain2Base extends Vue implements ControlInterface {
     ...
     * @memberof TestMain2
     */
    protected service: TestMain2Service = new TestMain2Service({ $store: this.$store });
     ...
     * @memberof TestMain2
     */
    protected appEntityService: ProductStatsService = new ProductStatsService({ $store: this.$store });
...
, 
    ...
    onAssigntomebugcntChange(newVal: any, oldVal: any) {
    ...
    protected print(){
        let _this:any = this;
        _this.$print({id:'form',popTitle:'嵌入测试统计'});
    }
    ...
    private load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'ProductStatsTestEditView9视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    protected loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ProductStatsTestEditView9视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    protected autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'ProductStatsTestEditView9视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    private remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'ProductStatsTestEditView9视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    protected async wfsubmit(data: any): Promise<any> {
        ...
        if(!this.WFSubmitAction){
            this.$Notice.error({ title: '错误', desc: 'ProductStatsTestEditView9视图表单WFSubmitAction参数未配置' });
            return;
        }
        const _this: any = this;
        const arg: any = data[0];
