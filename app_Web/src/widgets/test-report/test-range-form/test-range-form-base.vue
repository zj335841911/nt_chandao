...
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.testreport.testrange_form.details.group1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="true">
    ...
    :formState="formState"
    paramItem='testreport' 
    :parentdata='{"srfparentdename":"ZT_TESTREPORT","SRFPARENTTYPE":"CUSTOM"}'
    :parameters="[
        { pathName: 'products', parameterName: 'product' },
    ]"
    :context="context"
    :viewparams="viewparams"
    parameterName='testreport'
    parentName="TestReport"  
    refviewtype='DEGRIDVIEW' 
    refreshitems='' 
    :ignorefieldvaluechange="ignorefieldvaluechange"
    viewname='story-report-sub-grid-view' 
...
<i-col v-show="detailsModel.druipart2.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    ...
    refreshitems='' 
    :ignorefieldvaluechange="ignorefieldvaluechange"
    viewname='bug-test-report-sub-grid-view' 
    :data="JSON.stringify(this.data)" 
    @drdatasaved="drdatasaved($event)"
    style=";overflow: auto;">
</app-form-druipart>
...
export default class TestRangeBase extends Vue implements ControlInterface {
     ...
     * @memberof TestRange
     */
    protected service: TestRangeService = new TestRangeService({ $store: this.$store });
...
, 
        group1: new FormGroupPanelModel({ caption: '基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'entities.testreport.testrange_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
    ...
    protected print(){
        let _this:any = this;
        _this.$print({id:'form',popTitle:'测试范围'});
    }
    ...
    private load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'TestReportEditView_TestRange视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    protected loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TestReportEditView_TestRange视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    protected autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'TestReportEditView_TestRange视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    private remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'TestReportEditView_TestRange视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    protected async wfsubmit(data: any): Promise<any> {
        ...
        if(!this.WFSubmitAction){
            this.$Notice.error({ title: '错误', desc: 'TestReportEditView_TestRange视图表单WFSubmitAction参数未配置' });
            return;
        }
        const _this: any = this;
        const arg: any = data[0];
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.testreport || Object.is(arg.testreport, '')) {
            return;
        }
