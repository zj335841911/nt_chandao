...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('id')">
                <el-table-column show-overflow-tooltip :prop="'id'" :label="$t('entities.testreport.main_grid.columns.id')" :width="70" :align="'left'" :sortable="'custom'">
            ...
            <template v-if="getColumnState('title')">
                <el-table-column show-overflow-tooltip :prop="'title'" :label="$t('entities.testreport.main_grid.columns.title')" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <app-column-link deKeyField='testreport' :context="JSON.parse(JSON.stringify(context))" :viewparams="JSON.parse(JSON.stringify(viewparams))" :data="row" :linkview="{viewname: 'test-report-main-tab-exp-view', height: 0,width: 0,title: $t('entities.testreport.views.maintabexpview.title'),placement: 'DRAWER_TOP', isRedirectView: false,deResParameters: [
            ...
            <template v-if="getColumnState('createdby')">
                <el-table-column show-overflow-tooltip :prop="'createdby'" :label="$t('entities.testreport.main_grid.columns.createdby')" :width="100" :align="'center'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            <codelist :value="row.createdby" tag='UserRealName' codelistType='DYNAMIC' renderMode="STR" valueSeparator="," textSeparator="," ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('createddate')">
                <el-table-column show-overflow-tooltip :prop="'createddate'" :label="$t('entities.testreport.main_grid.columns.createddate')" :width="160" :align="'center'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <app-format-data format="YYYY-MM-DD HH:mm:ss" :data="row.createddate"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('projectname')">
                <el-table-column show-overflow-tooltip :prop="'projectname'" :label="$t('entities.testreport.main_grid.columns.projectname')" :width="250" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('tasks')">
                <el-table-column show-overflow-tooltip :prop="'tasks'" :label="$t('entities.testreport.main_grid.columns.tasks')" :width="250" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            <codelist :value="row.tasks" tag='TestTask' codelistType='DYNAMIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            ...
            FTL stack trace ("~" means nesting-related):
            	- Failed at: #if item.getPSDEUIActionGroup().getPS...  [in template "TEMPLCODE_en_US" at line 53, column 17]
            ----
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof Main
     */
    protected appEntityService: TestReportService = new TestReportService({ $store: this.$store });
    ...
    */
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TestReportGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TestReportGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(this.removeAction,Object.assign(context,{ testreport: keys.join(';') }),Object.assign({ id: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    protected rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.zt_testreport,$event.zt_testreport);
        });
    ...
    protected setColState() {
		const _data: any = localStorage.getItem('zt_testreport_main_grid');
		if (_data) {
    ...
    protected onColChange() {
        localStorage.setItem('zt_testreport_main_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'TestReportGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'TestReportGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.testreport){
                    Object.assign(this.context,{testreport:item.testreport})
                }
    ...
    protected getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.testreport,args.row.testreport);
        });
