...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('username')">
                <el-table-column show-overflow-tooltip :prop="'username'" :label="$t('entities.projectteam.main_editrow_grid.columns.username')" :width="400" :align="'left'" :sortable="'custom'">
              ...
              name='username' 
              deMajorField='realname'
              deKeyField='user'
              :service="service"
              :acParams="{ serviceName: 'UserService', interfaceName: 'FetchDefault'}"
              valueitem='account' 
              ...
              @formitemvaluechange="($event)=>{onGridItemValueChange(row,$event)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='username' editorType="PICKER" :value="row.username"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('role')">
                <el-table-column show-overflow-tooltip :prop="'role'" :label="$t('entities.projectteam.main_editrow_grid.columns.role')" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='role' editorType="TEXTBOX" :value="row.role"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('days')">
                <el-table-column show-overflow-tooltip :prop="'days'" :label="$t('entities.projectteam.main_editrow_grid.columns.days')" :width="200" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
            ...
            ----
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='days' editorType="NUMBER" :value="row.days"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('hours')">
                <el-table-column show-overflow-tooltip :prop="'hours'" :label="$t('entities.projectteam.main_editrow_grid.columns.hours')" :width="200" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
            ...
            ----
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='hours' editorType="NUMBER" :value="row.hours"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('limited')">
                <el-table-column show-overflow-tooltip :prop="'limited'" :label="$t('entities.projectteam.main_editrow_grid.columns.limited')" :width="200" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
                                    <app-radio-group v-model="row[column.property]" :disabled="row.srfuf === 1 ? (3 & 2) !== 2 : (3 & 1) !== 1" name="limited" @change="($event)=>{gridEditItemChange(row, column.property, $event)}" tag='YesNo3' codelistType='STATIC'  style=""></app-radio-group>
                                </app-form-item>
                            </i-form>
                        </template>
                        <template v-if="!actualIsOpenEdit">
...
export default class Main_EditRowBase extends Vue implements ControlInterface {

    /**
     * 名称
     *
     * @type {string}
     * @memberof Main_EditRow
     */
     ...
     * @memberof Main_EditRow
     */
    protected service: Main_EditRowService = new Main_EditRowService({ $store: this.$store });
    ...
    */
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ProjectTeamMainGridView_EditRow视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ProjectTeamMainGridView_EditRow视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
    ...
    protected setColState() {
		const _data: any = localStorage.getItem('ibz_projectteam_main_editrow_grid');
		if (_data) {
    ...
    protected onColChange() {
        localStorage.setItem('ibz_projectteam_main_editrow_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'ProjectTeamMainGridView_EditRow视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'ProjectTeamMainGridView_EditRow视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.projectteam){
    ...
    protected newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ProjectTeamMainGridView_EditRow视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
