...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('dates')">
                <el-table-column show-overflow-tooltip :prop="'dates'" :label="$t('entities.ibztaskestimate.main_grid.columns.dates')" :width="100" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='dates' editorType="DATEPICKER" :value="row.dates"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('consumed')">
                <el-table-column show-overflow-tooltip :prop="'consumed'" :label="$t('entities.ibztaskestimate.main_grid.columns.consumed')" :width="100" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
            ...
            <template v-if="getColumnState('left')">
                <el-table-column show-overflow-tooltip :prop="'left'" :label="$t('entities.ibztaskestimate.main_grid.columns.left')" :width="100" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
            ...
            <template v-if="getColumnState('work')">
                <el-table-column show-overflow-tooltip :prop="'work'" :label="$t('entities.ibztaskestimate.main_grid.columns.work')" :width="100" :align="'left'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
                ...
                <textarea class="ivu-input" :disabled="row.srfuf === 1 ? (3 & 2) !== 2 : (3 & 1) !== 1" v-model="row[column.property]" style=""  @change="($event)=>{gridEditItemChange(row, column.property, $event)}"></textarea>
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='work' editorType="TEXTAREA" :value="row.work"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof Main
     */
    protected appEntityService: IBZTaskEstimateService = new IBZTaskEstimateService({ $store: this.$store });
    ...
    */
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TaskEstimateGridView9视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TaskEstimateGridView9视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(this.removeAction,Object.assign(context,{ ibztaskestimate: keys.join(';') }),Object.assign({ id: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    protected rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.zt_taskestimate,$event.zt_taskestimate);
        });
    ...
    protected setColState() {
		const _data: any = localStorage.getItem('zt_taskestimate_main_grid');
		if (_data) {
    ...
    protected onColChange() {
        localStorage.setItem('zt_taskestimate_main_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'TaskEstimateGridView9视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'TaskEstimateGridView9视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.ibztaskestimate){
                    Object.assign(this.context,{ibztaskestimate:item.ibztaskestimate})
                }
    ...
    protected newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TaskEstimateGridView9视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    protected getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.ibztaskestimate,args.row.ibztaskestimate);
        });
