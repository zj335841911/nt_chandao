...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('name')">
                <el-table-column show-overflow-tooltip :prop="'name'" :label="$t('entities.todo.myupcoming_grid.columns.name')" :align="'left'">
                    <template v-slot="{row,column}">
...
export default class MyUpcomingBase extends Vue implements ControlInterface {
    ...
    */
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TodoGridView9视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TodoGridView9视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
    ...
    protected setColState() {
		const _data: any = localStorage.getItem('zt_todo_myupcoming_grid');
		if (_data) {
    ...
    protected onColChange() {
        localStorage.setItem('zt_todo_myupcoming_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'TodoGridView9视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'TodoGridView9视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.todo){
