...
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    ...
    <row>
        <i-col v-show="detailsModel.grouppanel2.visible" :style="{}"  :lg="{ span: 16, offset: 0 }" :xl="{ span: 16, offset: 0 }">
  ...
  deKeyField='product'
  :service="service"
  :acParams="{ serviceName: 'ProductService', interfaceName: 'FetchCurProject'}"
  valueitem='product' 
...
<i-col v-show="detailsModel.builder.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='builder' :itemRules="this.rules.builder" class='' :caption="$t('entities.build.quickcreate_form.details.builder')" uiStyle="DEFAULT" :labelWidth="100" :isShowCaption="true" :error="detailsModel.builder.error" :isEmptyCaption="false" labelPos="LEFT">
     <dropdown-list v-model="data.builder" :data="data" :itemParam="{}" :disabled="detailsModel.builder.disabled"  tag='UserRealName' codelistType='DYNAMIC' placeholder='请选择...' style=""></dropdown-list>
</app-form-item>
    ...
    */
    ...
    protected async wfsubmit(data: any): Promise<any> {
        ...
        if(!this.WFSubmitAction){
            this.$Notice.error({ title: '错误', desc: 'BuildEditView视图表单WFSubmitAction参数未配置' });
            return;
        }
        const _this: any = this;
        const arg: any = data[0];
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.build || Object.is(arg.build, '')) {
            return;
        }
