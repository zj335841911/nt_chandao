...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('id')">
                <el-table-column show-overflow-tooltip :prop="'id'" :label="$t('entities.build.main_grid.columns.id')" :width="80" :align="'left'" :sortable="'custom'">
            ...
            <template v-if="getColumnState('productname')">
                <el-table-column show-overflow-tooltip :prop="'productname'" :label="$t('entities.build.main_grid.columns.productname')" :width="200" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('name')">
                <el-table-column show-overflow-tooltip :prop="'name'" :label="$t('entities.build.main_grid.columns.name')" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            ...
            <template v-if="getColumnState('scmpath')">
                <el-table-column show-overflow-tooltip :prop="'scmpath'" :label="$t('entities.build.main_grid.columns.scmpath')" :width="300" :align="'center'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('filepath')">
                <el-table-column show-overflow-tooltip :prop="'filepath'" :label="$t('entities.build.main_grid.columns.filepath')" :width="300" :align="'center'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('date')">
                <el-table-column show-overflow-tooltip :prop="'date'" :label="$t('entities.build.main_grid.columns.date')" :width="150" :align="'center'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('builder')">
                <el-table-column show-overflow-tooltip :prop="'builder'" :label="$t('entities.build.main_grid.columns.builder')" :width="120" :align="'center'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            ...
            <template v-if="getColumnState('uagridcolumn1')">
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.build.main_grid.columns.uagridcolumn1')" :width="200" :align="'center'">
                    <template slot-scope="scope">
                        <span>
                            
                            <a @click="uiAction(scope.row, 'linkStories', $event)">{{$t('entities.build.main_grid.uiactions.linkstories')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'submitToTesting', $event)">{{$t('entities.build.main_grid.uiactions.submittotesting')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'viewBugs', $event)">{{$t('entities.build.main_grid.uiactions.viewbugs')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'editBuild', $event)">{{$t('entities.build.main_grid.uiactions.editbuild')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'Remove', $event)">{{$t('entities.build.main_grid.uiactions.remove')}}</a>
                        </span>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof Main
     */
    protected appEntityService: BuildService = new BuildService({ $store: this.$store });
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_uc9d0ae5_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_u1322db6_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_uae9d930_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_ua6d943e_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_u92e4430_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
    ...
    */
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'BuildMainGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'BuildMainGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(this.removeAction,Object.assign(context,{ build: keys.join(';') }),Object.assign({ id: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    protected rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.zt_build,$event.zt_build);
        });
    ...
    protected setColState() {
		const _data: any = localStorage.getItem('zt_build_main_grid');
		if (_data) {
    ...
    protected onColChange() {
        localStorage.setItem('zt_build_main_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'BuildMainGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'BuildMainGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.build){
                    Object.assign(this.context,{build:item.build})
                }
