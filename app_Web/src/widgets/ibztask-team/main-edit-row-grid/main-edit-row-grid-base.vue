...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('account')">
                <el-table-column show-overflow-tooltip :prop="'account'" :label="$t('entities.ibztaskteam.maineditrow_grid.columns.account')" :width="0" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
                                     <dropdown-list v-model="row[column.property]" :disabled="row.srfuf === 1 ? (3 & 2) !== 2 : (3 & 1) !== 1" tag='UserRealNameProject' codelistType='DYNAMIC' placeholder='请选择...' style="" @change="($event)=>{gridEditItemChange(row, column.property, $event)}"></dropdown-list>
                                </app-form-item>
                            </i-form>
                        </template>
                        <template v-if="!actualIsOpenEdit">
            <codelist :value="row.account" tag='UserRealNameProject' codelistType='DYNAMIC' renderMode="STR" valueSeparator="," textSeparator="," ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('estimate')">
                <el-table-column show-overflow-tooltip :prop="'estimate'" :label="$t('entities.ibztaskteam.maineditrow_grid.columns.estimate')" :width="300" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
            ...
            FTL stack trace ("~" means nesting-related):
            	- Failed at: #if item.getUnitName()??  [in template "TEMPLCODE_en_US" at line 4, column 3]
            ----
                                </app-form-item>
                            </i-form>
                        </template>
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='estimate' editorType="NUMBER" :value="row.estimate"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainEditRowBase extends Vue implements ControlInterface {
     ...
     * @memberof MainEditRow
     */
    protected service: MainEditRowService = new MainEditRowService({ $store: this.$store });
    ...
    */
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TaskTeamGridView9视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TaskTeamGridView9视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
    ...
    protected setColState() {
		const _data: any = localStorage.getItem('ibz_taskteam_maineditrow_grid');
		if (_data) {
    ...
    protected onColChange() {
        localStorage.setItem('ibz_taskteam_maineditrow_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'TaskTeamGridView9视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'TaskTeamGridView9视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.ibztaskteam){
    ...
    protected newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TaskTeamGridView9视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
...
<style lang='less'>
@import './main-edit-row-grid.less';
</style>