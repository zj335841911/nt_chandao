...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('expect')">
                <el-table-column show-overflow-tooltip :prop="'expect'" :label="$t('entities.ibzcasestep.main_rowedit_carryout_grid.columns.expect')" :align="'left'">
                    <template v-slot="{row,column}">
                        ...
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
                                     <dropdown-list v-model="row[column.property]" :disabled="row.srfuf === 1 ? (3 & 2) !== 2 : (3 & 1) !== 1" tag='Testresult__result' codelistType='STATIC' placeholder='请选择...' style="" @change="($event)=>{gridEditItemChange(row, column.property, $event)}"></dropdown-list>
                                </app-form-item>
                            </i-form>
                        </template>
                        <template v-if="!actualIsOpenEdit">
            ...
            <template v-if="getColumnState('reals')">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='reals' editorType="TEXTBOX" :value="row.reals"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class Main_RowEdit_CarryOutBase extends Vue implements ControlInterface {
     ...
     * @memberof Main_RowEdit_CarryOut
     */
    protected service: Main_RowEdit_CarryOutService = new Main_RowEdit_CarryOutService({ $store: this.$store });
     ...
     * @memberof Main_RowEdit_CarryOut
     */
    protected appEntityService: IBZCaseStepService = new IBZCaseStepService({ $store: this.$store });
    ...
    */
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'CaseStepCarryOutEditMode视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'CaseStepCarryOutEditMode视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(this.removeAction,Object.assign(context,{ ibzcasestep: keys.join(';') }),Object.assign({ id: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    protected setColState() {
		const _data: any = localStorage.getItem('zt_casestep_main_rowedit_carryout_grid');
		if (_data) {
    ...
    protected onColChange() {
        localStorage.setItem('zt_casestep_main_rowedit_carryout_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'CaseStepCarryOutEditMode视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'CaseStepCarryOutEditMode视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.ibzcasestep){
    ...
    protected newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'CaseStepCarryOutEditMode视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
...
<style lang='less'>
@import './main-row-edit-carry-out-grid.less';
</style>