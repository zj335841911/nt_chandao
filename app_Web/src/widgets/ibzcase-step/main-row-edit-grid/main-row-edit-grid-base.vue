...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('desc')">
                <el-table-column show-overflow-tooltip :prop="'desc'" :label="$t('entities.ibzcasestep.main_rowedit_grid.columns.desc')" :width="300" :align="'left'">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='desc' editorType="TEXTBOX" :value="row.desc"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('type')">
                <el-table-column show-overflow-tooltip :prop="'type'" :label="$t('entities.ibzcasestep.main_rowedit_grid.columns.type')" :width="100" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
                                     <dropdown-list v-model="row[column.property]" :disabled="row.srfuf === 1 ? (3 & 2) !== 2 : (3 & 1) !== 1" tag='Casestep__type' codelistType='STATIC' placeholder='请选择...' style="" @change="($event)=>{gridEditItemChange(row, column.property, $event)}"></dropdown-list>
                                </app-form-item>
                            </i-form>
                        </template>
                        <template v-if="!actualIsOpenEdit">
            ...
            <template v-if="getColumnState('expect')">
                <el-table-column show-overflow-tooltip :prop="'expect'" :label="$t('entities.ibzcasestep.main_rowedit_grid.columns.expect')" :align="'left'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='expect' editorType="TEXTBOX" :value="row.expect"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class Main_RowEditBase extends Vue implements ControlInterface {
     ...
     * @memberof Main_RowEdit
     */
    protected service: Main_RowEditService = new Main_RowEditService({ $store: this.$store });
