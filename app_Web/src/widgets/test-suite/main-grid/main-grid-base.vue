        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('name')">
                <el-table-column show-overflow-tooltip :prop="'name'" :label="$t('entities.testsuite.main_grid.columns.name')" :width="150" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <app-column-link deKeyField='testsuite' :context="JSON.parse(JSON.stringify(context))" :viewparams="JSON.parse(JSON.stringify(viewparams))" :data="row" :linkview="{viewname: 'test-suite-main-dashboard-view', height: 0,width: 0,title: $t('entities.testsuite.views.maindashboardview.title'),placement: 'DRAWER_TOP', isRedirectView: false,deResParameters: [
            ]
            ,parameters: [
            ...
            <template v-if="getColumnState('desc')">
                <el-table-column show-overflow-tooltip :prop="'desc'" :label="$t('entities.testsuite.main_grid.columns.desc')" :align="'left'">
                    <template v-slot="{row,column}">
                        <span>{{row.desc}}</span>
                    </template>
                </el-table-column>
            ...
            <template v-if="getColumnState('addedby')">
                <el-table-column show-overflow-tooltip :prop="'addedby'" :label="$t('entities.testsuite.main_grid.columns.addedby')" :width="100" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            <codelist :value="row.addedby" tag='UserRealName' codelistType='DYNAMIC' renderMode="STR" valueSeparator="," textSeparator="," ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            ...
            <template v-if="getColumnState('uagridcolumn1')">
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.testsuite.main_grid.columns.uagridcolumn1')" :width="100" :align="'center'">
                    <template slot-scope="scope">
                        <span>
                            
                            <a @click="uiAction(scope.row, 'Editsuite', $event)">{{$t('entities.testsuite.main_grid.uiactions.editsuite')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'Remove', $event)">{{$t('entities.testsuite.main_grid.uiactions.remove')}}</a>
                        </span>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_u880abe9_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_u8c051a2_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
    ...
    */
    ...
    protected async remove(datas: any[]): Promise<any> {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(this.removeAction,Object.assign(context,{ testsuite: keys.join(';') }),Object.assign({ id: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
