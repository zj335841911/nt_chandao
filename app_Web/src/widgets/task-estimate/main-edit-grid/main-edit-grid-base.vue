...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('id')">
                <el-table-column show-overflow-tooltip :prop="'id'" :label="$t('entities.taskestimate.mainedit_grid.columns.id')" :width="100" :align="'left'">
                    <template v-slot="{row,column}">
                        <template >
                                <app-span name='id' editorType="SPAN" :value="row.id"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            ...
            <template v-if="getColumnState('work')">
                <el-table-column show-overflow-tooltip :prop="'work'" :label="$t('entities.taskestimate.mainedit_grid.columns.work')" :align="'left'">
                    <template v-slot="{row,column}">
                        <span>{{row.work}}</span>
                    </template>
                </el-table-column>
            ...
            <template v-if="getColumnState('uagridcolumn1')">
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.taskestimate.mainedit_grid.columns.uagridcolumn1')" :width="100" :align="'center'">
                    <template slot-scope="scope">
                        <span>
                            
                            <a @click="uiAction(scope.row, 'Edit', $event)">{{$t('entities.taskestimate.mainedit_grid.uiactions.edit')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'Remove', $event)">{{$t('entities.taskestimate.mainedit_grid.uiactions.remove')}}</a>
                        </span>
                    </template>
                </el-table-column>
            </template>
...
export default class MainEditBase extends Vue implements ControlInterface {
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_ud0fe979_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_u1f5fe3c_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
    ...
    */
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TaskEstimateEditGridView9视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TaskEstimateEditGridView9视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(this.removeAction,Object.assign(context,{ taskestimate: keys.join(';') }),Object.assign({ id: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    protected setColState() {
		const _data: any = localStorage.getItem('zt_taskestimate_mainedit_grid');
		if (_data) {
    ...
    protected onColChange() {
        localStorage.setItem('zt_taskestimate_mainedit_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'TaskEstimateEditGridView9视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'TaskEstimateEditGridView9视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.taskestimate){
                    Object.assign(this.context,{taskestimate:item.taskestimate})
                }
    ...
    protected getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.taskestimate,args.row.taskestimate);
        });
