...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('name')">
                <el-table-column show-overflow-tooltip :prop="'name'" :label="$t('entities.testmodule.mainbranch_grid.columns.name')" :width="300" :align="'left'" :sortable="'custom'">
            ...
            <template v-if="getColumnState('branch')">
                <el-table-column show-overflow-tooltip :prop="'branch'" :label="$t('entities.testmodule.mainbranch_grid.columns.branch')" :width="120" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
            ...
            <template v-if="getColumnState('short')">
                <el-table-column show-overflow-tooltip :prop="'short'" :label="$t('entities.testmodule.mainbranch_grid.columns.short')" :width="100" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template v-if="actualIsOpenEdit">
                            <i-form style="height:100%;" :model="row">
                                <app-form-item :name="column.property" :itemRules="rules[column.property]">
            ...
            <template v-if="getColumnState('uagridcolumn1')">
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.testmodule.mainbranch_grid.columns.uagridcolumn1')" :width="50" :align="'center'">
                    <template slot-scope="scope">
                        <span>
                            
                            <a @click="uiAction(scope.row, 'Remove', $event)">{{$t('entities.testmodule.mainbranch_grid.uiactions.remove')}}</a>
                        </span>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBranchBase extends Vue implements ControlInterface {
    ...
    */
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TestModuleGridViewBranch视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TestModuleGridViewBranch视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
    ...
    protected setColState() {
		const _data: any = localStorage.getItem('ibz_testmodule_mainbranch_grid');
		if (_data) {
    ...
    protected onColChange() {
        localStorage.setItem('ibz_testmodule_mainbranch_grid', JSON.stringify(this.allColumns));
    }
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'TestModuleGridViewBranch视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'TestModuleGridViewBranch视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.testmodule){
    ...
    protected newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TestModuleGridViewBranch视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
