...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('pri')">
                <el-table-column show-overflow-tooltip :prop="'pri'" :label="$t('entities.bug.main_buildsub_grid.columns.pri')" :width="70" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            ...
            <template v-if="getColumnState('title')">
                <el-table-column show-overflow-tooltip :prop="'title'" :label="$t('entities.bug.main_buildsub_grid.columns.title')" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
            ...
            <template v-if="getColumnState('assignedto')">
                <el-table-column show-overflow-tooltip :prop="'assignedto'" :label="$t('entities.bug.main_buildsub_grid.columns.assignedto')" :width="100" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <template >
            <codelist :value="row.assignedto" tag='BugUserRealName' codelistType='DYNAMIC' renderMode="STR" valueSeparator="," textSeparator="," ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            ...
            <template v-if="getColumnState('uagridcolumn1')">
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.bug.main_buildsub_grid.columns.uagridcolumn1')" :width="200" :align="'center'">
                    <template slot-scope="scope">
                        <span>
                            
                            <a @click="uiAction(scope.row, 'ConfirmBug', $event)">{{$t('entities.bug.main_buildsub_grid.uiactions.confirmbug')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'ResolveBug', $event)">{{$t('entities.bug.main_buildsub_grid.uiactions.resolvebug')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'CloseBug', $event)">{{$t('entities.bug.main_buildsub_grid.uiactions.closebug')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'Copy', $event)">{{$t('entities.bug.main_buildsub_grid.uiactions.copy')}}</a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'MainEdit', $event)">{{$t('entities.bug.main_buildsub_grid.uiactions.mainedit')}}</a>
                        </span>
                    </template>
                </el-table-column>
            </template>
...
export default class Main_BuildSubBase extends Vue implements ControlInterface {
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_u5f3af70_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_u424511e_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_u13bca5a_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_udb5e3af_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
     ...
     * @memberof 
     */
    protected grid_uagridcolumn1_uea0faba_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
    ...
    */
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'BugBuildSubGridView_New_9212视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'BugBuildSubGridView_New_9212视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'BugBuildSubGridView_New_9212视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'BugBuildSubGridView_New_9212视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.bug){
