...
<div class='grid' style="height:100%;">
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('name')">
                <el-table-column show-overflow-tooltip :prop="'name'" :label="$t('entities.branch.platformmanagement_grid.columns.name')" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <span>{{row.name}}</span>
                    </template>
                </el-table-column>
            ...
            <template v-if="getColumnState('order')">
                <el-table-column show-overflow-tooltip :prop="'order'" :label="$t('entities.branch.platformmanagement_grid.columns.order')" :width="100" :align="'left'" :sortable="'custom'">
                    <template v-slot="{row,column}">
                        <span>{{row.order}}</span>
                    </template>
                </el-table-column>
...
export default class PlatformManagementBase extends Vue implements ControlInterface {
     ...
     * @memberof PlatformManagement
     */
    protected service: PlatformManagementService = new PlatformManagementService({ $store: this.$store });
     ...
     * @memberof PlatformManagement
     */
    protected appEntityService: BranchService = new BranchService({ $store: this.$store });
    ...
    */
    ...
    protected load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'BranchPMGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    protected async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'BranchPMGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(this.removeAction,Object.assign(context,{ branch: keys.join(';') }),Object.assign({ id: keys.join(';') },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    protected rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.zt_branch,$event.zt_branch);
        });
        ...
        _this.items.forEach((item:any)=>{
            if(!item.rowDataState){
                ...
                if(!this.createAction){
                    this.$Notice.error({ title: '错误', desc: 'BranchPMGridView视图表格createAction参数未配置' });
                    return;
                }
                ...
                if(!this.updateAction){
                    this.$Notice.error({ title: '错误', desc: 'BranchPMGridView视图表格updateAction参数未配置' });
                    return;
                }
                Object.assign(item,{viewparams:this.viewparams});
                if(item.branch){
                    Object.assign(this.context,{branch:item.branch})
                }
...
<style lang='less'>
@import './platform-management-grid.less';
</style>