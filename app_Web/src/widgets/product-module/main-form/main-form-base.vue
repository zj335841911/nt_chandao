...
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    ...
    <app-form-item name='name' :itemRules="this.rules.name" class='' :caption="$t('entities.productmodule.main_form.details.name')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.name.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.order.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='order' :itemRules="this.rules.order" class='' :caption="$t('entities.productmodule.main_form.details.order')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.order.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box 
  v-model="data.order"  
  @enter="onEnter($event)"  
  unit=""
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        order: [
            { type: 'number', message: 'order 值必须为数值类型', trigger: 'change' },
            { type: 'number', message: 'order 值必须为数值类型', trigger: 'blur' },
            { required: false, type: 'number', message: 'order 值不能为空', trigger: 'change' },
            { required: false, type: 'number', message: 'order 值不能为空', trigger: 'blur' },
        ],
        id: [
            { type: 'number', message: 'id 值必须为数值类型', trigger: 'change' },
            { type: 'number', message: 'id 值必须为数值类型', trigger: 'blur' },
...
, 
        order: new FormItemModel({ caption: 'order', detailType: 'FORMITEM', name: 'order', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'ProductModuleEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ProductModuleEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'ProductModuleEditView视图表单'+actionName+'参数未配置' });
            return;
        }
        ...
        post.then((response: any) => {
            ...
            this.sendAccMessage(data, 'IBZ_PRODUCTMODULE', isCreate ? 'create' : 'update');
        }).catch((response: any) => {
            if (response && response.status && response.data) {
                this.$Notice.error({ title: '错误', desc: response.data.message });
                return;
    ...
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'ProductModuleEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
            ...
            this.service.delete(_this.removeAction, JSON.parse(JSON.stringify(this.context)),arg, showResultInfo).then((response: any) => {
                if (response) {
                    ...
                    this.sendAccMessage(data, 'IBZ_PRODUCTMODULE', 'remove');
                    resolve(response);
                }
            }).catch((error: any) => {
