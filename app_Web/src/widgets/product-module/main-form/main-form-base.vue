...
<i-col v-show="detailsModel.branch.visible" :style="{}"  :lg="{ span: 8, offset: 0 }" :xl="{ span: 8, offset: 0 }">
    <app-form-item name='branch' :itemRules="this.rules.branch" class='' :caption="$t('entities.productmodule.main_form.details.branch')" uiStyle="DEFAULT" :labelWidth="0" :isShowCaption="false" :error="detailsModel.branch.error" :isEmptyCaption="false" labelPos="NONE">
     <dropdown-list v-model="data.branch" :data="data" :itemParam="{}" :disabled="detailsModel.branch.disabled"  tag='ProductBranch_Cache' codelistType='DYNAMIC' placeholder='请选择...' style=""></dropdown-list>
</app-form-item>
    ...
    */
    ...
    protected async wfsubmit(data: any): Promise<any> {
        ...
        if(!this.WFSubmitAction){
            this.$Notice.error({ title: '错误', desc: 'ProductModuleQuickCfgView视图表单WFSubmitAction参数未配置' });
            return;
        }
        const _this: any = this;
        const arg: any = data[0];
