...
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group layoutType="TABLE_24COL" titleStyle="" class='' uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.module.main_form.details.group1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false">
    
    <row>
        <i-col v-show="detailsModel.name.visible" :style="{}"  :sm="{ span: 12, offset: 0 }" :md="{ span: 12, offset: 0 }" :lg="{ span: 12, offset: 0 }" :xl="{ span: 12, offset: 0 }">
    <app-form-item name='name' :itemRules="this.rules.name" class='' :caption="$t('entities.module.main_form.details.name')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.name.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box v-model="data.name"  @enter="onEnter($event)"   unit=""  :disabled="detailsModel.name.disabled" type='text'  style=""></input-box>
</app-form-item>

</i-col>
<i-col v-show="detailsModel.short.visible" :style="{}"  :sm="{ span: 12, offset: 0 }" :md="{ span: 12, offset: 0 }" :lg="{ span: 12, offset: 0 }" :xl="{ span: 12, offset: 0 }">
    <app-form-item name='short' :itemRules="this.rules.short" class='' :caption="$t('entities.module.main_form.details.short')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.short.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box v-model="data.short"  @enter="onEnter($event)"   unit=""  :disabled="detailsModel.short.disabled" type='text'  style=""></input-box>
</app-form-item>

</i-col>
<i-col v-show="detailsModel.parentname.visible" :style="{}"  :sm="{ span: 12, offset: 0 }" :md="{ span: 12, offset: 0 }" :lg="{ span: 12, offset: 0 }" :xl="{ span: 12, offset: 0 }">
    <app-form-item name='parentname' :itemRules="this.rules.parentname" class='' :caption="$t('entities.module.main_form.details.parentname')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.parentname.error" :isEmptyCaption="false" labelPos="LEFT">
    <app-picker 
  :formState="formState"
  :data="data"
  :context="context"
...
<i-col v-show="detailsModel.order.visible" :style="{}"  :sm="{ span: 12, offset: 0 }" :md="{ span: 12, offset: 0 }" :lg="{ span: 12, offset: 0 }" :xl="{ span: 12, offset: 0 }">
    <app-form-item name='order' :itemRules="this.rules.order" class='' :caption="$t('entities.module.main_form.details.order')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.order.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box 
  v-model="data.order"  
  @enter="onEnter($event)"  
  unit=""
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof Main
     */
    protected appEntityService: ModuleService = new ModuleService({ $store: this.$store });
    ...
    protected detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: 'module基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, uiActionGroup: { caption: '', langbase: 'entities.module.main_form', extractMode: 'ITEM', details: [] } })
, 
...
, 
        srfmajortext: new FormItemModel({ caption: '模块名称', detailType: 'FORMITEM', name: 'srfmajortext', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        name: new FormItemModel({ caption: '模块名称', detailType: 'FORMITEM', name: 'name', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        order: new FormItemModel({ caption: '排序', detailType: 'FORMITEM', name: 'order', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
     ...
     */
    @Watch('data.short')
    onShortChange(newVal: any, oldVal: any) {
     ...
     */
    @Watch('data.order')
    onOrderChange(newVal: any, oldVal: any) {
    ...
    private onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.module){
            Object.assign(this.context,{module:data.module})
        }
    ...
    private load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'ModuleEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    protected loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ModuleEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    protected autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'ModuleEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    */
    private remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'ModuleEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    protected async wfsubmit(data: any): Promise<any> {
        ...
        if(!this.WFSubmitAction){
            this.$Notice.error({ title: '错误', desc: 'ModuleEditView视图表单WFSubmitAction参数未配置' });
            return;
        }
        const _this: any = this;
        const arg: any = data[0];
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.module || Object.is(arg.module, '')) {
            return;
        }
