...
export class FormTabPageModel extends FormDetailModel {
  ...
  public getTabPanelModel(): FormTabPanelModel | null {
    ...
    const tabPanels: any[] = Object.values(
      this.form.detailsModel
    ).filter((model: any) => Object.is(model.detailType, "TABPANEL"));
    let index = tabPanels.findIndex((tabPanel: any) => {
      return tabPanel.tabPages.some((tabPag: any) =>
        Object.is(tabPag.name, this.name)
      );
    });
    if (index === -1) {
      return null;
