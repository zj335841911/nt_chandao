...
// GetUserConcat
mock.onPut(new RegExp(/^\/products\/([a-zA-Z0-9\-\;]{1,35})\/stories\/([a-zA-Z0-9\-\;]{1,35})\/getuserconcat$/)).reply((config: any) => {
...
// GetUserConcat
mock.onPut(new RegExp(/^\/stories\/?([a-zA-Z0-9\-\;]{0,35})\/getuserconcat$/)).reply((config: any) => {
    console.groupCollapsed("实体:story 方法: GetUserConcat");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
