...
// UpdateProjectCycle
mock.onPut(new RegExp(/^\/projects\/?([a-zA-Z0-9\-\;]{0,35})\/updateprojectcycle$/)).reply((config: any) => {
    ...
    const paramArray:Array<any> = ['id'];
    const matchArray:any = new RegExp(/^\/projects\/([a-zA-Z0-9\-\;]{1,35})\/updateprojectcycle$/).exec(config.url);
    let tempValue: any = {};
    if(matchArray && matchArray.length >1 && paramArray && paramArray.length >0){
        paramArray.forEach((item: any, index: number) => {
            Object.defineProperty(tempValue, item, {
