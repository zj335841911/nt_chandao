...
mock.onGet(new RegExp(/^\/products\/([a-zA-Z0-9\-\;]{1,35})\/testreports\/([a-zA-Z0-9\-\;]{1,35})\/gettestreportbasicinfo$/)).reply((config: any) => {
...
// GetTestReportBasicInfo
mock.onGet(new RegExp(/^\/testreports\/([a-zA-Z0-9\-\;]{1,35})\/gettestreportbasicinfo$/)).reply((config: any) => {
    console.groupCollapsed("实体:testreport 方法: GetTestReportBasicInfo");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
