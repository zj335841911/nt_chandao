...
export default class ViewMessageService {
   ...
   * @memberof ViewMessageService
   */
  public async getViewMessageByTag(
    tag: string,
    context: any = {},
    viewparam: any = {}
  ) {
    ...
    if (
      messageService.dynamicMode &&
      Object.is(messageService.dynamicMode, "STATIC")
    ) {
      return messageService.getStaticViewMessage(context, viewparam);
    } else {
      return messageService.getDynamicViewMessage(
        tag,
        messageService,
        context,
        viewparam
    ...
    return new Promise((resolve: any, reject: any) => {
      ...
      if (isEnableCache) {
        const callback: Function = (
          ...
          const callbackKey: string = `${JSON.stringify(
            context
          )}-${JSON.stringify(data)}-${tag}`;
          promise
            .then((result: any) => {
              if (result.length > 0) {
                ViewMessageService.messageCached.set(callbackKey, {
                  items: result
                });
        ...
        const key: string = `${JSON.stringify(context)}-${JSON.stringify(
          data
        )}-${tag}`;
        // 加载完成,从本地缓存获取
        ...
        if (messageService) {
          ...
          if (ViewMessageService.messageCache.get(key)) {
            callback(
              ...
              data,
              tag,
              ViewMessageService.messageCache.get(key)
            );
          } else {
            let result: Promise<any> = messageService.getItems(
              context,
              data,
              isloading
            );
            ViewMessageService.messageCache.set(key, result);
            messageService.setExpirationTime(
