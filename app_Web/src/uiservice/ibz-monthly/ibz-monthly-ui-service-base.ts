...
export default class IbzMonthlyUIServiceBase extends UIService {
     ...
     * @returns {Promise<any>}
     */
    public async IbzMonthly_CreateUserMonthly(args: any[],context:any = {}, params:any = {}, $event?: any, xData?: any,actionContext?: any,srfParentDeName?:string){
        let data: any = {};
        let parentContext:any = {};
        let parentViewParam:any = {};
        const _this: any = actionContext;
        Object.assign(context,{IBZMONTHLY:"0"});
        Object.assign(params,{ibzmonthly:"0"});
        const _args: any[] = Util.deepCopy(args);
        ...
        const backend = () => {
            const curService:IbzMonthlyService =  new IbzMonthlyService();
            curService.CreateUserMonthly(context,data, true).then((response: any) => {
                if (!response || response.status !== 200) {
                    actionContext.$Notice.error({ title: '错误', desc: response.message });
                    return;
                }
