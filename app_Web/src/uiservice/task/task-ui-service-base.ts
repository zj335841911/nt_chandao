...
export default class TaskUIServiceBase extends UIService {
     ...
     * @returns {Promise<any>}
     */
    public async Task_RecordEstimate(args: any[],context:any = {}, params:any = {}, $event?: any, xData?: any,actionContext?: any,srfParentDeName?:string){
        let data: any = {};
        let parentContext:any = {};
        let parentViewParam:any = {};
        const _this: any = actionContext;
        ...
        const backend = () => {
            const curService:TaskService =  new TaskService();
            curService.RecordEstimate(context,data, true).then((response: any) => {
                if (!response || response.status !== 200) {
                    actionContext.$Notice.error({ title: '错误', desc: response.message });
                    return;
                }
