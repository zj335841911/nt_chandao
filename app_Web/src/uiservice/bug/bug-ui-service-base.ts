...
export default class BugUIServiceBase extends UIService {
     ...
     * @returns {Promise<any>}
     */
    public async Bug_BatchUnlinkBug(args: any[],context:any = {}, params:any = {}, $event?: any, xData?: any,actionContext?: any,srfParentDeName?:string){
        let confirmResult:boolean = await new Promise((resolve: any, reject: any) => {
          actionContext.$Modal.confirm({
              title: '警告',
              content: '是否移除关联Bug',
        ...
        const backend = () => {
            const curService:BugService =  new BugService();
            curService.BatchUnlinkBug(context,data, true).then((response: any) => {
                if (!response || response.status !== 200) {
                    actionContext.$Notice.error({ title: '错误', desc: response.message });
                    return;
                }
