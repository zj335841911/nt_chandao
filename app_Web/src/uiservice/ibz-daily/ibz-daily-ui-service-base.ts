...
export default class IbzDailyUIServiceBase extends UIService {
     ...
     * @returns {Promise<any>}
     */
    public async IbzDaily_LinkCompleteTask(args: any[],context:any = {}, params:any = {}, $event?: any, xData?: any,actionContext?: any,srfParentDeName?:string){
        let data: any = {};
        ...
        const backend = () => {
            ...
            curService.LinkCompleteTask(context,data, true).then((response: any) => {
                if (!response || response.status !== 200) {
                    actionContext.$Notice.error({ title: '错误', desc: response.message });
                    return;
                }
                actionContext.$Notice.success({ title: '成功', desc: '完成任务成功！' });
                const _this: any = actionContext;
        ...
        const view = { 
            viewname: 'taskreport-mpickup-view', 
            title: actionContext.$t('entities.task.views.reportmpickupview.title'),
            height: 0, 
            width: 0, 
        };
