...
export class Verify {
  ...
  public static testCond(value: any, op: any, value2: any): boolean {
    ...
    if (Object.is(op, "LEFTLIKE")) {
      return (
        value &&
        value2 &&
        value.toUpperCase().indexOf(value2.toUpperCase()) === 0
      );
    }
    // 文本包含
    if (Object.is(op, "LIKE")) {
      return (
        value &&
        value2 &&
        value.toUpperCase().indexOf(value2.toUpperCase()) !== -1
      );
    }
    // 小于操作
    if (Object.is(op, "LT")) {
  ...
  public static checkFieldValueRangeRule(
    ...
    const valueFormat = this.checkFieldRegExRule(
      viewValue,
      /^-?\d*\.?\d+$/,
      "",
      primaryModel
