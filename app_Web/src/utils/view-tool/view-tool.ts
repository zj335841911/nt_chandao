...
export class ViewTool {
      ...
      ({
        ...
        if (
          ...
          Object.assign(viewdata, {
            [parameterName]: viewParam[parameterName]
          });
  ...
  public static buildUpRoutePath(
    ...
    const deResRoutePath = this.getDeResRoutePath(
      viewParam,
      deResParameters,
      args
    );
    const deRoutePath = this.getActiveRoutePath(
      parameters,
      args,
      data,
      viewParam
   ...
   * @memberof ViewTool
   */
  public static getDeResRoutePath(
    viewParam: any = {},
    deResParameters: any[],
    args: any[]
  ): string {
    ...
    if (deResParameters && deResParameters.length > 0) {
      deResParameters.forEach(
        ({
          ...
          routePath =
            `${routePath}/${pathName}` +
            (isExistAndNotEmpty(value) ? `/${value}` : "");
        }
      );
    }
    return routePath;
   ...
   * @memberof ViewTool
   */
  public static getActiveRoutePath(
    parameters: any[],
    args: any[],
    data: any,
    viewParam: any = {}
    ...
    if (parameters && parameters.length > 0) {
      if (parameters.length === 1) {
        ...
        const _value: any =
          arg[_parameterName] || viewParam[_parameterName] || null;
        routePath = `/${_pathName}${
          isExistAndNotEmpty(_value) ? `/${_value}` : ""
        }/${_pathName2}`;
        if (Object.keys(data).length > 0) {
          routePath = `${routePath}?${qs.stringify(data, { delimiter: ";" })}`;
        }
   ...
   * @memberof ViewTool
   */
  public static formatRouteParams(
    params: any,
    route: any,
    context: any,
    viewparams: any
    ...
    if (route && route.fullPath && route.fullPath.indexOf("?") > -1) {
      const _viewparams: any = route.fullPath.slice(
        route.fullPath.indexOf("?") + 1
      );
      const _viewparamArray: Array<string> = decodeURIComponent(
        _viewparams
      ).split(";");
   ...
   * @memberof ViewTool
   */
  public static calcActionItemAuthState(
    data: any,
    ActionModel: any,
    UIService: any
  ): any[] {
