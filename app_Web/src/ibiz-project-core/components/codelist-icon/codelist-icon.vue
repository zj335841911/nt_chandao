    ...
    <div class="codelist">
        ...
        <template v-if="!ifEmpty">
          <template v-for="(item, index) in items">
              <span :key="index">{{ index != 0 ? textSeparator : ''}}</span>
              <div :key="index" v-if="item.iconCls">
                  <i :class="item.iconCls" :style='{color: item.color}'></i>
              </div>
              <div v-else :key="index" class="icon-border" :style="{ borderColor:item.color, color:item.color }">
                <span :class="item.textCls">{{isUseLangres ? $t(item.text) : item.text}}</span>
              </div>
          </template>
        </template>
    </div>
...
@Component({})
export default class CodeListIcon extends Vue {
    /**
     * 代码表服务对象
     *
     * @type {CodeListService}
     * @memberof CodeListIcon
     */  
