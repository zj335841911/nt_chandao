...
import cn.ibizlab.pms.util.helper.RuleUtils;
...
public class VersionCheckAspect
{
    ...
    @SneakyThrows
    @Before("execution(* cn.ibizlab.pms.*.rest.*.update(..)) &&  @annotation(versionCheck)")
    ...
    @SneakyThrows
    @Before("execution(* cn.ibizlab.pms.*.rest.*.updateBy*(..)) &&  @annotation(versionCheck)")
