...
| 1 | [虚拟实体（VIRTUALFLAG）](#属性-虚拟实体（VIRTUALFLAG）) | `=` |
| 1 | [访问控制体系（ACCCTRLARCH）](#属性-访问控制体系（ACCCTRLARCH）) | `=` |
| 1 | [审计模式（AUDITMODE）](#属性-审计模式（AUDITMODE）) | `=` |
| 1 | [预置业务功能模式（BIZTAG）](#属性-预置业务功能模式（BIZTAG）) | `=` |
| 1 | [数据访问控制方式（DATAACCMODE）](#属性-数据访问控制方式（DATAACCMODE）) | `=` |
| 1 | [数据导入导出能力（DATAIMPEXPFLAG）](#属性-数据导入导出能力（DATAIMPEXPFLAG）) | `=` |
| 1 | [数据变更日志类型（DATACHGLOGMODE）](#属性-数据变更日志类型（DATACHGLOGMODE）) | `=` |
| 1 | [数据库表空间（DBTABSPACE）](#属性-数据库表空间（DBTABSPACE）) | `=` |
| 1 | [实体类别（DECAT）](#属性-实体类别（DECAT）) | `=` |
| 1 | [实体类型（DETYPE）](#属性-实体类型（DETYPE）) | `=` |
| 1 | [扩展模式（DYNAMICMODE）](#属性-扩展模式（DYNAMICMODE）) | `=` |
| 1 | [默认数据源（DSLINK）](#属性-默认数据源（DSLINK）) | `=` |
| 1 | [动态模型类型（DYNAMODELFLAG）](#属性-动态模型类型（DYNAMODELFLAG）) | `=` |
| 1 | [启用临时数据（ENATEMPDATA）](#属性-启用临时数据（ENATEMPDATA）) | `=` |
| 1 | [模型锁模式（LOCKFLAG）](#属性-模型锁模式（LOCKFLAG）) | `=` |
| 1 | [索引类型（INDEXDETYPE）](#属性-索引类型（INDEXDETYPE）) | `=` |
| 1 | [模型导入导出能力（MODELIMPEXPFLAG）](#属性-模型导入导出能力（MODELIMPEXPFLAG）) | `=` |
| 1 | [SaaS构型（SAASMODE）](#属性-SaaS构型（SAASMODE）) | `=` |
| 1 | [删除模式（REMOVEFLAG）](#属性-删除模式（REMOVEFLAG）) | `=` |
| 1 | [服务API模式（SERVICEAPIFLAG）](#属性-服务API模式（SERVICEAPIFLAG）) | `=` |
| 1 | [存储模式（STORAGEMODE）](#属性-存储模式（STORAGEMODE）) | `=` |
| 1 | [用户分类（USERCAT）](#属性-用户分类（USERCAT）) | `=` |
| 1 | [多视图级别（VIEWLEVEL）](#属性-多视图级别（VIEWLEVEL）) | `=` |
...
| 1 | [子系统接口实体（PSSUBSYSSADEID）](#属性-子系统接口实体（PSSUBSYSSADEID）) | `=` |
| 1 | [子系统接口实体（PSSUBSYSSADENAME）](#属性-子系统接口实体（PSSUBSYSSADENAME）) | `=` |
| 2 | [子系统接口实体（PSSUBSYSSADENAME）](#属性-子系统接口实体（PSSUBSYSSADENAME）) | `%like%` |
| 1 | [子系统服务接口（PSSUBSYSSERVICEAPIID）](#属性-子系统服务接口（PSSUBSYSSERVICEAPIID）) | `=` |
| 1 | [子系统服务接口（PSSUBSYSSERVICEAPINAME）](#属性-子系统服务接口（PSSUBSYSSERVICEAPINAME）) | `=` |
| 2 | [子系统服务接口（PSSUBSYSSERVICEAPINAME）](#属性-子系统服务接口（PSSUBSYSSERVICEAPINAME）) | `%like%` |
| 1 | [系统设计需求（PSSYSREQITEMNAME）](#属性-系统设计需求（PSSYSREQITEMNAME）) | `=` |
| 2 | [系统设计需求（PSSYSREQITEMNAME）](#属性-系统设计需求（PSSYSREQITEMNAME）) | `%like%` |
| 1 | [系统设计需求（PSSYSREQITEMID）](#属性-系统设计需求（PSSYSREQITEMID）) | `=` |
