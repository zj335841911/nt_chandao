# 实体-项目(ZT_PROJECT)
## 实体说明
项目

## 所属模块
[禅道模块](../zentao)

## 实体属性
| 序号 | 属性 | 属性名 | 数据类型 | 主键 | 外键 | 允许为空 |
| -- | -- | -- | -- | -- | -- | -- |
| 1 | [当前系统版本](#属性-当前系统版本（OPENEDVERSION）) | OPENEDVERSION | TEXT | 否 | 否 | 是 |
| 2 | [开始时间](#属性-开始时间（BEGIN）) | BEGIN | DATE | 否 | 否 | 否 |
| 3 | [访问控制](#属性-访问控制（ACL）) | ACL | SSCODELIST | 否 | 否 | 是 |
| 4 | [已删除](#属性-已删除（DELETED）) | DELETED | TEXT | 否 | 否 | 是 |
| 5 | [项目描述](#属性-项目描述（DESC）) | DESC | LONGTEXT | 否 | 否 | 是 |
| 6 | [项目负责人](#属性-项目负责人（PM）) | PM | SSCODELIST | 否 | 否 | 是 |
| 7 | [项目编号](#属性-项目编号（ID）) | ID | ACID | 是 | 否 | 否 |
| 8 | [项目名称](#属性-项目名称（NAME）) | NAME | TEXT | 否 | 否 | 否 |
| 9 | [子状态](#属性-子状态（SUBSTATUS）) | SUBSTATUS | TEXT | 否 | 否 | 是 |
| 10 | [项目排序](#属性-项目排序（ORDER）) | ORDER | INT | 否 | 否 | 是 |
| 11 | [发布负责人](#属性-发布负责人（RD）) | RD | SSCODELIST | 否 | 否 | 是 |
| 12 | [分组白名单](#属性-分组白名单（WHITELIST）) | WHITELIST | LONGTEXT | 否 | 否 | 是 |
| 13 | [优先级](#属性-优先级（PRI）) | PRI | SSCODELIST | 否 | 否 | 是 |
| 14 | [结束日期](#属性-结束日期（END）) | END | DATE | 否 | 否 | 否 |
| 15 | [取消日期](#属性-取消日期（CANCELEDDATE）) | CANCELEDDATE | DATETIME | 否 | 否 | 是 |
| 16 | [项目代号](#属性-项目代号（CODE）) | CODE | TEXT | 否 | 否 | 是 |
| 17 | [catID](#属性-catID（CATID）) | CATID | INT | 否 | 否 | 是 |
| 18 | [statge](#属性-statge（STATGE）) | STATGE | SSCODELIST | 否 | 否 | 是 |
| 19 | [由谁取消](#属性-由谁取消（CANCELEDBY）) | CANCELEDBY | TEXT | 否 | 否 | 是 |
| 20 | [isCat](#属性-isCat（ISCAT）) | ISCAT | SSCODELIST | 否 | 否 | 是 |
| 21 | [创建日期](#属性-创建日期（OPENEDDATE）) | OPENEDDATE | DATETIME | 否 | 否 | 是 |
| 22 | [由谁关闭](#属性-由谁关闭（CLOSEDBY）) | CLOSEDBY | TEXT | 否 | 否 | 是 |
| 23 | [项目类型](#属性-项目类型（TYPE）) | TYPE | SSCODELIST | 否 | 否 | 是 |
| 24 | [产品负责人](#属性-产品负责人（PO）) | PO | SSCODELIST | 否 | 否 | 是 |
| 25 | [项目状态](#属性-项目状态（STATUS）) | STATUS | SSCODELIST | 否 | 否 | 是 |
| 26 | [可用工作日](#属性-可用工作日（DAYS）) | DAYS | INT | 否 | 否 | 是 |
| 27 | [团队名称](#属性-团队名称（TEAM）) | TEAM | TEXT | 否 | 否 | 是 |
| 28 | [关闭日期](#属性-关闭日期（CLOSEDDATE）) | CLOSEDDATE | DATETIME | 否 | 否 | 是 |
| 29 | [由谁创建](#属性-由谁创建（OPENEDBY）) | OPENEDBY | TEXT | 否 | 否 | 是 |
| 30 | [测试负责人](#属性-测试负责人（QD）) | QD | SSCODELIST | 否 | 否 | 是 |
| 31 | [parent](#属性-parent（PARENTNAME）) | PARENTNAME | PICKUPTEXT | 否 | 是 | 是 |
| 32 | [父项目](#属性-父项目（PARENT）) | PARENT | PICKUP | 否 | 是 | 是 |
| 33 | [任务总数](#属性-任务总数（TASKCNT）) | TASKCNT | INT | 否 | 是 | 是 |
| 34 | [Bug总数](#属性-Bug总数（BUGCNT）) | BUGCNT | INT | 否 | 是 | 是 |
| 35 | [需求总数](#属性-需求总数（STORYCNT）) | STORYCNT | INT | 否 | 是 | 是 |
| 36 | [关联产品](#属性-关联产品（PRODUCTS）) | PRODUCTS | TEXT | 否 | 是 | 是 |
| 37 | [关联产品平台集合](#属性-关联产品平台集合（BRANCHS）) | BRANCHS | TEXT | 否 | 是 | 是 |
| 38 | [关联计划](#属性-关联计划（PLANS）) | PLANS | TEXT | 否 | 是 | 是 |
| 39 | [关联数据数组](#属性-关联数据数组（SRFARRAY）) | SRFARRAY | TEXT | 否 | 是 | 是 |
| 40 | [备注](#属性-备注（COMMENT）) | COMMENT | HTMLTEXT | 否 | 是 | 是 |
| 41 | [时间段](#属性-时间段（PERIOD）) | PERIOD | SSCODELIST | 否 | 是 | 是 |
| 42 | [项目团队成员](#属性-项目团队成员（ACCOUNT）) | ACCOUNT | SSCODELIST | 否 | 是 | 是 |
| 43 | [加盟日](#属性-加盟日（JOIN）) | JOIN | DATE | 否 | 是 | 是 |
| 44 | [可用工时/天](#属性-可用工时/天（HOURS）) | HOURS | DECIMAL | 否 | 是 | 是 |
| 45 | [角色](#属性-角色（ROLE）) | ROLE | TEXT | 否 | 是 | 是 |
| 46 | [任务消耗总工时](#属性-任务消耗总工时（TOTALCONSUMED）) | TOTALCONSUMED | FLOAT | 否 | 是 | 是 |
| 47 | [总工时](#属性-总工时（TOTALWH）) | TOTALWH | INT | 否 | 是 | 是 |
| 48 | [任务预计剩余总工时](#属性-任务预计剩余总工时（TOTALLEFT）) | TOTALLEFT | FLOAT | 否 | 是 | 是 |
| 49 | [任务最初预计总工时](#属性-任务最初预计总工时（TOTALESTIMATE）) | TOTALESTIMATE | FLOAT | 否 | 是 | 是 |
| 50 | [可用工时](#属性-可用工时（TOTALHOURS）) | TOTALHOURS | DECIMAL | 否 | 是 | 是 |
| 51 | [移动端图片](#属性-移动端图片（MOBIMAGE）) | MOBIMAGE | TEXT | 否 | 是 | 是 |
| 52 | [项目团队相关成员](#属性-项目团队相关成员（ACCOUNTS）) | ACCOUNTS | TEXT | 否 | 是 | 是 |
| 53 | [项目排序](#属性-项目排序（ORDER1）) | ORDER1 | INT | 否 | 是 | 是 |
| 54 | [是否置顶](#属性-是否置顶（ISTOP）) | ISTOP | INT | 否 | 是 | 是 |
| 55 | [选择部门](#属性-选择部门（DEPT）) | DEPT | SSCODELIST | 否 | 是 | 是 |
| 56 | [复制团队](#属性-复制团队（MANAGEMEMBERS）) | MANAGEMEMBERS | SSCODELIST | 否 | 是 | 是 |
| 57 | [版本总数](#属性-版本总数（BUILDCNT）) | BUILDCNT | INT | 否 | 是 | 是 |
| 58 | [团队成员总数](#属性-团队成员总数（TEAMCNT）) | TEAMCNT | INT | 否 | 是 | 是 |
| 59 | [所有任务数](#属性-所有任务数（ALLTASKCNT）) | ALLTASKCNT | INT | 否 | 是 | 是 |
| 60 | [未关闭任务数](#属性-未关闭任务数（UNCLOSETASKCNT）) | UNCLOSETASKCNT | INT | 否 | 是 | 是 |
| 61 | [指派给我任务数](#属性-指派给我任务数（ASSTOMYTASKCNT）) | ASSTOMYTASKCNT | INT | 否 | 是 | 是 |
| 62 | [未开始任务数](#属性-未开始任务数（UNSTARTTASKCNT）) | UNSTARTTASKCNT | INT | 否 | 是 | 是 |
| 63 | [更多任务数](#属性-更多任务数（MORETASKCNT）) | MORETASKCNT | INT | 否 | 是 | 是 |
| 64 | [进行中任务数](#属性-进行中任务数（YSTARTASKCNT）) | YSTARTASKCNT | INT | 否 | 是 | 是 |
| 65 | [未完成任务数](#属性-未完成任务数（UNCOMPLETETASKCNT）) | UNCOMPLETETASKCNT | INT | 否 | 是 | 是 |
| 66 | [已完成任务数](#属性-已完成任务数（YCOMPLETETASKCNT）) | YCOMPLETETASKCNT | INT | 否 | 是 | 是 |
| 67 | [我完成任务数](#属性-我完成任务数（MYCOMPLETETASKCNT）) | MYCOMPLETETASKCNT | INT | 否 | 是 | 是 |
| 68 | [关闭任务数](#属性-关闭任务数（CLOSETASKCNT）) | CLOSETASKCNT | INT | 否 | 是 | 是 |
| 69 | [取消任务数](#属性-取消任务数（CANCELTASKCNT）) | CANCELTASKCNT | INT | 否 | 是 | 是 |
| 70 | [需求变更数](#属性-需求变更数（STORYCHANGECNT）) | STORYCHANGECNT | INT | 否 | 是 | 是 |

### 属性-当前系统版本（OPENEDVERSION）
#### 属性说明
当前系统版本

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
文本，可指定长度

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-开始时间（BEGIN）
#### 属性说明
开始时间

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
日期型

#### Java类型
Timestamp

#### 是否允许为为空
否

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
时间格式：yyyy-MM-dd

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-访问控制（ACL）
#### 属性说明
访问控制

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
单项选择(文本值)

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 | open |

#### 取值范围/公式
参照数据字典【[项目访问控制（Project__acl）](../../codelist/Project__acl)】

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-已删除（DELETED）
#### 属性说明
已删除

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
文本，可指定长度

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 | 0 |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-项目描述（DESC）
#### 属性说明
项目描述

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
长文本，没有长度限制

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-项目负责人（PM）
#### 属性说明
项目负责人

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
单项选择(文本值)

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
参照数据字典【[用户真实名称（动态）（UserRealName）](../../codelist/UserRealName)】

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-项目编号（ID）
#### 属性说明
项目编号

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
自增标识，整数类型，用户不可见

#### Java类型
Long

#### 是否允许为为空
否

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-项目名称（NAME）
#### 属性说明
项目名称

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
文本，可指定长度

#### Java类型
String

#### 是否允许为为空
否

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-子状态（SUBSTATUS）
#### 属性说明
子状态

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
文本，可指定长度

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-项目排序（ORDER）
#### 属性说明
项目排序

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 | 0 |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-发布负责人（RD）
#### 属性说明
发布负责人

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
单项选择(文本值)

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
参照数据字典【[用户真实名称（动态）（UserRealName）](../../codelist/UserRealName)】

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-分组白名单（WHITELIST）
#### 属性说明
分组白名单

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
长文本，没有长度限制

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-优先级（PRI）
#### 属性说明
优先级

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
单项选择(文本值)

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 | 1 |

#### 取值范围/公式
参照数据字典【[项目优先级（Project__pri）](../../codelist/Project__pri)】

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-结束日期（END）
#### 属性说明
结束日期

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
日期型

#### Java类型
Timestamp

#### 是否允许为为空
否

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
时间格式：yyyy-MM-dd

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-取消日期（CANCELEDDATE）
#### 属性说明
取消日期

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
日期时间型

#### Java类型
Timestamp

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
时间格式：yyyy-MM-dd HH:mm:ss

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-项目代号（CODE）
#### 属性说明
项目代号

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
文本，可指定长度

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-catID（CATID）
#### 属性说明
catID

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 | 0 |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-statge（STATGE）
#### 属性说明
statge

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
单项选择(文本值)

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 | 1 |

#### 取值范围/公式
参照数据字典【[项目阶段（Project__statge）](../../codelist/Project__statge)】

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-由谁取消（CANCELEDBY）
#### 属性说明
由谁取消

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
文本，可指定长度

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-isCat（ISCAT）
#### 属性说明
暂时还不知道做什么用的，代码也没搜到相关含义以及处理（除了赋值默认值）

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
单项选择(文本值)

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 | 0 |

#### 取值范围/公式
参照数据字典【[是否（YesNo）](../../codelist/YesNo)】

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-创建日期（OPENEDDATE）
#### 属性说明
创建日期

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
日期时间型

#### Java类型
Timestamp

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
时间格式：yyyy-MM-dd HH:mm:ss

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-由谁关闭（CLOSEDBY）
#### 属性说明
由谁关闭

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
文本，可指定长度

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-项目类型（TYPE）
#### 属性说明
项目类型

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
单项选择(文本值)

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 | sprint |

#### 取值范围/公式
参照数据字典【[项目类型（Project__type）](../../codelist/Project__type)】

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-产品负责人（PO）
#### 属性说明
产品负责人

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
单项选择(文本值)

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
参照数据字典【[用户真实名称（动态）（UserRealName）](../../codelist/UserRealName)】

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-项目状态（STATUS）
#### 属性说明
项目状态

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
单项选择(文本值)

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 | wait |

#### 取值范围/公式
参照数据字典【[项目状态（Project__status）](../../codelist/Project__status)】

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-可用工作日（DAYS）
#### 属性说明
可用工作日

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 | 0 |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-团队名称（TEAM）
#### 属性说明
不是外键文本，仅仅是团队名称

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
文本，可指定长度

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-关闭日期（CLOSEDDATE）
#### 属性说明
关闭日期

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
日期时间型

#### Java类型
Timestamp

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
时间格式：yyyy-MM-dd HH:mm:ss

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-由谁创建（OPENEDBY）
#### 属性说明
由谁创建

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
文本，可指定长度

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-测试负责人（QD）
#### 属性说明
测试负责人

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
单项选择(文本值)

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
参照数据字典【[用户真实名称（动态）（UserRealName）](../../codelist/UserRealName)】

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-parent（PARENTNAME）
#### 属性说明
parent

#### 属性类型
链接字段[来自关系实体字段]

#### 数据类型
外键值文本

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目名称（NAME）](../zentao/Project/#属性-项目名称（NAME）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-父项目（PARENT）
#### 属性说明
父项目

#### 属性类型
物理字段[来自当前实体物理表字段]

#### 数据类型
外键值

#### Java类型
Long

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 | 0 |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-任务总数（TASKCNT）
#### 属性说明
任务总数

#### 属性类型
逻辑字段[来自计算式]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
(SELECT COUNT(1) FROM ZT_TASK WHERE PROJECT = %1$s AND DELETED = '0')

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-Bug总数（BUGCNT）
#### 属性说明
Bug总数

#### 属性类型
逻辑字段[来自计算式]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
(SELECT COUNT(1) FROM ZT_BUG WHERE PROJECT = %1$s AND DELETED = '0')

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-需求总数（STORYCNT）
#### 属性说明
需求总数

#### 属性类型
逻辑字段[来自计算式]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
(SELECT COUNT(1) FROM ZT_STORY LEFT JOIN ZT_PROJECTSTORY ON ZT_STORY.ID = ZT_PROJECTSTORY.STORY WHERE PROJECT = %1$s AND DELETED = '0')

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-关联产品（PRODUCTS）
#### 属性说明
关联产品

#### 属性类型
应用界面字段[无存储]

#### 数据类型
文本，可指定长度

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
参照数据字典【[产品（动态）（Product）](../../codelist/Product)】

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-关联产品平台集合（BRANCHS）
#### 属性说明
关联产品平台集合

#### 属性类型
应用界面字段[无存储]

#### 数据类型
文本，可指定长度

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
参照数据字典【[产品平台（动态）（ProductBranch）](../../codelist/ProductBranch)】

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-关联计划（PLANS）
#### 属性说明
关联计划

#### 属性类型
应用界面字段[无存储]

#### 数据类型
文本，可指定长度

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
参照数据字典【[产品计划（动态）（ProductPlan）](../../codelist/ProductPlan)】

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-关联数据数组（SRFARRAY）
#### 属性说明
关联数据数组

#### 属性类型
应用界面字段[无存储]

#### 数据类型
文本，可指定长度

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-备注（COMMENT）
#### 属性说明
备注

#### 属性类型
应用界面字段[无存储]

#### 数据类型
HTML文本，没有长度限制

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-时间段（PERIOD）
#### 属性说明
时间段

#### 属性类型
应用界面字段[无存储]

#### 数据类型
单项选择(文本值)

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
参照数据字典【[时间周期（Zt__delta）](../../codelist/Zt__delta)】

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-项目团队成员（ACCOUNT）
#### 属性说明
项目团队成员

#### 属性类型
应用界面字段[无存储]

#### 数据类型
单项选择(文本值)

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
参照数据字典【[用户真实名称（动态）（UserRealName）](../../codelist/UserRealName)】

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-加盟日（JOIN）
#### 属性说明
加盟日

#### 属性类型
应用界面字段[无存储]

#### 数据类型
日期型

#### Java类型
Timestamp

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
时间格式：yyyy-MM-dd

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-可用工时/天（HOURS）
#### 属性说明
可用工时/天

#### 属性类型
应用界面字段[无存储]

#### 数据类型
数值

#### Java类型
BigDecimal

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-角色（ROLE）
#### 属性说明
角色

#### 属性类型
应用界面字段[无存储]

#### 数据类型
文本，可指定长度

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-任务消耗总工时（TOTALCONSUMED）
#### 属性说明
任务消耗总工时

#### 属性类型
逻辑字段[来自计算式]

#### 数据类型
浮点

#### Java类型
Double

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
(SELECT round(SUM(CONSUMED),0) FROM ZT_TASK WHERE PROJECT = %1$s AND DELETED = '0' AND ( `parent` = '' or `parent` = '0' or `parent` = '-1'))

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-总工时（TOTALWH）
#### 属性说明
总工时

#### 属性类型
逻辑字段[来自计算式]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
((SELECT round(SUM( `LEFT` ),0) FROM ZT_TASK WHERE PROJECT = t1.`ID` AND DELETED = '0' AND ( `parent` = '' OR `parent` = '0' OR `parent` = '-1' ) AND `status` in ('doing','wait','pause')) + (SELECT round(SUM( CONSUMED ),0) FROM ZT_TASK WHERE PROJECT = t1.`ID` AND DELETED = '0' AND ( `parent` = '' OR `parent` = '0' OR `parent` = '-1' )))

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-任务预计剩余总工时（TOTALLEFT）
#### 属性说明
任务预计剩余总工时

#### 属性类型
逻辑字段[来自计算式]

#### 数据类型
浮点

#### Java类型
Double

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
(SELECT round(SUM(`LEFT`),0) FROM ZT_TASK WHERE PROJECT = t1.`ID` AND DELETED = '0' and `status` in ('doing','wait','pause') AND ( `parent` = '' or `parent` = '0' or `parent` = '-1'))

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-任务最初预计总工时（TOTALESTIMATE）
#### 属性说明
任务最初预计总工时

#### 属性类型
逻辑字段[来自计算式]

#### 数据类型
浮点

#### Java类型
Double

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
(SELECT round(SUM(ESTIMATE),0) FROM ZT_TASK WHERE PROJECT = %1$s AND DELETED =  '0' AND ( `parent` = '' or `parent` = '0' or `parent` = '-1'))

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-可用工时（TOTALHOURS）
#### 属性说明
可用工时

#### 属性类型
逻辑字段[来自计算式]

#### 数据类型
数值

#### Java类型
BigDecimal

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
(select sum(days * hours)  from zt_team tt where type = 'project' and root = t1.id)

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-移动端图片（MOBIMAGE）
#### 属性说明
移动端图片

#### 属性类型
应用界面字段[无存储]

#### 数据类型
文本，可指定长度

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-项目团队相关成员（ACCOUNTS）
#### 属性说明
项目团队相关成员

#### 属性类型
应用界面字段[无存储]

#### 数据类型
文本，可指定长度

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-项目排序（ORDER1）
#### 属性说明
项目排序

#### 属性类型
逻辑字段[来自计算式]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
t1.`order`

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-是否置顶（ISTOP）
#### 属性说明
是否置顶

#### 属性类型
逻辑字段[来自计算式]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
'0'

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-选择部门（DEPT）
#### 属性说明
选择部门

#### 属性类型
应用界面字段[无存储]

#### 数据类型
单项选择(文本值)

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
参照数据字典【[真实部门（RealDept）](../../codelist/RealDept)】

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-复制团队（MANAGEMEMBERS）
#### 属性说明
复制团队

#### 属性类型
应用界面字段[无存储]

#### 数据类型
单项选择(文本值)

#### Java类型
String

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-版本总数（BUILDCNT）
#### 属性说明
版本总数

#### 属性类型
应用界面字段[无存储]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-团队成员总数（TEAMCNT）
#### 属性说明
团队成员总数

#### 属性类型
应用界面字段[无存储]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-所有任务数（ALLTASKCNT）
#### 属性说明
所有任务数

#### 属性类型
应用界面字段[无存储]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-未关闭任务数（UNCLOSETASKCNT）
#### 属性说明
未关闭任务数

#### 属性类型
应用界面字段[无存储]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-指派给我任务数（ASSTOMYTASKCNT）
#### 属性说明
指派给我任务数

#### 属性类型
应用界面字段[无存储]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-未开始任务数（UNSTARTTASKCNT）
#### 属性说明
未开始任务数

#### 属性类型
应用界面字段[无存储]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-更多任务数（MORETASKCNT）
#### 属性说明
更多任务数

#### 属性类型
应用界面字段[无存储]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-进行中任务数（YSTARTASKCNT）
#### 属性说明
进行中任务数

#### 属性类型
应用界面字段[无存储]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-未完成任务数（UNCOMPLETETASKCNT）
#### 属性说明
未完成任务数

#### 属性类型
应用界面字段[无存储]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-已完成任务数（YCOMPLETETASKCNT）
#### 属性说明
已完成任务数

#### 属性类型
应用界面字段[无存储]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-我完成任务数（MYCOMPLETETASKCNT）
#### 属性说明
我完成任务数

#### 属性类型
应用界面字段[无存储]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-关闭任务数（CLOSETASKCNT）
#### 属性说明
关闭任务数

#### 属性类型
应用界面字段[无存储]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-取消任务数（CANCELTASKCNT）
#### 属性说明
取消任务数

#### 属性类型
应用界面字段[无存储]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |

### 属性-需求变更数（STORYCHANGECNT）
#### 属性说明
需求变更数

#### 属性类型
应用界面字段[无存储]

#### 数据类型
整型

#### Java类型
Integer

#### 是否允许为为空
是

#### 默认值
| 项目 | 说明 |
| -- | -- |
| 类型 |  |
| 值 |  |

#### 取值范围/公式
无

#### 数据格式
无

#### 关系属性
| 项目 | 说明 |
| -- | -- |
| 关系实体 | [项目（ZT_PROJECT）](../zentao/Project) |
| 关系属性 | [项目编号（ID）](../zentao/Project/#属性-项目编号（ID）) |
| 关系类型 | 关系属性 1:N 当前属性 |


## 业务状态
| 序号 | 状态名称 | [项目状态](#属性-项目状态（STATUS）)<br>（STATUS） | [是否置顶](#属性-是否置顶（ISTOP）)<br>（ISTOP） | 默认 |
| -- | -- | -- | -- | -- |
| 1 | [已关闭_未置顶](#业务状态-已关闭_未置顶（closed_0）) | closed | 0 |  | 否 |
| 2 | [已关闭_已置顶](#业务状态-已关闭_已置顶（closed_1）) | closed | 1 |  | 否 |
| 3 | [进行中_未置顶](#业务状态-进行中_未置顶（doing_0）) | doing | 0 |  | 否 |
| 4 | [进行中_已置顶](#业务状态-进行中_已置顶（doing_1）) | doing | 1 |  | 否 |
| 5 | [已挂起_未置顶](#业务状态-已挂起_未置顶（suspended_0）) | suspended | 0 |  | 否 |
| 6 | [已挂起_已置顶](#业务状态-已挂起_已置顶（suspended_1）) | suspended | 1 |  | 否 |
| 7 | [未开始_未置顶](#业务状态-未开始_未置顶（wait_0）) | wait | 0 |  | 否 |
| 8 | [未开始_已置顶](#业务状态-未开始_已置顶（wait_1）) | wait | 1 |  | 否 |
### 业务状态-已关闭_未置顶（closed_0）
#### 说明
已关闭_未置顶

#### 是否是默认值
否

#### 状态值
| 属性名 | 状态值 |
| -- | -- |
| [项目状态](#属性-项目状态（STATUS）)<br>（STATUS） | closed |
| [是否置顶](#属性-是否置顶（ISTOP）)<br>（ISTOP） | 0 |


#### 流程相关状态
无

#### 实体行为控制
允许模式：允许

| 序号 | 实体行为 |
| -- | -- |
| 1 | [Update](#实体行为-Update（Update）)<br>（Update） |
| 2 | [Save](#实体行为-Save（Save）)<br>（Save） |
| 3 | [Get](#实体行为-Get（Get）)<br>（Get） |
| 4 | [激活](#实体行为-激活（Activate）)<br>（Activate） |
| 5 | [Remove](#实体行为-Remove（Remove）)<br>（Remove） |

#### 操作权限控制
允许模式：拒绝
拒绝提示信息：无
| 序号 | 操作权限 |
| -- | -- |
| 1 | [项目关闭](#操作权限-项目关闭（SRFUR__PROJ_CLOSED_BUT）)<br>（SRFUR__PROJ_CLOSED_BUT） |
| 2 | [项目挂起](#操作权限-项目挂起（SRFUR__PROJ_SUSPEND_BUT）)<br>（SRFUR__PROJ_SUSPEND_BUT） |
| 3 | [项目延期](#操作权限-项目延期（SRFUR__PROJ_DELAY_BUT）)<br>（SRFUR__PROJ_DELAY_BUT） |
| 4 | [项目开始](#操作权限-项目开始（SRFUR__PROJ_START_BUT）)<br>（SRFUR__PROJ_START_BUT） |
| 5 | [置顶](#操作权限-置顶（TOP）)<br>（TOP） |
### 业务状态-已关闭_已置顶（closed_1）
#### 说明
已关闭_已置顶

#### 是否是默认值
否

#### 状态值
| 属性名 | 状态值 |
| -- | -- |
| [项目状态](#属性-项目状态（STATUS）)<br>（STATUS） | closed |
| [是否置顶](#属性-是否置顶（ISTOP）)<br>（ISTOP） | 1 |


#### 流程相关状态
无

#### 实体行为控制
允许模式：允许


#### 操作权限控制
允许模式：拒绝
拒绝提示信息：无
| 序号 | 操作权限 |
| -- | -- |
| 1 | [项目关闭](#操作权限-项目关闭（SRFUR__PROJ_CLOSED_BUT）)<br>（SRFUR__PROJ_CLOSED_BUT） |
| 2 | [未置顶](#操作权限-未置顶（NOTOP）)<br>（NOTOP） |
| 3 | [项目挂起](#操作权限-项目挂起（SRFUR__PROJ_SUSPEND_BUT）)<br>（SRFUR__PROJ_SUSPEND_BUT） |
| 4 | [项目延期](#操作权限-项目延期（SRFUR__PROJ_DELAY_BUT）)<br>（SRFUR__PROJ_DELAY_BUT） |
| 5 | [项目开始](#操作权限-项目开始（SRFUR__PROJ_START_BUT）)<br>（SRFUR__PROJ_START_BUT） |
### 业务状态-进行中_未置顶（doing_0）
#### 说明
进行中_未置顶

#### 是否是默认值
否

#### 状态值
| 属性名 | 状态值 |
| -- | -- |
| [项目状态](#属性-项目状态（STATUS）)<br>（STATUS） | doing |
| [是否置顶](#属性-是否置顶（ISTOP）)<br>（ISTOP） | 0 |


#### 流程相关状态
无

#### 实体行为控制
允许模式：允许

| 序号 | 实体行为 |
| -- | -- |
| 1 | [关闭](#实体行为-关闭（Close）)<br>（Close） |
| 2 | [挂起](#实体行为-挂起（Suspend）)<br>（Suspend） |
| 3 | [Save](#实体行为-Save（Save）)<br>（Save） |
| 4 | [Remove](#实体行为-Remove（Remove）)<br>（Remove） |
| 5 | [Get](#实体行为-Get（Get）)<br>（Get） |
| 6 | [延期](#实体行为-延期（Putoff）)<br>（Putoff） |
| 7 | [Update](#实体行为-Update（Update）)<br>（Update） |

#### 操作权限控制
允许模式：拒绝
拒绝提示信息：无
| 序号 | 操作权限 |
| -- | -- |
| 1 | [项目激活](#操作权限-项目激活（SRFUR__PROJ_ACTIVATION_BUT）)<br>（SRFUR__PROJ_ACTIVATION_BUT） |
| 2 | [项目开始](#操作权限-项目开始（SRFUR__PROJ_START_BUT）)<br>（SRFUR__PROJ_START_BUT） |
| 3 | [置顶](#操作权限-置顶（TOP）)<br>（TOP） |
### 业务状态-进行中_已置顶（doing_1）
#### 说明
进行中_已置顶

#### 是否是默认值
否

#### 状态值
| 属性名 | 状态值 |
| -- | -- |
| [项目状态](#属性-项目状态（STATUS）)<br>（STATUS） | doing |
| [是否置顶](#属性-是否置顶（ISTOP）)<br>（ISTOP） | 1 |


#### 流程相关状态
无

#### 实体行为控制
允许模式：允许


#### 操作权限控制
允许模式：拒绝
拒绝提示信息：无
| 序号 | 操作权限 |
| -- | -- |
| 1 | [项目开始](#操作权限-项目开始（SRFUR__PROJ_START_BUT）)<br>（SRFUR__PROJ_START_BUT） |
| 2 | [项目激活](#操作权限-项目激活（SRFUR__PROJ_ACTIVATION_BUT）)<br>（SRFUR__PROJ_ACTIVATION_BUT） |
| 3 | [未置顶](#操作权限-未置顶（NOTOP）)<br>（NOTOP） |
### 业务状态-已挂起_未置顶（suspended_0）
#### 说明
已挂起_未置顶

#### 是否是默认值
否

#### 状态值
| 属性名 | 状态值 |
| -- | -- |
| [项目状态](#属性-项目状态（STATUS）)<br>（STATUS） | suspended |
| [是否置顶](#属性-是否置顶（ISTOP）)<br>（ISTOP） | 0 |


#### 流程相关状态
无

#### 实体行为控制
允许模式：允许


#### 操作权限控制
允许模式：拒绝
拒绝提示信息：无
| 序号 | 操作权限 |
| -- | -- |
| 1 | [项目开始](#操作权限-项目开始（SRFUR__PROJ_START_BUT）)<br>（SRFUR__PROJ_START_BUT） |
| 2 | [置顶](#操作权限-置顶（TOP）)<br>（TOP） |
| 3 | [项目挂起](#操作权限-项目挂起（SRFUR__PROJ_SUSPEND_BUT）)<br>（SRFUR__PROJ_SUSPEND_BUT） |
| 4 | [项目延期](#操作权限-项目延期（SRFUR__PROJ_DELAY_BUT）)<br>（SRFUR__PROJ_DELAY_BUT） |
### 业务状态-已挂起_已置顶（suspended_1）
#### 说明
已挂起_已置顶

#### 是否是默认值
否

#### 状态值
| 属性名 | 状态值 |
| -- | -- |
| [项目状态](#属性-项目状态（STATUS）)<br>（STATUS） | suspended |
| [是否置顶](#属性-是否置顶（ISTOP）)<br>（ISTOP） | 1 |


#### 流程相关状态
无

#### 实体行为控制
允许模式：允许

| 序号 | 实体行为 |
| -- | -- |
| 1 | [Remove](#实体行为-Remove（Remove）)<br>（Remove） |
| 2 | [Get](#实体行为-Get（Get）)<br>（Get） |
| 3 | [激活](#实体行为-激活（Activate）)<br>（Activate） |
| 4 | [Save](#实体行为-Save（Save）)<br>（Save） |
| 5 | [关闭](#实体行为-关闭（Close）)<br>（Close） |

#### 操作权限控制
允许模式：拒绝
拒绝提示信息：无
| 序号 | 操作权限 |
| -- | -- |
| 1 | [未置顶](#操作权限-未置顶（NOTOP）)<br>（NOTOP） |
| 2 | [项目延期](#操作权限-项目延期（SRFUR__PROJ_DELAY_BUT）)<br>（SRFUR__PROJ_DELAY_BUT） |
| 3 | [项目挂起](#操作权限-项目挂起（SRFUR__PROJ_SUSPEND_BUT）)<br>（SRFUR__PROJ_SUSPEND_BUT） |
| 4 | [项目开始](#操作权限-项目开始（SRFUR__PROJ_START_BUT）)<br>（SRFUR__PROJ_START_BUT） |
### 业务状态-未开始_未置顶（wait_0）
#### 说明
未开始_未置顶

#### 是否是默认值
否

#### 状态值
| 属性名 | 状态值 |
| -- | -- |
| [项目状态](#属性-项目状态（STATUS）)<br>（STATUS） | wait |
| [是否置顶](#属性-是否置顶（ISTOP）)<br>（ISTOP） | 0 |


#### 流程相关状态
无

#### 实体行为控制
允许模式：允许

| 序号 | 实体行为 |
| -- | -- |
| 1 | [关闭](#实体行为-关闭（Close）)<br>（Close） |
| 2 | [挂起](#实体行为-挂起（Suspend）)<br>（Suspend） |
| 3 | [Get](#实体行为-Get（Get）)<br>（Get） |
| 4 | [Remove](#实体行为-Remove（Remove）)<br>（Remove） |
| 5 | [开始](#实体行为-开始（Start）)<br>（Start） |
| 6 | [Update](#实体行为-Update（Update）)<br>（Update） |
| 7 | [延期](#实体行为-延期（Putoff）)<br>（Putoff） |
| 8 | [Save](#实体行为-Save（Save）)<br>（Save） |

#### 操作权限控制
允许模式：拒绝
拒绝提示信息：无
| 序号 | 操作权限 |
| -- | -- |
| 1 | [项目激活](#操作权限-项目激活（SRFUR__PROJ_ACTIVATION_BUT）)<br>（SRFUR__PROJ_ACTIVATION_BUT） |
| 2 | [置顶](#操作权限-置顶（TOP）)<br>（TOP） |
### 业务状态-未开始_已置顶（wait_1）
#### 说明
未开始_已置顶

#### 是否是默认值
否

#### 状态值
| 属性名 | 状态值 |
| -- | -- |
| [项目状态](#属性-项目状态（STATUS）)<br>（STATUS） | wait |
| [是否置顶](#属性-是否置顶（ISTOP）)<br>（ISTOP） | 1 |


#### 流程相关状态
无

#### 实体行为控制
允许模式：允许


#### 操作权限控制
允许模式：拒绝
拒绝提示信息：无
| 序号 | 操作权限 |
| -- | -- |
| 1 | [未置顶](#操作权限-未置顶（NOTOP）)<br>（NOTOP） |
| 2 | [项目激活](#操作权限-项目激活（SRFUR__PROJ_ACTIVATION_BUT）)<br>（SRFUR__PROJ_ACTIVATION_BUT） |

## 实体行为
| 序号 | 行为 | 行为名 | 行为类型 | 行为持有者 |
| -- | -- | -- | -- | -- |
| 1 | [Create](#实体行为-Create（Create）) | Create | 内置方法 | 后台及前台 |
| 2 | [Update](#实体行为-Update（Update）) | Update | 内置方法 | 后台及前台 |
| 3 | [Remove](#实体行为-Remove（Remove）) | Remove | 内置方法 | 后台及前台 |
| 4 | [Get](#实体行为-Get（Get）) | Get | 内置方法 | 后台及前台 |
| 5 | [GetDraft](#实体行为-GetDraft（GetDraft）) | GetDraft | 内置方法 | 后台及前台 |
| 6 | [激活](#实体行为-激活（Activate）) | Activate | 用户自定义 | 后台及前台 |
| 7 | [批量解除关联需求](#实体行为-批量解除关联需求（BatchUnlinkStory）) | BatchUnlinkStory | 用户自定义 | 后台及前台 |
| 8 | [取消置顶](#实体行为-取消置顶（CancelProjectTop）) | CancelProjectTop | 实体处理逻辑 | 后台 |
| 9 | [CheckKey](#实体行为-CheckKey（CheckKey）) | CheckKey | 内置方法 | 后台及前台 |
| 10 | [关闭](#实体行为-关闭（Close）) | Close | 用户自定义 | 后台及前台 |
| 11 | [关联需求](#实体行为-关联需求（LinkStory）) | LinkStory | 用户自定义 | 后台及前台 |
| 12 | [团队管理](#实体行为-团队管理（ManageMembers）) | ManageMembers | 用户自定义 | 后台及前台 |
| 13 | [移动端项目计数器](#实体行为-移动端项目计数器（MobProjectCount）) | MobProjectCount | 实体处理逻辑 | 后台 |
| 14 | [项目任务快速分组计数器](#实体行为-项目任务快速分组计数器（ProjectTaskQCnt）) | ProjectTaskQCnt | 实体处理逻辑 | 后台 |
| 15 | [置顶](#实体行为-置顶（projectTop）) | projectTop | 实体处理逻辑 | 后台 |
| 16 | [延期](#实体行为-延期（Putoff）) | Putoff | 用户自定义 | 后台及前台 |
| 17 | [退出](#实体行为-退出（ReturnEdit）) | ReturnEdit | 用户自定义 | 前台 |
| 18 | [Save](#实体行为-Save（Save）) | Save | 内置方法 | 后台及前台 |
| 19 | [开始](#实体行为-开始（Start）) | Start | 用户自定义 | 后台及前台 |
| 20 | [挂起](#实体行为-挂起（Suspend）) | Suspend | 用户自定义 | 后台及前台 |
| 21 | [移除成员](#实体行为-移除成员（UnlinkMember）) | UnlinkMember | 用户自定义 | 后台及前台 |
| 22 | [解除关联需求](#实体行为-解除关联需求（UnlinkStory）) | UnlinkStory | 用户自定义 | 后台及前台 |
| 23 | [根据结束日期更新可用工作日](#实体行为-根据结束日期更新可用工作日（UpdateCycle）) | UpdateCycle | 用户自定义 | 前台 |
| 24 | [排序](#实体行为-排序（UpdateOrder）) | UpdateOrder | 用户自定义 | 后台及前台 |
| 25 | [更新项目周期及可用工作日](#实体行为-更新项目周期及可用工作日（UpdateProjectCycle）) | UpdateProjectCycle | 用户自定义 | 前台 |
### 实体行为-Create（Create）
#### 说明
Create

#### 行为类型
内置方法

#### 行为持有者
后台及前台

### 实体行为-Update（Update）
#### 说明
Update

#### 行为类型
内置方法

#### 行为持有者
后台及前台

### 实体行为-Remove（Remove）
#### 说明
Remove

#### 行为类型
内置方法

#### 行为持有者
后台及前台

### 实体行为-Get（Get）
#### 说明
Get

#### 行为类型
内置方法

#### 行为持有者
后台及前台

### 实体行为-GetDraft（GetDraft）
#### 说明
GetDraft

#### 行为类型
内置方法

#### 行为持有者
后台及前台

### 实体行为-激活（Activate）
#### 说明
激活

#### 行为类型
用户自定义

#### 行为持有者
后台及前台

### 实体行为-批量解除关联需求（BatchUnlinkStory）
#### 说明
批量解除关联需求

#### 行为类型
用户自定义

#### 行为持有者
后台及前台

### 实体行为-取消置顶（CancelProjectTop）
#### 说明
项目置顶操作

#### 行为类型
实体处理逻辑

#### 行为持有者
后台

### 实体行为-CheckKey（CheckKey）
#### 说明
CheckKey

#### 行为类型
内置方法

#### 行为持有者
后台及前台

### 实体行为-关闭（Close）
#### 说明
关闭

#### 行为类型
用户自定义

#### 行为持有者
后台及前台

### 实体行为-关联需求（LinkStory）
#### 说明
关联需求

#### 行为类型
用户自定义

#### 行为持有者
后台及前台

### 实体行为-团队管理（ManageMembers）
#### 说明
团队管理

#### 行为类型
用户自定义

#### 行为持有者
后台及前台

### 实体行为-移动端项目计数器（MobProjectCount）
#### 说明
移动端项目计数器

#### 行为类型
实体处理逻辑

#### 行为持有者
后台

### 实体行为-项目任务快速分组计数器（ProjectTaskQCnt）
#### 说明
项目任务快速分组计数器

#### 行为类型
实体处理逻辑

#### 行为持有者
后台

### 实体行为-置顶（projectTop）
#### 说明
项目置顶操作

#### 行为类型
实体处理逻辑

#### 行为持有者
后台

### 实体行为-延期（Putoff）
#### 说明
延期

#### 行为类型
用户自定义

#### 行为持有者
后台及前台

### 实体行为-退出（ReturnEdit）
#### 说明
退出

#### 行为类型
用户自定义

#### 行为持有者
前台

### 实体行为-Save（Save）
#### 说明
Save

#### 行为类型
内置方法

#### 行为持有者
后台及前台

### 实体行为-开始（Start）
#### 说明
开始

#### 行为类型
用户自定义

#### 行为持有者
后台及前台

### 实体行为-挂起（Suspend）
#### 说明
挂起

#### 行为类型
用户自定义

#### 行为持有者
后台及前台

### 实体行为-移除成员（UnlinkMember）
#### 说明
移除成员

#### 行为类型
用户自定义

#### 行为持有者
后台及前台

### 实体行为-解除关联需求（UnlinkStory）
#### 说明
解除关联需求

#### 行为类型
用户自定义

#### 行为持有者
后台及前台

### 实体行为-根据结束日期更新可用工作日（UpdateCycle）
#### 说明
根据结束日期更新可用工作日

#### 行为类型
用户自定义

#### 行为持有者
前台

### 实体行为-排序（UpdateOrder）
#### 说明
排序

#### 行为类型
用户自定义

#### 行为持有者
后台及前台

### 实体行为-更新项目周期及可用工作日（UpdateProjectCycle）
#### 说明
更新项目周期及可用工作日

#### 行为类型
用户自定义

#### 行为持有者
前台



