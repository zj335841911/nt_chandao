...
| 18 | [本周完成的任务(汇报)](#数据集合-本周完成的任务(汇报)（ThisWeekCompleteTaskZS）) | ThisWeekCompleteTaskZS | 否 |
| 19 | [todo列表查询](#数据集合-todo列表查询（TodoListTask）) | TodoListTask | 否 |
| 20 | [任务类型分组](#数据集合-任务类型分组（TypeGroup）) | TypeGroup | 否 |
