...
| 5 | [产品需求汇总查询](#数据查询-产品需求汇总查询（ProductStorySum）) | ProductStorySum | 否 |
...
sum( IF ( t1.`deleted` = '0', t1.`rowcnt`, 0 ) ) AS `STORYCNT` 
from (select t1.`id`, t1.`name`, t1.`po`, t2.`stage`, t2.`deleted`, 1 as `rowcnt` from zt_product t1 left join zt_story t2 on t1.`id` = t2.`product` where t1.`deleted` = '0') t1 group by t1.id
```
### 数据查询-默认（全部数据）（View）
#### 说明
默认（全部数据）
