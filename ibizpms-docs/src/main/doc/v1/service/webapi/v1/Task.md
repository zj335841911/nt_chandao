...
| 70 | plan | Long | 允许 | 编号 |
| 71 | taskspecies | String | 允许 | 任务种别 |
| 72 | configweek | String | 允许 | 周期设置周几 |
| 73 | configmonth | String | 允许 | 周期设置月 |
| 74 | configtype | String | 允许 | 周期类型 |
| 75 | configbeforedays | Integer | 允许 | 提前天数 |
| 76 | configday | Integer | 允许 | 间隔天数 |
| 77 | configend | Timestamp | 允许 | 过期日期<br>时间格式：yyyy-MM-dd |
| 78 | configbegin | Timestamp | 允许 | 开始日期<br>时间格式：yyyy-MM-dd |
...
| 33 | n_project_in | Long | 允许 | 条件字段：project<br>条件组合方式：`in(...)` |
| 34 | n_story_eq | Long | 允许 | 条件字段：story<br>条件组合方式：`=` |
| 35 | n_parent_eq | Long | 允许 | 条件字段：parent<br>条件组合方式：`=` |
| 36 | n_parent_gtandeq | Long | 允许 | 条件字段：parent<br>条件组合方式：`>=` |
| 37 | n_frombug_eq | Long | 允许 | 条件字段：frombug<br>条件组合方式：`=` |
| 38 | n_module_eq | Long | 允许 | 条件字段：module<br>条件组合方式：`=` |
| 39 | n_path_like | String | 允许 | 条件字段：path<br>条件组合方式：`%like%` |
| 40 | n_status1_eq | String | 允许 | 条件字段：status1<br>条件组合方式：`=` |
| 41 | n_tasktype_eq | String | 允许 | 条件字段：tasktype<br>条件组合方式：`=` |
| 42 | n_plan_eq | Long | 允许 | 条件字段：plan<br>条件组合方式：`=` |
| 43 | n_taskspecies_eq | String | 允许 | 条件字段：taskspecies<br>条件组合方式：`=` |
| 44 | n_config_type_eq | String | 允许 | 条件字段：config_type<br>条件组合方式：`=` |
| 45 | n_planname_eq | String | 允许 | 条件字段：planname<br>条件组合方式：`=` |
| 46 | n_planname_like | String | 允许 | 条件字段：planname<br>条件组合方式：`%like%` |
| 47 | n_cycle_eq | Integer | 允许 | 条件字段：cycle<br>条件组合方式：`=` |
