...
| 34 | n_frombug_eq | Long | 允许 | 条件字段：frombug<br>条件组合方式：`=` |
| 35 | n_module_eq | Long | 允许 | 条件字段：module<br>条件组合方式：`=` |
| 36 | n_path_like | String | 允许 | 条件字段：path<br>条件组合方式：`%like%` |
| 37 | n_status1_eq | String | 允许 | 条件字段：status1<br>条件组合方式：`=` |
| 38 | n_tasktype_eq | String | 允许 | 条件字段：tasktype<br>条件组合方式：`=` |
| 39 | customcond | String | 允许 | 自定义查询条件 |
| 40 | customparams | String | 允许 | 自定义查询参数 |
| 41 | query | String | 允许 | 快速搜索 |
| 42 | filter | QueryFilter | 允许 | 条件表达式<br>参照`cn.ibizlab.pms.util.filter.QueryFilter` |
| 43 | page | int | 允许 | 当前页数<br>默认值0 |
| 44 | size | int | 允许 | 每页显示条数<br>默认值20 |
| 45 | sort | String | 允许 | 排序 |
