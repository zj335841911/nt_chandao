...
| 31 | n_module_eq | Long | 允许 | 条件字段：module<br>条件组合方式：`=` |
| 32 | n_product_eq | Long | 允许 | 条件字段：product<br>条件组合方式：`=` |
| 33 | n_duplicatestory_eq | Long | 允许 | 条件字段：duplicatestory<br>条件组合方式：`=` |
| 34 | n_branch_eq | Long | 允许 | 条件字段：branch<br>条件组合方式：`=` |
| 35 | n_tobug_eq | Long | 允许 | 条件字段：tobug<br>条件组合方式：`=` |
| 36 | n_result_eq | String | 允许 | 条件字段：result<br>条件组合方式：`=` |
| 37 | n_branchname_eq | String | 允许 | 条件字段：branchname<br>条件组合方式：`=` |
| 38 | n_branchname_like | String | 允许 | 条件字段：branchname<br>条件组合方式：`%like%` |
| 39 | n_preversion_eq | Integer | 允许 | 条件字段：preversion<br>条件组合方式：`=` |
