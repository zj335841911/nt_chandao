...
| 24 | submittime | Timestamp | 允许 | 提交时间<br>时间格式：HH:mm:ss |
| 25 | <动态属性> | Object | 允许 | 支持动态属性 |

#### IbzReportSearchContext
| 序号 | 属性名 | 属性类型 | 是否可以为空 | 说明 |
| ---- | ---- | ---- | ---- | ---- |
| 1 | n_reportto_eq | String | 允许 | 条件字段：reportto<br>条件组合方式：`=`<br>时间格式：HH:mm:ss |
| 2 | n_issubmit_eq | String | 允许 | 条件字段：issubmit<br>条件组合方式：`=`<br>时间格式：HH:mm:ss |
| 3 | n_reportstatus_eq | String | 允许 | 条件字段：reportstatus<br>条件组合方式：`=`<br>时间格式：HH:mm:ss |
| 4 | n_account_eq | String | 允许 | 条件字段：account<br>条件组合方式：`=`<br>时间格式：HH:mm:ss |
| 5 | n_ibz_dailyname_like | String | 允许 | 条件字段：ibz_dailyname<br>条件组合方式：`%like%`<br>时间格式：HH:mm:ss |
| 6 | n_type_eq | String | 允许 | 条件字段：type<br>条件组合方式：`=`<br>时间格式：HH:mm:ss |
