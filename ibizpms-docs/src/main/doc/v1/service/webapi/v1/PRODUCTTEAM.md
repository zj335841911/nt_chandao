...
| 3 | n_join_gtandeq | Timestamp | 允许 | 条件字段：join<br>条件组合方式：`>=`<br>时间格式：yyyy-MM-dd |
| 4 | n_end_ltandeq | Timestamp | 允许 | 条件字段：end<br>条件组合方式：`<=`<br>时间格式：yyyy-MM-dd |
| 5 | n_type_eq | String | 允许 | 条件字段：type<br>条件组合方式：`=` |
| 6 | n_leadingcadre_eq | String | 允许 | 条件字段：leadingcadre<br>条件组合方式：`=` |
| 7 | n_limited_eq | String | 允许 | 条件字段：limited<br>条件组合方式：`=` |
| 8 | n_root_eq | Long | 允许 | 条件字段：root<br>条件组合方式：`=` |
| 9 | n_root_in | Long | 允许 | 条件字段：root<br>条件组合方式：`in(...)` |
