...
WHERE FIND_IN_SET(t1.id,(select t1.todaytask from t_ibz_daily t1 where t1.IBZ_DAILYID =  #{srf.sessioncontext.srfloginname}
...
LEFT JOIN zt_task t51 ON t1.PARENT = t51.ID
WHERE t1.DELETED = '0' 
FIND_IN_SET(t1.id,(select thismonthtask from t_ibz_monthly where IBZ_MONTHLYID = #{srf.sessioncontext.srfloginname})) 
