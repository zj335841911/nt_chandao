...
| 序号 | 属性名 | 属性类型 | 是否可以为空 | 说明 |
| -- | -- | -- | -- | -- |
| 56 | n_canceledby_eq | String | 允许 | 条件字段：canceledby<br>条件组合方式：`=` |
| 57 | n_color_eq | String | 允许 | 条件字段：color<br>条件组合方式：`=` |
| 58 | n_id_noteq | Long | 允许 | 条件字段：id<br>条件组合方式：`!=`或者`<>` |
| 59 | n_finishedby_eq | String | 允许 | 条件字段：finishedby<br>条件组合方式：`=` |
| 60 | n_closedby_eq | String | 允许 | 条件字段：closedby<br>条件组合方式：`=` |
| 61 | n_closedreason_eq | String | 允许 | 条件字段：closedreason<br>条件组合方式：`=` |
| 62 | n_assigneddate_gtandeq | Timestamp | 允许 | 条件字段：assigneddate<br>条件组合方式：`>=`<br>时间格式：yyyy-MM-dd HH:mm:ss |
| 63 | n_pri_eq | Integer | 允许 | 条件字段：pri<br>条件组合方式：`=` |
| 64 | n_lasteditedby_eq | String | 允许 | 条件字段：lasteditedby<br>条件组合方式：`=` |
| 65 | n_status_eq | String | 允许 | 条件字段：status<br>条件组合方式：`=` |
| 66 | n_status_in | String | 允许 | 条件字段：status<br>条件组合方式：`in(...)` |
| 67 | n_status_noteq | String | 允许 | 条件字段：status<br>条件组合方式：`!=`或者`<>` |
| 68 | n_name_like | String | 允许 | 条件字段：name<br>条件组合方式：`%like%` |
| 69 | n_closeddate_ltandeq | Timestamp | 允许 | 条件字段：closeddate<br>条件组合方式：`<=`<br>时间格式：yyyy-MM-dd HH:mm:ss |
| 70 | n_type_eq | String | 允许 | 条件字段：type<br>条件组合方式：`=` |
| 71 | n_assignedto_eq | String | 允许 | 条件字段：assignedto<br>条件组合方式：`=` |
| 72 | n_eststarted_gtandeq | Timestamp | 允许 | 条件字段：eststarted<br>条件组合方式：`>=`<br>时间格式：yyyy-MM-dd |
| 73 | n_eststarted_ltandeq | Timestamp | 允许 | 条件字段：eststarted<br>条件组合方式：`<=`<br>时间格式：yyyy-MM-dd |
| 74 | n_openedby_eq | String | 允许 | 条件字段：openedby<br>条件组合方式：`=` |
| 75 | n_finisheddate_ltandeq | Timestamp | 允许 | 条件字段：finisheddate<br>条件组合方式：`<=`<br>时间格式：yyyy-MM-dd |
| 76 | n_modulename_eq | String | 允许 | 条件字段：modulename<br>条件组合方式：`=` |
| 77 | n_modulename_like | String | 允许 | 条件字段：modulename<br>条件组合方式：`%like%` |
| 78 | n_storyname_eq | String | 允许 | 条件字段：storyname<br>条件组合方式：`=` |
| 79 | n_storyname_like | String | 允许 | 条件字段：storyname<br>条件组合方式：`%like%` |
| 80 | n_projectname_eq | String | 允许 | 条件字段：projectname<br>条件组合方式：`=` |
| 81 | n_projectname_like | String | 允许 | 条件字段：projectname<br>条件组合方式：`%like%` |
| 82 | n_product_eq | Long | 允许 | 条件字段：product<br>条件组合方式：`=` |
| 83 | n_parentname_eq | String | 允许 | 条件字段：parentname<br>条件组合方式：`=` |
| 84 | n_parentname_like | String | 允许 | 条件字段：parentname<br>条件组合方式：`%like%` |
| 85 | n_project_eq | Long | 允许 | 条件字段：project<br>条件组合方式：`=` |
| 86 | n_story_eq | Long | 允许 | 条件字段：story<br>条件组合方式：`=` |
| 87 | n_parent_eq | Long | 允许 | 条件字段：parent<br>条件组合方式：`=` |
| 88 | n_frombug_eq | Long | 允许 | 条件字段：frombug<br>条件组合方式：`=` |
| 89 | n_module_eq | Long | 允许 | 条件字段：module<br>条件组合方式：`=` |
| 90 | n_path_like | String | 允许 | 条件字段：path<br>条件组合方式：`%like%` |
| 91 | n_status1_eq | String | 允许 | 条件字段：status1<br>条件组合方式：`=` |
| 92 | n_tasktype_eq | String | 允许 | 条件字段：tasktype<br>条件组合方式：`=` |
| 93 | customcond | String | 允许 | 自定义查询条件 |
| 94 | customparams | String | 允许 | 自定义查询参数 |
| 95 | query | String | 允许 | 快速搜索 |
| 96 | filter | QueryFilter | 允许 | 条件表达式<br>参照`cn.ibizlab.pms.util.filter.QueryFilter` |
| 97 | page | int | 允许 | 当前页数<br>默认值0 |
| 98 | size | int | 允许 | 每页显示条数<br>默认值20 |
| 99 | sort | String | 允许 | 排序 |
