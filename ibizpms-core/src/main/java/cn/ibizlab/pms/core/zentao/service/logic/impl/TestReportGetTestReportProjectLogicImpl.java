...
import cn.ibizlab.pms.core.zentao.service.logic.ITestReportGetTestReportProjectLogic;
import cn.ibizlab.pms.core.zentao.domain.TestReport;

/**
 * 关系型数据实体[GetTestReportProject] 对象
 */
@Slf4j
@Service
public class TestReportGetTestReportProjectLogicImpl implements ITestReportGetTestReportProjectLogic{
    ...
    public void execute(TestReport et){
        ...
        try{
           ...
           kieSession.setGlobal("testreportgettestreportprojectdefault",et);
           ...
           kieSession.startProcess("cn.ibizlab.pms.core.zentao.service.logic.testreportgettestreportproject");

        }catch(Exception e){
            throw new RuntimeException("执行[根据测试报告获取基本信息（项目报告）]处理逻辑发生异常"+e);
        }finally {
            if(kieSession!=null)
            kieSession.destroy();
        }
